...
function calculateRemainingAmount(service) {
  ...
  const completedPayments = service.payment_schedule?.filter(p => p.status === 'completed' || p.status === 'paid') || [];
...
mongoose.connect(process.env.MONGODB_URI, {
  ...
  services.forEach((service, index) => {
    ...
    service.payment_schedule?.forEach((payment, pIndex) => {
      const statusIcon = payment.status === 'completed' || payment.status === 'paid' ? '✅' : 
                        payment.status === 'pending' || payment.status === 'due' ? '🔄' : '⏳';
  ...
  console.log(`   ✅ Calculation ${testCalculation.remaining === 840 ? 'CORRECT' : 'INCORRECT'}`);
