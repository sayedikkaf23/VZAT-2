...
async function testSubscriptionWebhook() {
  ...
  const isSuccess = args[2] !***REMOVED*** 'failed';
  const amount = parseFloat(args[3]) || 210;
  
  console.log('🚀 ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***= SUBSCRIPTION WEBHOOK TEST ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***=');
  ...
  try {
    ...
    console.log('');
    console.log('🎭 ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***= SIMULATING WEBHOOK ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***=');
    ...
    console.log('');
    console.log('📊 ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***= UPDATED SUBSCRIPTION STATUS ***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED******REMOVED***=');
    ...
    if (updatedSubscription) {
      ...
      if (subscription.subscription_status !***REMOVED*** updatedSubscription.subscription_status) {
        console.log(`  ✅ Subscription Status: ${subscription.subscription_status} → ${updatedSubscription.subscription_status}`);
      }
      
      if ((subscription.payments_completed || 0) !***REMOVED*** (updatedSubscription.payments_completed || 0)) {
      ...
      if (updatedSubscription.payment_schedule && updatedSubscription.payment_schedule.length > 0) {
        ...
        updatedSubscription.payment_schedule.forEach((payment, index) => {
          ...
          if (oldPayment && oldPayment.status !***REMOVED*** payment.status) {
