...
export const getActiveServices = async (req, res) => {
  try {
    ...
    const response = {
      ...
      summary: {
        totalPaid: allPayments.filter(p => p.status ***REMOVED***= 'paid' || p.status ***REMOVED***= 'completed').length,
        totalDue: allPayments.filter(p => p.status ***REMOVED***= 'due').length,
        totalPending: allPayments.filter(p => p.status ***REMOVED***= 'pending').length,
        totalFailed: allPayments.filter(p => p.status ***REMOVED***= 'failed').length,
        totalAmountPaid: allPayments
          .filter(p => p.status ***REMOVED***= 'paid' || p.status ***REMOVED***= 'completed')
          .reduce((sum, p) => sum + (p.amount || 0), 0),
        totalAmountDue: allPayments
          .filter(p => p.status ***REMOVED***= 'due')
          .reduce((sum, p) => sum + (p.amount || 0), 0),
        totalAmountPending: allPayments
          .filter(p => p.status ***REMOVED***= 'pending')
          .reduce((sum, p) => sum + (p.amount || 0), 0)
      }
    };
