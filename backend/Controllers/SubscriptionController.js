  ...
  try {
    ...
    if (isFirstPayment) {
      ...
      try {
        ...
        const currentPayment = subscription.payment_schedule.find(p => p.installment_number ***REMOVED***= updatedRecord.payments_completed);
    ...
    for (const subscription of dueSubscriptions) {
      try {
        ...
        try {
          ...
          const shouldSendEmail = !lastFailureEmailDate || new Date(lastFailureEmailDate).toDateString() !***REMOVED*** todayString;
          ...
          if (shouldSendEmail) {
            ...
            const currentPaymentNumber = (subscription.payments_completed || 0) + 1;
            const failedPayment = subscription.payment_schedule.find(p => p.installment_number ***REMOVED***= currentPaymentNumber);
