...
export const createCardChangePaymentForm = async (req, res) => {
  try {
    ...
    if (!customer || customer.quotepaymentId !***REMOVED*** quotepaymentId) {
...
export const handleCardChangeWebhook = async (req, res) => {
  try {
    ...
    if (paymentType ***REMOVED***= 'PA' && result.code.startsWith('000.') && registrationId) {
...
export const getCardChangeHistory = async (req, res) => {
  try {
    ...
    const cardHistory = cards.map(card => ({
      ...
      isCurrentSubscriptionCard: card.afs_registration_id ***REMOVED***= subscription?.afs_registration_id,
