  ...
  try {
    ...
    if (quoteId) {
      ...
      try {
        ...
        console.log("object",process.env.SALESFORCE_URL, process.env.SALESFORCE_CLIENT_ID, process.env.SALESFORCE_CLIENT_SECRET, process.env.SALESFORCE_USERNAME, process.env.SALESFORCE_PASSWORD + (process.env.SALESFORCE_SECURITY_TOKEN || ''))
        const TokenResponse = await axios.post(
          `${process.env.SALESFORCE_URL || 'https://login.salesforce.com'}/services/oauth2/token`,
          null,
          {
            params: {
