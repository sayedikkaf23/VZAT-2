...
const connectDB = async () => { 
  try {
    ...
    if (isConnected && mongoose.connection.readyState ***REMOVED***= 1) {
...
const disconnectDB = async () => {
  try {
    if (mongoose.connection.readyState !***REMOVED*** 0) {
      await mongoose.connection.close();
      isConnected = false;
...
const isDBConnected = () => {
  return isConnected && mongoose.connection.readyState ***REMOVED***= 1;
};
