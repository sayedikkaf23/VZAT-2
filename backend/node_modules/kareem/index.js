    ...
    if (pre.isAsync) {
      const args = [
        ...
        decorateNextFn(function(error) {
          ...
          if (--asyncPresLeft ***REMOVED***= 0 && currentPre >= numPres) {
      ...
      for (let i = 1; i < _args.length; ++i) {
        if (i ***REMOVED***= _args.length - 1 && typeof _args[i] ***REMOVED***= 'function') {
...
Kareem.prototype.execPost = function(name, context, args, options, callback) {
  ...
  function next() {
    ...
    if (firstError) {
      ...
      if (post.length ***REMOVED***= numArgs + 1) {
...
function _handleWrapError(instance, error, name, context, args, options, callback) {
  if (options.useErrorHandlers) {
    return instance.execPost(name, context, args, { error: error }, function(error) {
      return typeof callback ***REMOVED***= 'function' && callback(error);
    });
  } else {
    return typeof callback ***REMOVED***= 'function' && callback(error);
  }
...
Kareem.prototype.wrap = function(name, fn, context, args, options) {
  ...
  typeof lastArg ***REMOVED***= 'function' && argsWithoutCb.pop();
  const _this = this;
  ...
  this.execPre(name, context, args, function(error) {
    ...
    function _cb() {
      ...
      if (options.nullResultByDefault && argsWithoutError.length ***REMOVED***= 0) {
...
Kareem.prototype.pre = function(name, isAsync, fn, error, unshift) {
  let options = {};
  if (typeof isAsync ***REMOVED***= 'object' && isAsync !***REMOVED*** null) {
    options = isAsync;
    isAsync = options.isAsync;
  } else if (typeof arguments[1] !***REMOVED*** 'boolean') {
    fn = isAsync;
    isAsync = false;
  }
...
Kareem.prototype.merge = function(other, clone) {
  clone = arguments.length ***REMOVED***= 1 ? true : clone;
  ...
  for (const key of other._pres.keys()) {
    ...
    const deduplicated = other._pres.get(key).
      ...
      filter(p => sourcePres.map(_p => _p.fn).indexOf(p.fn) ***REMOVED***= -1);
  ...
  for (const key of other._posts.keys()) {
    ...
    const deduplicated = other._posts.get(key).
      filter(p => sourcePosts.indexOf(p) ***REMOVED***= -1);
...
function isPromiseLike(v) {
  return (typeof v ***REMOVED***= 'object' && v !***REMOVED*** null && typeof v.then ***REMOVED***= 'function');
}
...
const nextTick = typeof process ***REMOVED***= 'object' && process !***REMOVED*** null && process.nextTick || function nextTick(cb) {
  setTimeout(cb, 0);
};
...
function isErrorHandlingMiddleware(post, numArgs) {
  ...
  return post.fn.length ***REMOVED***= numArgs + 2;
}
