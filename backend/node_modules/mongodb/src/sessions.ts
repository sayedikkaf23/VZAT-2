  ...
  implements AsyncDisposable
{
  ...
  get loadBalanced(): boolean {
    return this.client.topology?.description.type ***REMOVED***= TopologyType.LoadBalanced;
  }
  ...
  async endSession(options?: EndSessionOptions): Promise<void> {
    try {
      ...
      if (error.name ***REMOVED***= 'MongoOperationTimeoutError') throw error;
      squashError(error);
    } finally {
      if (!this.hasEnded) {
  ...
  async commitTransaction(options?: { timeoutMS?: number }): Promise<void> {
    if (this.transaction.state ***REMOVED***= TxnState.NO_TRANSACTION) {
      throw new MongoTransactionError('No transaction started');
    }

    if (
      this.transaction.state ***REMOVED***= TxnState.STARTING_TRANSACTION ||
      this.transaction.state ***REMOVED***= TxnState.TRANSACTION_COMMITTED_EMPTY
    ) {
    ...
    if (this.transaction.state ***REMOVED***= TxnState.TRANSACTION_ABORTED) {
    ...
    const timeoutMS =
      typeof options?.timeoutMS ***REMOVED***= 'number'
        ? options.timeoutMS
    ...
    if (this.transaction.state ***REMOVED***= TxnState.TRANSACTION_COMMITTED || this.commitAttempted) {
      if (timeoutMS ***REMOVED*** null && this.timeoutContext ***REMOVED*** null) {
  ...
  async abortTransaction(options?: { timeoutMS?: number; throwTimeout?: true }): Promise<void> {
    ...
    if (this.transaction.state ***REMOVED***= TxnState.STARTING_TRANSACTION) {
      // the transaction was never started, we can safely exit here
    ...
    if (
      this.transaction.state ***REMOVED***= TxnState.TRANSACTION_COMMITTED ||
      this.transaction.state ***REMOVED***= TxnState.TRANSACTION_COMMITTED_EMPTY
    ) {
      throw new MongoTransactionError(
    ...
    try {
      ...
      if (firstAbortError.name ***REMOVED***= 'MongoRuntimeError') throw firstAbortError;
      ...
      if (firstAbortError instanceof MongoError && isRetryableWriteError(firstAbortError)) {
        try {
          ...
          if (secondAbortError.name ***REMOVED***= 'MongoRuntimeError') throw secondAbortError;
    ...
    try {
      while (!committed) {
        ...
        try {
          ...
          if (
            this.transaction.state ***REMOVED***= TxnState.NO_TRANSACTION ||
            this.transaction.state ***REMOVED***= TxnState.TRANSACTION_COMMITTED ||
            this.transaction.state ***REMOVED***= TxnState.TRANSACTION_ABORTED
          ) {
          ...
          if (
            this.transaction.state ***REMOVED***= TxnState.STARTING_TRANSACTION ||
            this.transaction.state ***REMOVED***= TxnState.TRANSACTION_IN_PROGRESS
          ) {
            await this.abortTransaction();
          }
...
function isUnknownTransactionCommitResult(err: MongoError): err is MongoError {
  ...
  return (
    ...
    (!isNonDeterministicWriteConcernError &&
      err.code !***REMOVED*** MONGODB_ERROR_CODES.UnsatisfiableWriteConcern &&
      err.code !***REMOVED*** MONGODB_ERROR_CODES.UnknownReplWriteConcern)
  );
}
...
export function maybeClearPinnedConnection(
  ...
  if (conn && topology != null) {
    ...
    if (options?.error ***REMOVED*** null || options?.force) {
      ...
      conn.emit(
        UNPINNED,
        session.transaction.state !***REMOVED*** TxnState.NO_TRANSACTION
...
function isMaxTimeMSExpiredError(err: MongoError): boolean {
  if (err ***REMOVED*** null || !(err instanceof MongoServerError)) {
    return false;
  }

  return (
    err.code ***REMOVED***= MONGODB_ERROR_CODES.MaxTimeMSExpired ||
    err.writeConcernError?.code ***REMOVED***= MONGODB_ERROR_CODES.MaxTimeMSExpired
  );
}
...
export function applySession(
  ...
  if (!inTxnOrTxnCommand) {
    if (session.transaction.state !***REMOVED*** TxnState.NO_TRANSACTION) {
  ...
  if (session.transaction.state ***REMOVED***= TxnState.STARTING_TRANSACTION) {
