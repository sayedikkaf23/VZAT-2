...
function isMeta(t: SortDirection): t is { $meta: string } {
  return typeof t ***REMOVED***= 'object' && t != null && '$meta' in t && typeof t.$meta ***REMOVED***= 'string';
}

/** @internal */
function isPair(t: Sort): t is readonly [string, SortDirection] {
  if (Array.isArray(t) && t.length ***REMOVED***= 2) {
    try {
...
export function formatSort(
  ...
  if (sort ***REMOVED*** null) return undefined;

  if (typeof sort ***REMOVED***= 'string') return new Map([[sort, prepareDirection(direction)]]); // 'fieldName'

  if (typeof sort !***REMOVED*** 'object') {
