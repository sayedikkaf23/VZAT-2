...
export class ConnectionPool extends TypedEventEmitter<ConnectionPoolEvents> {
   ...
   */
  get closed(): boolean {
    return this.poolState ***REMOVED***= PoolState.closed;
  }
   ...
   */
  ready(): void {
    if (this.poolState !***REMOVED*** PoolState.paused) {
      return;
    }
  ...
  clear(options: { serviceId?: ObjectId; interruptInUseConnections?: boolean } = {}): void {
    ...
    this.generation += 1;
    const alreadyPaused = this.poolState ***REMOVED***= PoolState.paused;
    this.poolState = PoolState.paused;
  ...
  private createConnection(callback: Callback<Connection>) {
    ...
    connect(connectOptions).then(
      connection => {
        ...
        if (this.poolState !***REMOVED*** PoolState.ready) {
          this.pending--;
          connection.destroy();
  ...
  private ensureMinPoolSize() {
    ...
    if (this.poolState !***REMOVED*** PoolState.ready || minPoolSize ***REMOVED***= 0) {
      return;
    }
