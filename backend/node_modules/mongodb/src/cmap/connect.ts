...
export async function performInitialHandshake(
  ...
  if (credentials) {
    if (
      !(credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_DEFAULT) &&
...
export async function prepareHandshakeDocument(
  ...
  const handshakeDoc: HandshakeDocument = {
    [serverApi?.version || options.loadBalanced ***REMOVED***= true ? 'hello' : LEGACY_HELLO_COMMAND]: 1,
    helloOk: true,
  ...
  if (credentials) {
    if (credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_DEFAULT && credentials.username) {
