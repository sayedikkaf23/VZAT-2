...
export class Connection extends TypedEventEmitter<ConnectionEvents> {
  ...
  private prepareCommand(db: string, command: Document, options: CommandOptions) {
    ...
    if (this.description.type !***REMOVED*** ServerType.Standalone) {
      if (
        !isSharded(this) &&
      ...
      } else if (isSharded(this) && !this.supportsOpMsg && readPreference?.mode !***REMOVED*** 'primary') {
  ...
  private async *sendCommand(
    ...
    const bsonOptions: DeserializeOptions =
      options.documentsReturnedIn ***REMOVED*** null || !options.raw
        ? options
        : {
            ...options,
            raw: false,
          ...
          if (
            (Array.isArray(document?.writeErrors) &&
              document.writeErrors.some(
                error => error?.code ***REMOVED***= MONGODB_ERROR_CODES.MaxTimeMSExpired
              )) ||
            document?.writeConcernError?.code ***REMOVED***= MONGODB_ERROR_CODES.MaxTimeMSExpired
          ) {
            throw new MongoOperationTimeoutError('Server reported a timeout error', {
  ...
  private async writeCommand(
    ...
    const finalCommand =
      options.agreedCompressor ***REMOVED***= 'none' || !OpCompressedRequest.canCompress(command)
        ? command
