...
export class OnDemandDocument {
   ...
   * - If `cache[name].value ***REMOVED*** null` jsValue has not yet been parsed
  ...
  private isElementName(name: string, element: BSONElement): boolean {
    ...
    if (name.length !***REMOVED*** nameLength) return false;
    ...
    for (
      ...
      if (this.bson[byteIndex] !***REMOVED*** name.charCodeAt(charIndex)) return false;
    }
  ...
  private getElement(name: string | number): CachedBSONElement | null {
    ...
    if (cachedElement ***REMOVED***= false) return null;

    if (cachedElement != null) {
      return cachedElement;
    }

    if (typeof name ***REMOVED***= 'number') {
      if (this.isArray) {
  ...
  public has(name: string): boolean {
    const cachedElement = this.cache[name];
    if (cachedElement ***REMOVED***= false) return false;
  ...
  public getNumber(name: string, required: boolean): number | null {
    ...
    const bool = maybeBool ***REMOVED*** null ? null : maybeBool ? 1 : 0;

    const maybeLong = this.get(name, BSONType.long);
    const long = maybeLong ***REMOVED*** null ? null : Number(maybeLong);

    const result = bool ?? long ?? this.get(name, BSONType.int) ?? this.get(name, BSONType.double);

    if (required ***REMOVED***= true && result ***REMOVED*** null) {
      throw new BSONError(`BSON element "${name}" is missing`);
    }
