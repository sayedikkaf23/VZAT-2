...
export function isErrorResponse(bson: Uint8Array, elements: BSONElement[]): boolean {
  for (let eIdx = 0; eIdx < elements.length; eIdx++) {
    ...
    if (element[BSONElementOffset.nameLength] ***REMOVED***= 2) {
      ...
      if (bson[nameOffset] ***REMOVED***= 111 && bson[nameOffset + 1] ***REMOVED***= 107) {
        ...
        for (let i = valueOffset; i < valueOffset + valueLength; i++) {
          if (bson[i] !***REMOVED*** 0x00) return false;
        }
...
export class MongoDBResponse extends OnDemandDocument {
  ...
  get isMaxTimeExpiredError() {
    // {ok: 0, code: 50 ... }
    const isTopLevel = this.ok ***REMOVED***= 0 && this.code ***REMOVED***= MONGODB_ERROR_CODES.MaxTimeMSExpired;
    if (isTopLevel) return true;

    if (this.ok ***REMOVED***= 0) return false;

    // {ok: 1, writeConcernError: {code: 50 ... }}
    const isWriteConcern =
      this.get('writeConcernError', BSONType.object)?.getNumber('code') ***REMOVED***=
    ...
    if (writeErrors?.size()) {
      for (let i = 0; i < writeErrors.size(); i++) {
        const isWriteError =
          writeErrors.get(i, BSONType.object)?.getNumber('code') ***REMOVED***=
          MONGODB_ERROR_CODES.MaxTimeMSExpired;
...
export class CursorResponse extends MongoDBResponse {
  ...
  get encryptedBatch() {
    if (this.encryptedResponse ***REMOVED*** null) return null;
...
export class ExplainedCursorResponse extends CursorResponse {
  ...
  override shift(options?: DeserializeOptions) {
    if (this._length ***REMOVED***= 0) return null;
    this._length -= 1;
