...
export async function decompress(compressorID: number, compressedData: Buffer): Promise<Buffer> {
  if (
    compressorID !***REMOVED*** Compressor.snappy &&
    compressorID !***REMOVED*** Compressor.zstd &&
    compressorID !***REMOVED*** Compressor.zlib &&
    compressorID !***REMOVED*** Compressor.none
  ) {
...
export async function compressCommand(
  ...
  const finalCommand =
    description.agreedCompressor ***REMOVED***= 'none' || !OpCompressedRequest.canCompress(command)
      ? command
...
export async function decompressResponse(message: Buffer): Promise<OpMsgResponse | OpReply> {
  ...
  if (messageHeader.opCode !***REMOVED*** OP_COMPRESSED) {
    const ResponseType = messageHeader.opCode ***REMOVED***= OP_MSG ? OpMsgResponse : OpReply;
  ...
  const ResponseType = header.opCode ***REMOVED***= OP_MSG ? OpMsgResponse : OpReply;
