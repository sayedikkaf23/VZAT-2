...
export function makeClientMetadata(options: MakeClientMetadataOptions): ClientMetadata {
  ...
  if (!metadataDocument.ifItFitsItSits('os', osInfo)) {
    for (const key of osInfo.keys()) {
      osInfo.delete(key);
      if (osInfo.size ***REMOVED***= 0) break;
  ...
  if (faasEnv != null) {
    if (!metadataDocument.ifItFitsItSits('env', faasEnv)) {
      for (const key of faasEnv.keys()) {
        faasEnv.delete(key);
        if (faasEnv.size ***REMOVED***= 0) break;
...
export async function addContainerMetadata(originalMetadata: ClientMetadata) {
  ...
  if (Object.keys(containerMetadata).length ***REMOVED***= 0) return originalMetadata;
...
function getRuntimeInfo(): string {
  if ('Deno' in globalThis) {
    const version = typeof Deno?.version?.deno ***REMOVED***= 'string' ? Deno?.version?.deno : '0.0.0-unknown';

    return `Deno v${version}, ${os.endianness()}`;
  }

  if ('Bun' in globalThis) {
    const version = typeof Bun?.version ***REMOVED***= 'string' ? Bun?.version : '0.0.0-unknown';
