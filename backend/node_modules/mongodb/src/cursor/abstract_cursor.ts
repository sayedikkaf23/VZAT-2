  ...
  implements AsyncDisposable
{
  ...
  protected constructor(
    ...
    if (this.cursorOptions.timeoutMS != null) {
      if (options.timeoutMode ***REMOVED*** null) {
        if (options.tailable) {
        ...
        if (options.tailable && options.timeoutMode ***REMOVED***= CursorTimeoutMode.LIFETIME) {
          throw new MongoInvalidArgumentError(
    ...
    this.cursorOptions.omitMaxTimeMS =
      ...
      ((this.cursorOptions.timeoutMode ***REMOVED***= CursorTimeoutMode.ITERATION &&
    ...
    if (typeof options.batchSize ***REMOVED***= 'number') {
      this.cursorOptions.batchSize = options.batchSize;
    }

    // we check for undefined specifically here to allow falsy values
    // eslint-disable-next-line no-restricted-syntax
    if (options.comment !***REMOVED*** undefined) {
      this.cursorOptions.comment = options.comment;
    }

    if (typeof options.maxTimeMS ***REMOVED***= 'number') {
      this.cursorOptions.maxTimeMS = options.maxTimeMS;
    }

    if (typeof options.maxAwaitTimeMS ***REMOVED***= 'number') {
    ...
    this.deserializationOptions = {
      ...this.cursorOptions,
      validation: {
        utf8: options?.enableUtf8Validation ***REMOVED***= false ? false : true
      }
    };
   ...
   */
  get closed(): boolean {
    return this.isClosed && (this.documents?.length ?? 0) ***REMOVED***= 0;
  }
  ...
  async *[Symbol.asyncIterator](): AsyncGenerator<TSchema, void, void> {
    ...
    try {
      while (true) {
        ...
        if (this.cursorId != null && this.isDead && (this.documents?.length ?? 0) ***REMOVED***= 0) {
          return;
        }
  ...
  async hasNext(): Promise<boolean> {
    ...
    if (this.cursorId ***REMOVED***= Long.ZERO) {
      return false;
    }

    if (this.cursorOptions.timeoutMode ***REMOVED***= CursorTimeoutMode.ITERATION && this.cursorId != null) {
      this.timeoutContext?.refresh();
    }
    try {
      do {
        if ((this.documents?.length ?? 0) !***REMOVED*** 0) {
          return true;
        }
        await this.fetchBatch();
      } while (!this.isDead || (this.documents?.length ?? 0) !***REMOVED*** 0);
    } finally {
      if (this.cursorOptions.timeoutMode ***REMOVED***= CursorTimeoutMode.ITERATION) {
  ...
  async forEach(iterator: (doc: TSchema) => boolean | void): Promise<void> {
    ...
    if (typeof iterator !***REMOVED*** 'function') {
  ...
  withReadPreference(readPreference: ReadPreferenceLike): this {
    ...
    } else if (typeof readPreference ***REMOVED***= 'string') {
   ...
   */
  rewind(): void {
    if (this.timeoutContext && this.timeoutContext.owner !***REMOVED*** this) {
    ...
    if (this.cursorSession.explicit ***REMOVED***= false) {
  ...
  private async fetchBatch(): Promise<void> {
    ...
    if (this.cursorId ***REMOVED*** null) {
      ...
      if ((this.documents?.length ?? 0) !***REMOVED*** 0 || this.isDead) return;
  ...
  private async cleanup(timeoutMS?: number, error?: Error) {
    ...
    try {
      ...
      try {
        if (this.cursorSession?.owner ***REMOVED***= this) {
  ...
  private emitClose() {
    try {
      if (!this.hasEmittedClose && ((this.documents?.length ?? 0) ***REMOVED***= 0 || this.isClosed)) {
  ...
  private async transformDocument(document: NonNullable<TSchema>): Promise<NonNullable<TSchema>> {
    if (this.transform ***REMOVED*** null) return document;

    try {
      const transformedDocument = this.transform(document);
      // eslint-disable-next-line no-restricted-syntax
      if (transformedDocument ***REMOVED***= null) {
...
class ReadableCursorStream extends Readable {
  ...
  private _readNext() {
    if (this._cursor.id ***REMOVED***= Long.ZERO) {
      this.push(null);
      return;
    }
...
export class CursorTimeoutContext extends TimeoutContext {
  ...
  override refresh(): void {
    if (typeof this.owner !***REMOVED*** 'symbol') return this.timeoutContext.refresh();
  }
  override clear(): void {
    if (typeof this.owner !***REMOVED*** 'symbol') return this.timeoutContext.clear();
  }
