  ...
  implements AsyncDisposable
{
  ...
  constructor(
    ...
    this.on('newListener', eventName => {
      if (eventName ***REMOVED***= 'change' && this.cursor && this.listenerCount('change') ***REMOVED***= 0) {
        this._streamEvents(this.cursor);
      }
    });

    this.on('removeListener', eventName => {
      if (eventName ***REMOVED***= 'change' && this.listenerCount('change') ***REMOVED***= 0 && this.cursor) {
      ...
      while (true) {
        try {
          ...
          try {
            ...
            if (error instanceof MongoOperationTimeoutError && this.cursor.id ***REMOVED*** null) {
              throw error;
            }
            try {
              await this.close();
      ...
      while (true) {
        try {
          ...
          try {
            ...
            if (error instanceof MongoOperationTimeoutError && this.cursor.id ***REMOVED*** null) throw error;
            try {
              await this.close();
            } catch (error) {
              squashError(error);
  ...
  private _createChangeStreamCursor(
    ...
    if (this.type ***REMOVED***= CHANGE_DOMAIN_TYPES.CLUSTER) {
      changeStreamStageOptions.allChangesForCluster = true;
    }
    const pipeline = [{ $changeStream: changeStreamStageOptions }, ...this.pipeline];

    const client: MongoClient | null =
      this.type ***REMOVED***= CHANGE_DOMAIN_TYPES.CLUSTER
        ? (this.parent as MongoClient)
        : this.type ***REMOVED***= CHANGE_DOMAIN_TYPES.DATABASE
          ? (this.parent as Db).client
          : this.type ***REMOVED***= CHANGE_DOMAIN_TYPES.COLLECTION
