...
function getBoolean(name: string, value: unknown): boolean {
  if (typeof value ***REMOVED***= 'boolean') return value;
  switch (value) {
    case 'true':
      return true;
    case 'false':
...
export function parseOptions(
  ...
  if (url.pathname !***REMOVED*** '/' && url.pathname !***REMOVED*** '') {
    const dbName = decodeURIComponent(
      url.pathname[0] ***REMOVED***= '/' ? url.pathname.slice(1) : url.pathname
    );
    if (dbName) {
  ...
  if (mongoOptions.credentials) {
    const isGssapi = mongoOptions.credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_GSSAPI;
    const isX509 = mongoOptions.credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_X509;
    const isAws = mongoOptions.credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_AWS;
    const isOidc = mongoOptions.credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_OIDC;
    if (
      (isGssapi || isX509) &&
    ...
    if (
      mongoOptions.credentials.password ***REMOVED***= '' &&
      mongoOptions.credentials.username ***REMOVED***= '' &&
      mongoOptions.credentials.mechanism ***REMOVED***= AuthMechanism.MONGODB_DEFAULT &&
...
export const OPTIONS = {
  ...
  authMechanism: {
    ...
    transform({ options, values: [value] }): MongoCredentials {
      ...
      if (
        mechanism ***REMOVED***= AuthMechanism.MONGODB_PLAIN ||
      ...
      if (mechanism ***REMOVED***= AuthMechanism.MONGODB_X509 && password ***REMOVED***= '') {
        password = undefined;
      }
      return MongoCredentials.merge(options.credentials, {
        mechanism,
  ...
  serverApi: {
    ...
    transform({ values: [version] }): ServerApi {
      const serverApiToValidate =
        typeof version ***REMOVED***= 'string' ? ({ version } as ServerApi) : (version as ServerApi);
      ...
      if (!Object.values(ServerApiVersion).some(v => v ***REMOVED***= versionToValidate)) {
        throw new MongoParseError(
  ...
  pkFactory: {
    ...
    transform({ values: [value] }): PkFactory {
      if (isRecord(value, ['createPk'] as const) && typeof value.createPk ***REMOVED***= 'function') {
  ...
  mongodbLogPath: {
    transform({ values: [value] }) {
      if (
        !(
          (typeof value ***REMOVED***= 'string' && ['stderr', 'stdout'].includes(value)) ||
          (value &&
  ...
  mongodbLogComponentSeverities: {
    transform({ values: [value] }) {
      ...
      for (const [k, v] of Object.entries(value)) {
        ...
        if (!Object.values(MongoLoggableComponent).some(val => val ***REMOVED***= k) && k !***REMOVED*** 'default') {
          throw new MongoAPIError(
            `User input for option 'mongodbLogComponentSeverities' contains invalid key: ${k}`
          );
        }
        if (!Object.values(SeverityLevel).some(val => val ***REMOVED***= v)) {
          throw new MongoAPIError(
