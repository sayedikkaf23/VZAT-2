...
export function sameServerSelector(description?: ServerDescription): ServerSelector {
  return function sameServerSelector(
    ...
    return servers.filter(sd => {
      return sd.address ***REMOVED***= description.address && sd.type !***REMOVED*** ServerType.Unknown;
    });
  };
}
...
function maxStalenessReducer(
  ...
  if (topologyDescription.type ***REMOVED***= TopologyType.ReplicaSetWithPrimary) {
  ...
  if (topologyDescription.type ***REMOVED***= TopologyType.ReplicaSetNoPrimary) {
...
function tagSetReducer(
  ...
  if (
    readPreference.tags ***REMOVED*** null ||
...
function primaryFilter(server: ServerDescription): boolean {
  return server.type ***REMOVED***= ServerType.RSPrimary;
}

function secondaryFilter(server: ServerDescription): boolean {
  return server.type ***REMOVED***= ServerType.RSSecondary;
}

function nearestFilter(server: ServerDescription): boolean {
  return server.type ***REMOVED***= ServerType.RSSecondary || server.type ***REMOVED***= ServerType.RSPrimary;
}

function knownFilter(server: ServerDescription): boolean {
  return server.type !***REMOVED*** ServerType.Unknown;
}

function loadBalancerFilter(server: ServerDescription): boolean {
  return server.type ***REMOVED***= ServerType.LoadBalancer;
}
...
export function readPreferenceServerSelector(readPreference: ReadPreference): ServerSelector {
  ...
  return function readPreferenceServers(
    ...
    if (topologyDescription.type ***REMOVED***= TopologyType.LoadBalanced) {
      return servers.filter(loadBalancerFilter);
    }

    if (topologyDescription.type ***REMOVED***= TopologyType.Unknown) {
      return [];
    }

    if (topologyDescription.type ***REMOVED***= TopologyType.Single) {
      return latencyWindowReducer(topologyDescription, servers.filter(knownFilter));
    }

    if (topologyDescription.type ***REMOVED***= TopologyType.Sharded) {
    ...
    if (mode ***REMOVED***= ReadPreference.PRIMARY) {
      return servers.filter(primaryFilter);
    }

    if (mode ***REMOVED***= ReadPreference.PRIMARY_PREFERRED) {
    ...
    const filter = mode ***REMOVED***= ReadPreference.NEAREST ? nearestFilter : secondaryFilter;
    ...
    if (mode ***REMOVED***= ReadPreference.SECONDARY_PREFERRED && selectedServers.length ***REMOVED***= 0) {
      return servers.filter(primaryFilter);
    }
