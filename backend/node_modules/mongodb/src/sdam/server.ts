...
export class Server extends TypedEventEmitter<ServerEvents> {
  ...
  constructor(topology: Topology, description: ServerDescription, options: ServerOptions) {
    ...
    this.monitor.on(Server.SERVER_HEARTBEAT_SUCCEEDED, (event: ServerHeartbeatSucceededEvent) => {
      ...
      if (this.s.state ***REMOVED***= STATE_CONNECTING) {
  ...
  get loadBalanced(): boolean {
    return this.topology.description.type ***REMOVED***= TopologyType.LoadBalanced;
  }
   ...
   */
  connect(): void {
    if (this.s.state !***REMOVED*** STATE_CLOSED) {
      return;
    }
  ...
  public async command(
    ...
    if (this.s.state ***REMOVED***= STATE_CLOSING || this.s.state ***REMOVED***= STATE_CLOSED) {
    ...
    try {
      ...
      if (
        ...
        operationError.code ***REMOVED***= MONGODB_ERROR_CODES.Reauthenticate
      ) {
  ...
  private decorateCommandError(
    ...
    if (error.name ***REMOVED***= 'AbortError' && 'cause' in error && error.cause instanceof MongoError) {
      error = error.cause;
    }
...
function connectionIsStale(pool: ConnectionPool, connection: Connection) {
  if (connection.serviceId) {
    return (
      connection.generation !***REMOVED*** pool.serviceGenerations.get(connection.serviceId.toHexString())
    );
  }
