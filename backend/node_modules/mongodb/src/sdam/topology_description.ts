...
export class TopologyDescription {
  ...
  constructor(
    ...
    for (const serverDescription of this.servers.values()) {
      ...
      if (
        serverDescription.type ***REMOVED***= ServerType.Unknown ||
        serverDescription.type ***REMOVED***= ServerType.LoadBalancer
      ) {
        continue;
      }
  ...
  update(serverDescription: ServerDescription): TopologyDescription {
    ...
    if (
      typeof serverDescription.setName ***REMOVED***= 'string' &&
      typeof setName ***REMOVED***= 'string' &&
      serverDescription.setName !***REMOVED*** setName
    ) {
      if (topologyType ***REMOVED***= TopologyType.Single) {
    ...
    if (topologyType ***REMOVED***= TopologyType.Unknown) {
      if (serverType ***REMOVED***= ServerType.Standalone && this.servers.size !***REMOVED*** 1) {
        serverDescriptions.delete(address);
      } else {
    ...
    if (topologyType ***REMOVED***= TopologyType.Sharded) {
      if (!MONGOS_OR_UNKNOWN.has(serverType)) {
        serverDescriptions.delete(address);
      }
    }

    if (topologyType ***REMOVED***= TopologyType.ReplicaSetNoPrimary) {
      if (MONGOS_OR_STANDALONE.has(serverType)) {
        serverDescriptions.delete(address);
      }

      if (serverType ***REMOVED***= ServerType.RSPrimary) {
    ...
    if (topologyType ***REMOVED***= TopologyType.ReplicaSetWithPrimary) {
      if (MONGOS_OR_STANDALONE.has(serverType)) {
        serverDescriptions.delete(address);
        topologyType = checkHasPrimary(serverDescriptions);
      } else if (serverType ***REMOVED***= ServerType.RSPrimary) {
        const result = updateRsFromPrimary(
          serverDescriptions,
          serverDescription,
          setName,
  ...
  get hasKnownServers(): boolean {
    return Array.from(this.servers.values()).some(
      (sd: ServerDescription) => sd.type !***REMOVED*** ServerType.Unknown
    );
  }
...
function updateRsFromPrimary(
  ...
  for (const [address, server] of serverDescriptions) {
    if (server.type ***REMOVED***= ServerType.RSPrimary && server.address !***REMOVED*** serverDescription.address) {
  ...
  currentAddresses
    .filter((addr: string) => responseAddresses.indexOf(addr) ***REMOVED***= -1)
...
function updateRsWithPrimaryFromMember(
  ...
  if (
    setName !***REMOVED*** serverDescription.setName ||
...
function checkHasPrimary(serverDescriptions: Map<string, ServerDescription>): TopologyType {
  for (const serverDescription of serverDescriptions.values()) {
    if (serverDescription.type ***REMOVED***= ServerType.RSPrimary) {
      return TopologyType.ReplicaSetWithPrimary;
    }
  }
