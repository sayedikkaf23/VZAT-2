...
function isInCloseState(monitor: Monitor) {
  return monitor.s.state ***REMOVED***= STATE_CLOSED || monitor.s.state ***REMOVED***= STATE_CLOSING;
}
...
export class Monitor extends TypedEventEmitter<MonitorEvents> {
  ...
  reset(): void {
    ...
    if (isInCloseState(this) || topologyVersion ***REMOVED*** null) {
      return;
    }
...
function useStreamingProtocol(monitor: Monitor, topologyVersion: TopologyVersion | null): boolean {
  ...
  if (topologyVersion ***REMOVED*** null) return false;

  const serverMonitoringMode = monitor.options.serverMonitoringMode;
  if (serverMonitoringMode ***REMOVED***= ServerMonitoringMode.poll) return false;
  if (serverMonitoringMode ***REMOVED***= ServerMonitoringMode.stream) return true;
...
function checkServer(monitor: Monitor, callback: Callback<Document | null>) {
  ...
  if (connection && !connection.closed) {
    ...
    if (isAwaitable && monitor.rttPinger ***REMOVED*** null) {
...
function monitorServer(monitor: Monitor) {
  return (callback: Callback) => {
    if (monitor.s.state ***REMOVED***= STATE_MONITORING) {
    ...
    checkServer(monitor, (err, hello) => {
      if (err) {
        ...
        if (monitor.server.description.type ***REMOVED***= ServerType.Unknown) {
          return done();
        }
      }
...
export class RTTPinger {
  ...
  private measureAndReschedule(start: number, conn?: Connection) {
    ...
    if (this.connection ***REMOVED*** null) {
...
export class RTTSampler {
   ...
   */
  average(): number {
    if (this.length ***REMOVED***= 0) return 0;
    let sum = 0;
  ...
  get last(): number | null {
    if (this.length ***REMOVED***= 0) return null;
    return this.rttSamples[this.writeIndex ***REMOVED***= 0 ? this.length - 1 : this.writeIndex - 1];
  }
