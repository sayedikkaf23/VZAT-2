...
export class Topology extends TypedEventEmitter<TopologyEvents> {
  ...
  constructor(
    ...
    const selectedHosts =
      options.srvMaxHosts ***REMOVED*** null ||
  ...
  private detectShardedTopology(event: TopologyDescriptionChangedEvent) {
    ...
    const transitionToSharded =
      previousType !***REMOVED*** TopologyType.Sharded && newType ***REMOVED***= TopologyType.Sharded;
  ...
  private async _connect(options?: ConnectOptions): Promise<Topology> {
    options = options ?? {};
    if (this.s.state ***REMOVED***= STATE_CONNECTED) {
      return this;
    }
  ...
  /** Close this topology */
  close(): void {
    if (this.s.state ***REMOVED***= STATE_CLOSED || this.s.state ***REMOVED***= STATE_CLOSING) {
      return;
    }
  ...
  async selectServer(
    ...
    const isSharded = this.description.type ***REMOVED***= TopologyType.Sharded;
  ...
  auth(credentials?: MongoCredentials, callback?: Callback): void {
    if (typeof credentials ***REMOVED***= 'function') (callback = credentials), (credentials = undefined);
    if (typeof callback ***REMOVED***= 'function') callback(undefined, true);
  }
  ...
  isConnected(): boolean {
    return this.s.state ***REMOVED***= STATE_CONNECTED;
  }

  isDestroyed(): boolean {
    return this.s.state ***REMOVED***= STATE_CLOSED;
  }
  ...
  lastHello(): Document {
    ...
    if (serverDescriptions.length ***REMOVED***= 0) return {};
    ...
    if (server) {
      ...
      if (
        ...
        const shouldMarkPoolReady =
          ...
          (incomingServerDescription.type !***REMOVED*** ServerType.Unknown &&
            newTopologyType ***REMOVED***= TopologyType.Single);
...
function processWaitQueue(topology: Topology) {
  if (topology.s.state ***REMOVED***= STATE_CLOSED) {
    drainWaitQueue(topology.waitQueue, new MongoTopologyClosedError());
    return;
  }

  const isSharded = topology.description.type ***REMOVED***= TopologyType.Sharded;
        ...
        if (
          ...
          topology.client.mongoLogger?.info(
            ...
            new WaitingForSuitableServerEvent(
              waitQueueMember.serverSelector,
              topology.description,
              topology.s.serverSelectionTimeoutMS !***REMOVED*** 0
