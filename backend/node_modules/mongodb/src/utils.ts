...
export function isUint8Array(value: unknown): value is Uint8Array {
  return (
    ...
    value[Symbol.toStringTag] ***REMOVED***= 'Uint8Array'
  );
}
...
export function normalizeHintField(hint?: Hint): Hint | undefined {
  ...
  } else if (hint != null && typeof hint ***REMOVED***= 'object') {
 ...
 * - **NOTE**: the check is based on the `[Symbol.toStringTag]() ***REMOVED***= 'Object'`
 * @internal
 */

export function isObject(arg: unknown): arg is object {
  return '[object Object]' ***REMOVED***= TO_STRING(arg);
}
...
export function isPromiseLike<T = unknown>(value?: unknown): value is PromiseLike<T> {
  return (
    ...
    typeof value ***REMOVED***= 'object' &&
    'then' in value &&
    typeof value.then ***REMOVED***= 'function'
  );
}
...
export function decorateWithCollation(
  ...
  if (options.collation && typeof options.collation ***REMOVED***= 'object') {
...
export class MongoDBNamespace {
  ...
  constructor(
    ...
    this.collection = collection ***REMOVED***= '' ? undefined : collection;
  }
  ...
  static fromString(namespace?: string): MongoDBNamespace {
    if (typeof namespace !***REMOVED*** 'string' || namespace ***REMOVED***= '') {
    ...
    return new MongoDBNamespace(db, collection ***REMOVED***= '' ? undefined : collection);
  }
}
...
export function maxWireVersion(topologyOrServer?: Connection | Topology | Server): number {
  if (topologyOrServer) {
    ...
    if ('lastHello' in topologyOrServer && typeof topologyOrServer.lastHello ***REMOVED***= 'function') {
...
export function arrayStrictEqual(arr: unknown[], arr2: unknown[]): boolean {
  ...
  return arr.length ***REMOVED***= arr2.length && arr.every((elt, idx) => elt ***REMOVED***= arr2[idx]);
}
...
export function errorStrictEqual(lhs?: AnyError | null, rhs?: AnyError | null): boolean {
  ...
  if (!lhs || !rhs) {
    return lhs ***REMOVED***= rhs;
  }

  if ((lhs ***REMOVED*** null && rhs != null) || (lhs != null && rhs ***REMOVED*** null)) {
    return false;
  }

  if (lhs.constructor.name !***REMOVED*** rhs.constructor.name) {
    return false;
  }

  if (lhs.message !***REMOVED*** rhs.message) {
    return false;
  }
...
export function calculateDurationInMs(started: number | undefined): number {
  if (typeof started !***REMOVED*** 'number') {
    return -1;
  }
...
export function hasAtomicOperators(
  ...
  return keys.length > 0 && keys[0][0] ***REMOVED***= '$';
}
...
export class List<T = unknown> {
  ...
  *[Symbol.iterator](): Generator<T, void, void> {
  ...
  private remove(node: ListNode<T> | EmptyNode): T | null {
    if (node ***REMOVED***= this.head || this.length ***REMOVED***= 0) {
      return null;
    }
...
export class BufferPool {
  ...
  read(size: number): Buffer {
    if (typeof size !***REMOVED*** 'number' || size < 0) {
...
export class HostAddress {
  ...
  constructor(hostString: string) {
    ...
    if (typeof port ***REMOVED***= 'number') {
      this.port = port;
    } else if (typeof port ***REMOVED***= 'string' && port !***REMOVED*** '') {
      this.port = Number.parseInt(port, 10);
    } else {
      this.port = 27017;
    }

    if (this.port ***REMOVED***= 0) {
  ...
  toString(): string {
    if (typeof this.host ***REMOVED***= 'string') {
      if (this.isIPv6) {
 ...
 *  if (warning.code ***REMOVED***= MONGODB_WARNING_CODE) console.error('Ah an important warning! :)')
 * })
 * ```
 */
...
export function supportsRetryableWrites(server?: Server): boolean {
  ...
  if (server.description.logicalSessionTimeoutMinutes != null) {
    ...
    if (server.description.type !***REMOVED*** ServerType.Standalone) {
...
export function shuffle<T>(sequence: Iterable<T>, limit = 0): Array<T> {
  ...
  const lowerBound = limit % items.length ***REMOVED***= 0 ? 1 : items.length - limit;
  ...
  return limit % items.length ***REMOVED***= 0 ? items : items.slice(lowerBound);
}
...
export function compareObjectId(oid1?: ObjectId | null, oid2?: ObjectId | null): 0 | 1 | -1 {
  if (oid1 ***REMOVED*** null && oid2 ***REMOVED*** null) {
    return 0;
  }

  if (oid1 ***REMOVED*** null) {
    return -1;
  }

  if (oid2 ***REMOVED*** null) {
    return 1;
  }
...
export function parseInteger(value: unknown): number | null {
  if (typeof value ***REMOVED***= 'number') return Math.trunc(value);
...
export function maybeAddIdToDocuments(
  ...
  const forceServerObjectId =
    typeof options.forceServerObjectId ***REMOVED***= 'boolean'
      ? options.forceServerObjectId
      : coll.s.db.options?.forceServerObjectId;

  // no need to modify the docs if server sets the ObjectId
  if (forceServerObjectId ***REMOVED***= true) {
    return docOrDocs;
  }

  const transform = (doc: Document): Document => {
    if (doc._id ***REMOVED*** null) {
...
export function csotMin(duration1: number, duration2: number): number {
  if (duration1 ***REMOVED***= 0) return duration2;
  if (duration2 ***REMOVED***= 0) return duration1;
...
export function decorateDecryptionResult(
  ...
  if (!decrypted || typeof decrypted !***REMOVED*** 'object') return;
  for (const k of Object.keys(decrypted)) {
    const originalValue = original[k];

    // An object was decrypted by libmongocrypt if and only if it was
    // a BSON Binary object with subtype 6.
    if (originalValue && originalValue._bsontype ***REMOVED***= 'Binary' && originalValue.sub_type ***REMOVED***= 6) {
