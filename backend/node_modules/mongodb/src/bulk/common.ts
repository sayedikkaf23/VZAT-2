...
export class BulkWriteResult {
  ...
  private getSuccessfullyInsertedIds(bulkResult: BulkResult, isOrdered: boolean): Document[] {
    if (bulkResult.writeErrors.length ***REMOVED***= 0) return bulkResult.insertedIds;
    ...
    return bulkResult.insertedIds.filter(
      ({ index }) => !bulkResult.writeErrors.some(writeError => index ***REMOVED***= writeError.index)
    );
  }
  ...
  getWriteConcernError(): WriteConcernError | undefined {
    if (this.result.writeConcernErrors.length ***REMOVED***= 0) {
      return;
    } else if (this.result.writeConcernErrors.length ***REMOVED***= 1) {
      // Return the error
      ...
      for (let i = 0; i < this.result.writeConcernErrors.length; i++) {
        ...
        if (i ***REMOVED***= 0) errmsg = errmsg + ' and ';
      }
  ...
  isOk(): boolean {
    return this.result.ok ***REMOVED***= 1;
  }
}
...
export function mergeBatchResults(
  ...
  if (result.ok ***REMOVED***= 0 && bulkResult.ok ***REMOVED***= 1) {
    bulkResult.ok = 0;
    ...
    return;
  } else if (result.ok ***REMOVED***= 0 && bulkResult.ok ***REMOVED***= 0) {
    return;
  }
  ...
  if (isUpdateBatch(batch) && result.n) {
    ...
    if (typeof nModified ***REMOVED***= 'number') {
...
async function executeCommands(
  ...
  if (bulkOperation.s.batches.length ***REMOVED***= 0) {
  ...
  for (const batch of bulkOperation.s.batches) {
    ...
    if (finalOptions.bypassDocumentValidation !***REMOVED*** true) {
      delete finalOptions.bypassDocumentValidation;
    }

    // Is the bypassDocumentValidation options specific
    if (bulkOperation.s.bypassDocumentValidation ***REMOVED***= true) {
      finalOptions.bypassDocumentValidation = true;
    }

    // Is the checkKeys option disabled
    if (bulkOperation.s.checkKeys ***REMOVED***= false) {
    ...
    if (finalOptions.retryWrites) {
      ...
      if (isDeleteBatch(batch)) {
        finalOptions.retryWrites =
          finalOptions.retryWrites && !batch.operations.some(op => op.limit ***REMOVED***= 0);
      }
    }
    ...
    if (operation ***REMOVED*** null) throw new MongoRuntimeError(`Unknown batchType: ${batch.batchType}`);
...
export class BulkWriteShimOperation extends AbstractOperation {
  ...
  async execute(
    ...
    if (this.options.session ***REMOVED*** null) {
...
export abstract class BulkOperationBase {
  ...
  constructor(
    ...
    this.s = {
      ...
      err: undefined,
      // check keys
      checkKeys: typeof options.checkKeys ***REMOVED***= 'boolean' ? options.checkKeys : false
    };

    // bypass Validation
    if (options.bypassDocumentValidation ***REMOVED***= true) {
  ...
  async execute(options: BulkWriteOptions = {}): Promise<BulkWriteResult> {
    ...
    if (this.s.batches.length ***REMOVED***= 0) {
  ...
  private shouldForceServerObjectId(): boolean {
    return (
      this.s.options.forceServerObjectId ***REMOVED***= true ||
      this.s.collection.s.db.options?.forceServerObjectId ***REMOVED***= true
    );
  }
}

function isInsertBatch(batch: Batch): boolean {
  return batch.batchType ***REMOVED***= BatchType.INSERT;
}

function isUpdateBatch(batch: Batch): batch is Batch<UpdateStatement> {
  return batch.batchType ***REMOVED***= BatchType.UPDATE;
}

function isDeleteBatch(batch: Batch): batch is Batch<DeleteStatement> {
  return batch.batchType ***REMOVED***= BatchType.DELETE;
}
