...
async function checkChunksIndex(stream) {
    ...
    try {
        ...
        if (error instanceof error_1.MongoError && error.code ***REMOVED***= error_1.MONGODB_ERROR_CODES.NamespaceNotFound) {
            indexes = [];
        }
        else {
            throw error;
    ...
    const hasChunksIndex = !!indexes.find(index => {
        ...
        if (keys.length ***REMOVED***= 2 && index.key.files_id ***REMOVED***= 1 && index.key.n ***REMOVED***= 1) {
            return true;
        }
        return false;
    });
...
function checkDone(stream, callback) {
    ...
    if (stream.state.streamEnd && stream.state.outstandingRequests ***REMOVED***= 0 && !stream.state.errored) {
...
async function checkIndexes(stream) {
    ...
    const hasFileIndex = !!indexes.find(index => {
        const keys = Object.keys(index.key);
        if (keys.length ***REMOVED***= 2 && index.key.filename ***REMOVED***= 1 && index.key.uploadDate ***REMOVED***= 1) {
            return true;
        }
        return false;
    });
