...
class BulkWriteResult {
    ...
    getSuccessfullyInsertedIds(bulkResult, isOrdered) {
        if (bulkResult.writeErrors.length ***REMOVED***= 0)
            return bulkResult.insertedIds;
        if (isOrdered) {
            return bulkResult.insertedIds.slice(0, bulkResult.writeErrors[0].index);
        }
        return bulkResult.insertedIds.filter(({ index }) => !bulkResult.writeErrors.some(writeError => index ***REMOVED***= writeError.index));
    }
...
class BulkOperationBase {
    ...
    raw(op) {
        ...
        if ('insertOne' in op) {
            ...
            const document = op.insertOne && op.insertOne.document ***REMOVED*** null
    ...
    shouldForceServerObjectId() {
        return (this.s.options.forceServerObjectId ***REMOVED***= true ||
            this.s.collection.s.db.options?.forceServerObjectId ***REMOVED***= true);
    }
}
exports.BulkOperationBase = BulkOperationBase;
function isInsertBatch(batch) {
    return batch.batchType ***REMOVED***= exports.BatchType.INSERT;
}
function isUpdateBatch(batch) {
    return batch.batchType ***REMOVED***= exports.BatchType.UPDATE;
}
function isDeleteBatch(batch) {
    return batch.batchType ***REMOVED***= exports.BatchType.DELETE;
}
