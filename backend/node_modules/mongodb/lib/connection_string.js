...
function retryDNSTimeoutFor(api) {
    return async function dnsReqRetryTimeout(lookupAddress) {
        try {
            ...
            if (firstDNSError.code ***REMOVED***= dns.TIMEOUT) {
                return await dns.promises[api](lookupAddress);
            }
            else {
                throw firstDNSError;
...
async function resolveSRVRecord(options) {
    if (typeof options.srvHost !***REMOVED*** 'string') {
    ...
    try {
        ...
        if (error.code !***REMOVED*** 'ENODATA' && error.code !***REMOVED*** 'ENOTFOUND') {
            throw error;
        }
    ...
    if (VALID_TXT_RECORDS.some(option => txtRecordOptions.get(option) ***REMOVED***= '')) {
...
function parseOptions(uri, mongoClient = undefined, options = {}) {
    ...
    if (options.useBigInt64 && typeof options.promoteLongs ***REMOVED***= 'boolean' && !options.promoteLongs) {
        throw new error_1.MongoAPIError('Must request either bigint or Long for int64 deserialization');
    }
    if (options.useBigInt64 && typeof options.promoteValues ***REMOVED***= 'boolean' && !options.promoteValues) {
        throw new error_1.MongoAPIError('Must request either bigint or Long for int64 deserialization');
    }
    ...
    if (url.pathname !***REMOVED*** '/' && url.pathname !***REMOVED*** '') {
        const dbName = decodeURIComponent(url.pathname[0] ***REMOVED***= '/' ? url.pathname.slice(1) : url.pathname);
        if (dbName) {
    ...
    if (url.username !***REMOVED*** '') {
        ...
        if (typeof url.password ***REMOVED***= 'string') {
    ...
    if (allProvidedOptions.has('tls') || allProvidedOptions.has('ssl')) {
        ...
        if (new Set(tlsAndSslOpts).size !***REMOVED*** 1) {
            throw new error_1.MongoParseError('All values of tls/ssl must be the same.');
        }
    }
    checkTLSOptions(allProvidedOptions);
    const unsupportedOptions = (0, utils_1.setDifference)(allProvidedKeys, Array.from(Object.keys(exports.OPTIONS)).map(s => s.toLowerCase()));
    if (unsupportedOptions.size !***REMOVED*** 0) {
    ...
    for (const [key, descriptor] of Object.entries(exports.OPTIONS)) {
        ...
        if (!values || values.length ***REMOVED***= 0) {
            ...
            if (deprecated) {
                const deprecatedMsg = typeof deprecated ***REMOVED***= 'string' ? `: ${deprecated}` : '';
    ...
    if (mongoOptions.credentials) {
        const isGssapi = mongoOptions.credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_GSSAPI;
        const isX509 = mongoOptions.credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_X509;
        const isAws = mongoOptions.credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_AWS;
        const isOidc = mongoOptions.credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_OIDC;
        if ((isGssapi || isX509) &&
            allProvidedOptions.has('authSource') &&
            mongoOptions.credentials.source !***REMOVED*** '$external') {
        ...
        if (mongoOptions.credentials.password ***REMOVED***= '' &&
            mongoOptions.credentials.username ***REMOVED***= '' &&
            mongoOptions.credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_DEFAULT &&
            Object.keys(mongoOptions.credentials.mechanismProperties).length ***REMOVED***= 0) {
    ...
    if (isSRV) {
        ...
        if (mongoOptions.srvMaxHosts > 0 && typeof mongoOptions.replicaSet ***REMOVED***= 'string') {
    ...
    if (mongoOptions.directConnection && mongoOptions.hosts.length !***REMOVED*** 1) {
...
exports.OPTIONS = {
    ...
    authMechanism: {
        ...
        transform({ options, values: [value] }) {
            ...
            if (mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_PLAIN ||
                providers_1.AUTH_MECHS_AUTH_SRC_EXTERNAL.has(mechanism)) {
                // some mechanisms have '$external' as the Auth Source
                source = '$external';
            }
            let password = options.credentials?.password;
            if (mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_X509 && password ***REMOVED***= '') {
                password = undefined;
            }
            return mongo_credentials_1.MongoCredentials.merge(options.credentials, {
                mechanism,
    ...
    authMechanismProperties: {
        ...
        transform({ options, values }) {
            ...
            for (const optionValue of values) {
                if (typeof optionValue ***REMOVED***= 'string') {
    ...
    serverApi: {
        ...
        transform({ values: [version] }) {
            const serverApiToValidate = typeof version ***REMOVED***= 'string' ? { version } : version;
            const versionToValidate = serverApiToValidate && serverApiToValidate.version;
            if (!versionToValidate) {
                throw new error_1.MongoParseError(`Invalid \`serverApi\` property; must specify a version from the following enum: ["${Object.values(mongo_client_1.ServerApiVersion).join('", "')}"]`);
            }
            if (!Object.values(mongo_client_1.ServerApiVersion).some(v => v ***REMOVED***= versionToValidate)) {
    ...
    compressors: {
        ...
        transform({ values }) {
            ...
            for (const compVal of values) {
                const compValArray = typeof compVal ***REMOVED***= 'string' ? compVal.split(',') : compVal;
    ...
    family: {
        transform({ name, values: [value] }) {
            ...
            if (transformValue ***REMOVED***= 4 || transformValue ***REMOVED***= 6) {
    ...
    pkFactory: {
        ...
        transform({ values: [value] }) {
            if ((0, utils_1.isRecord)(value, ['createPk']) && typeof value.createPk ***REMOVED***= 'function') {
                return value;
            }
    ...
    writeConcern: {
        ...
        transform({ values: [value], options }) {
            ...
            else if (value ***REMOVED***= 'majority' || typeof value ***REMOVED***= 'number') {
                return write_concern_1.WriteConcern.fromOptions({
                    writeConcern: {
                        ...options.writeConcern,
                        w: value
    ...
    mongodbLogPath: {
        transform({ values: [value] }) {
            if (!((typeof value ***REMOVED***= 'string' && ['stderr', 'stdout'].includes(value)) ||
                (value &&
                    typeof value ***REMOVED***= 'object' &&
                    'write' in value &&
                    typeof value.write ***REMOVED***= 'function'))) {
    ...
    mongodbLogComponentSeverities: {
        transform({ values: [value] }) {
            ...
            for (const [k, v] of Object.entries(value)) {
                ...
                if (!Object.values(mongo_logger_1.MongoLoggableComponent).some(val => val ***REMOVED***= k) && k !***REMOVED*** 'default') {
                    throw new error_1.MongoAPIError(`User input for option 'mongodbLogComponentSeverities' contains invalid key: ${k}`);
                }
                if (!Object.values(mongo_logger_1.SeverityLevel).some(val => val ***REMOVED***= v)) {
