...
async function executeOperation(client, operation, timeoutContext) {
    ...
    const topology = client.topology ***REMOVED*** null
    ...
    else if (session.client !***REMOVED*** client) {
    ...
    if (inTransaction &&
        ...
        (hasReadAspect || operation.commandName ***REMOVED***= 'runCommand')) {
    ...
    try {
        ...
        if (session?.owner != null && session.owner ***REMOVED***= owner) {
...
async function tryOperation(operation, { topology, timeoutContext, session, readPreference }) {
    ...
    for (let tries = 0; tries < maxTries; tries++) {
        if (previousOperationError) {
            if (hasWriteAspect && previousOperationError.code ***REMOVED***= MMAPv1_RETRY_WRITES_ERROR_CODE) {
