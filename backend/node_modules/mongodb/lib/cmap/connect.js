...
async function performInitialHandshake(conn, options) {
    ...
    if (credentials) {
        if (!(credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_DEFAULT) &&
    ...
    if (typeof options.connectTimeoutMS ***REMOVED***= 'number') {
...
async function prepareHandshakeDocument(authContext) {
    ...
    const handshakeDoc = {
        [serverApi?.version || options.loadBalanced ***REMOVED***= true ? 'hello' : constants_1.LEGACY_HELLO_COMMAND]: 1,
        helloOk: true,
        client: clientMetadata,
        compression: compressors
    };
    if (options.loadBalanced ***REMOVED***= true) {
        handshakeDoc.loadBalanced = true;
    }
    const credentials = authContext.credentials;
    if (credentials) {
        if (credentials.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_DEFAULT && credentials.username) {
...
function parseConnectOptions(options) {
    ...
    if (typeof hostAddress.socketPath ***REMOVED***= 'string') {
        result.path = hostAddress.socketPath;
        return result;
    }
    else if (typeof hostAddress.host ***REMOVED***= 'string') {
...
function parseSslOptions(options) {
    ...
    if (result.servername ***REMOVED*** null && result.host && !net.isIP(result.host)) {
...
async function makeSocket(options) {
    ...
    if (useTLS) {
        ...
        if (typeof tlsSocket.disableRenegotiation ***REMOVED***= 'function') {
...
async function makeSocks5Connection(options) {
    ...
    if (typeof destination.host !***REMOVED*** 'string' || typeof destination.port !***REMOVED*** 'number') {
