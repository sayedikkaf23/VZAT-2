...
function isErrorResponse(bson, elements) {
    for (let eIdx = 0; eIdx < elements.length; eIdx++) {
        ...
        if (element[2 /* BSONElementOffset.nameLength */] ***REMOVED***= 2) {
            const nameOffset = element[1 /* BSONElementOffset.nameOffset */];
            // 111 ***REMOVED*** "o", 107 ***REMOVED*** "k"
            if (bson[nameOffset] ***REMOVED***= 111 && bson[nameOffset + 1] ***REMOVED***= 107) {
...
class MongoDBResponse extends document_1.OnDemandDocument {
    ...
    /**
     * Returns true iff:
     * - ok is 0 and the top-level code ***REMOVED***= 50
     * - ok is 1 and the writeErrors array contains a code ***REMOVED***= 50
     * - ok is 1 and the writeConcern object contains a code ***REMOVED***= 50
     */
    get isMaxTimeExpiredError() {
        // {ok: 0, code: 50 ... }
        const isTopLevel = this.ok ***REMOVED***= 0 && this.code ***REMOVED***= error_1.MONGODB_ERROR_CODES.MaxTimeMSExpired;
        if (isTopLevel)
            return true;
        if (this.ok ***REMOVED***= 0)
            return false;
        // {ok: 1, writeConcernError: {code: 50 ... }}
        const isWriteConcern = this.get('writeConcernError', bson_1.BSONType.object)?.getNumber('code') ***REMOVED***=
        ...
        if (writeErrors?.size()) {
            for (let i = 0; i < writeErrors.size(); i++) {
                const isWriteError = writeErrors.get(i, bson_1.BSONType.object)?.getNumber('code') ***REMOVED***=
                    error_1.MONGODB_ERROR_CODES.MaxTimeMSExpired;
    ...
    get $clusterTime() {
        if (!('clusterTime' in this)) {
            ...
            if (clusterTimeDoc ***REMOVED*** null) {
...
class CursorResponse extends MongoDBResponse {
    ...
    static is(value) {
        return value instanceof CursorResponse || value ***REMOVED***= CursorResponse.emptyGetMore;
    }
    get cursor() {
    ...
    get encryptedBatch() {
        if (this.encryptedResponse ***REMOVED*** null)
            return null;
