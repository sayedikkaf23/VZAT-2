...
class Encrypter {
    constructor(client, uri, options) {
        if (typeof options.autoEncryption !***REMOVED*** 'object') {
        ...
        if (options.maxPoolSize ***REMOVED***= 0 && options.autoEncryption.keyVaultClient ***REMOVED*** null) {
            options.autoEncryption.keyVaultClient = client;
        }
        else if (options.autoEncryption.keyVaultClient ***REMOVED*** null) {
            options.autoEncryption.keyVaultClient = this.getInternalClient(client, uri, options);
        }
        if (this.bypassAutoEncryption) {
            options.autoEncryption.metadataClient = undefined;
        }
        else if (options.maxPoolSize ***REMOVED***= 0) {
    ...
    getInternalClient(client, uri, options) {
        ...
        if (internalClient ***REMOVED*** null) {
            const clonedOptions = {};
            for (const key of [
    ...
    async close(client) {
        ...
        const internalClient = this.internalClient;
        if (internalClient != null && client !***REMOVED*** internalClient) {
