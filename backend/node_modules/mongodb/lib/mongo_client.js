...
class MongoClient extends mongo_types_1.TypedEventEmitter {
    constructor(url, options) {
        ...
        const shouldSetLogger = Object.values(this.options.mongoLoggerOptions.componentSeverities).some(value => value !***REMOVED*** mongo_logger_1.SeverityLevel.OFF);
    ...
    checkForNonGenuineHosts() {
        ...
        if (documentDBHostnames.length !***REMOVED*** 0 || srvHostIsDocumentDB) {
            this.mongoLogger?.info('client', utils_1.DOCUMENT_DB_MSG);
        }
        else if (cosmosDBHostnames.length !***REMOVED*** 0 || srvHostIsCosmosDB) {
    ...
    async _connect() {
        ...
        if (options.tls) {
            if (typeof options.tlsCAFile ***REMOVED***= 'string') {
                options.ca ??= await fs_1.promises.readFile(options.tlsCAFile);
            }
            if (typeof options.tlsCRLFile ***REMOVED***= 'string') {
                options.crl ??= await fs_1.promises.readFile(options.tlsCRLFile);
            }
            if (typeof options.tlsCertificateKeyFile ***REMOVED***= 'string') {
        ...
        if (options.credentials?.mechanism ***REMOVED***= providers_1.AuthMechanism.MONGODB_OIDC) {
    ...
    async _close() {
        ...
        if (servers.length !***REMOVED*** 0) {
            ...
            if (endSessions.length !***REMOVED*** 0) {
                try {
    ...
    async withSession(optionsOrExecutor, executor) {
        const options = {
            ...
            ...(typeof optionsOrExecutor ***REMOVED***= 'object' ? optionsOrExecutor : {})
        };
        const withSessionCallback = typeof optionsOrExecutor ***REMOVED***= 'function' ? optionsOrExecutor : executor;
        if (withSessionCallback ***REMOVED*** null) {
