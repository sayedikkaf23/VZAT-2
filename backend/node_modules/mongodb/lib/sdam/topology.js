...
class Topology extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(client, seeds, options) {
        ...
        if (typeof seeds ***REMOVED***= 'string') {
        ...
        const selectedHosts = options.srvMaxHosts ***REMOVED*** null ||
            options.srvMaxHosts ***REMOVED***= 0 ||
    ...
    detectShardedTopology(event) {
        ...
        const transitionToSharded = previousType !***REMOVED*** common_1.TopologyType.Sharded && newType ***REMOVED***= common_1.TopologyType.Sharded;
    ...
    detectSrvRecords(ev) {
        ...
        if (this.s.description ***REMOVED***= previousTopologyDescription) {
    ...
    async _connect(options) {
        options = options ?? {};
        if (this.s.state ***REMOVED***= common_1.STATE_CONNECTED) {
            return this;
        }
        ...
        try {
            ...
            const skipPingOnConnect = this.s.options.__skipPingOnConnect ***REMOVED***= true;
    ...
    /** Close this topology */
    close() {
        if (this.s.state ***REMOVED***= common_1.STATE_CLOSED || this.s.state ***REMOVED***= common_1.STATE_CLOSING) {
            return;
        }
        for (const server of this.s.servers.values()) {
    ...
    async selectServer(selector, options) {
        let serverSelector;
        if (typeof selector !***REMOVED*** 'function') {
            if (typeof selector ***REMOVED***= 'string') {
        ...
        const isSharded = this.description.type ***REMOVED***= common_1.TopologyType.Sharded;
        ...
        try {
            ...
            if (options.timeoutContext?.csotEnabled() && server.description.minRoundTripTime !***REMOVED*** 0) {
    ...
    auth(credentials, callback) {
        if (typeof credentials ***REMOVED***= 'function')
            (callback = credentials), (credentials = undefined);
        if (typeof callback ***REMOVED***= 'function')
    ...
    isConnected() {
        return this.s.state ***REMOVED***= common_1.STATE_CONNECTED;
    }
    isDestroyed() {
        return this.s.state ***REMOVED***= common_1.STATE_CLOSED;
    }
    ...
    lastHello() {
        ...
        if (serverDescriptions.length ***REMOVED***= 0)
            return {};
        const sd = serverDescriptions.filter((sd) => sd.type !***REMOVED*** common_1.ServerType.Unknown)[0];
...
function updateServers(topology, incomingServerDescription) {
    ...
    if (incomingServerDescription && topology.s.servers.has(incomingServerDescription.address)) {
        ...
        if (server) {
            ...
            else if (incomingServerDescription.error ***REMOVED*** null) {
                const newTopologyType = topology.s.description.type;
                const shouldMarkPoolReady = incomingServerDescription.isDataBearing ||
                    (incomingServerDescription.type !***REMOVED*** common_1.ServerType.Unknown &&
                        newTopologyType ***REMOVED***= common_1.TopologyType.Single);
...
function processWaitQueue(topology) {
    if (topology.s.state ***REMOVED***= common_1.STATE_CLOSED) {
        drainWaitQueue(topology.waitQueue, new error_1.MongoTopologyClosedError());
        return;
    }
    const isSharded = topology.description.type ***REMOVED***= common_1.TopologyType.Sharded;
    ...
    for (let i = 0; i < membersToProcess; ++i) {
        ...
        let selectedServer;
        if (selectedDescriptions.length ***REMOVED***= 0) {
            if (!waitQueueMember.waitingLogged) {
                if (topology.client.mongoLogger?.willLog(mongo_logger_1.MongoLoggableComponent.SERVER_SELECTION, mongo_logger_1.SeverityLevel.INFORMATIONAL)) {
                    topology.client.mongoLogger?.info(mongo_logger_1.MongoLoggableComponent.SERVER_SELECTION, new server_selection_events_1.WaitingForSuitableServerEvent(waitQueueMember.serverSelector, topology.description, topology.s.serverSelectionTimeoutMS !***REMOVED*** 0
        ...
        else if (selectedDescriptions.length ***REMOVED***= 1) {
