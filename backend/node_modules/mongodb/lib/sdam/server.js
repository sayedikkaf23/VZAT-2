...
class Server extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(topology, description, options) {
        ...
        this.monitor.on(Server.SERVER_HEARTBEAT_SUCCEEDED, (event) => {
            ...
            if (this.s.state ***REMOVED***= common_1.STATE_CONNECTING) {
    ...
    get loadBalanced() {
        return this.topology.description.type ***REMOVED***= common_1.TopologyType.LoadBalanced;
    }
    /**
     * Initiate server connect
     */
    connect() {
        if (this.s.state !***REMOVED*** common_1.STATE_CLOSED) {
            return;
        }
    ...
    async command(ns, cmd, { ...options }, responseType) {
        if (ns.db ***REMOVED*** null || typeof ns ***REMOVED***= 'string') {
            throw new error_1.MongoInvalidArgumentError('Namespace must not be a string');
        }
        if (this.s.state ***REMOVED***= common_1.STATE_CLOSING || this.s.state ***REMOVED***= common_1.STATE_CLOSED) {
        ...
        try {
            ...
            if (operationError instanceof error_1.MongoError &&
                operationError.code ***REMOVED***= error_1.MONGODB_ERROR_CODES.Reauthenticate) {
            ...
            this.decrementOperationCount();
            if (session?.pinnedConnection !***REMOVED*** conn) {
    ...
    decorateCommandError(connection, cmd, options, error) {
        if (typeof error !***REMOVED*** 'object' || error ***REMOVED*** null || !('name' in error)) {
            throw new error_1.MongoRuntimeError('An unexpected error type: ' + typeof error);
        }
        if (error.name ***REMOVED***= 'AbortError' && 'cause' in error && error.cause instanceof error_1.MongoError) {
            error = error.cause;
        }
        if (!(error instanceof error_1.MongoError)) {
...
function connectionIsStale(pool, connection) {
    if (connection.serviceId) {
        return (connection.generation !***REMOVED*** pool.serviceGenerations.get(connection.serviceId.toHexString()));
    }
    return connection.generation !***REMOVED*** pool.generation;
}
...
function isRetryableWritesEnabled(topology) {
    return topology.s.options.retryWrites !***REMOVED*** false;
}
