...
class ClientEncryption {
    ...
    constructor(client, options) {
        ...
        if (options.keyVaultNamespace ***REMOVED*** null) {
    ...
    async createDataKey(provider, options = {}) {
        ...
        if (options.keyAltNames && options.keyAltNames.length > 0) {
            keyAltNames = options.keyAltNames.map((keyAltName, i) => {
                if (typeof keyAltName !***REMOVED*** 'string') {
     ...
     * const result = await clientEncryption.rewrapManyDataKey(filter);
     *
     * if (result.bulkWriteResult ***REMOVED*** null) {
    ...
    async createEncryptedCollection(db, name, options) {
        ...
        if (Array.isArray(encryptedFields.fields)) {
            const createDataKeyPromises = encryptedFields.fields.map(async (field) => field ***REMOVED*** null || typeof field !***REMOVED*** 'object' || field.keyId != null
                ? field
                : {
                    ...field,
            ...
            encryptedFields.fields = createDataKeyResolutions.map((resolution, index) => resolution.status ***REMOVED***= 'fulfilled' ? resolution.value : encryptedFields.fields[index]);
            const rejection = createDataKeyResolutions.find((result) => result.status ***REMOVED***= 'rejected');
            if (rejection != null) {
    ...
    async _encrypt(value, expressionMode, options) {
        ...
        if (typeof contentionFactor ***REMOVED***= 'number' || typeof contentionFactor ***REMOVED***= 'bigint') {
            contextOptions.contentionFactor = contentionFactor;
        }
        if (typeof queryType ***REMOVED***= 'string') {
            contextOptions.queryType = queryType;
        }
        if (typeof rangeOptions ***REMOVED***= 'object') {
