...
class ClientSession extends mongo_types_1.TypedEventEmitter {
    ...
    constructor(client, sessionPool, options, clientOptions) {
        ...
        if (client ***REMOVED*** null) {
            // TODO(NODE-3483)
            throw new error_1.MongoRuntimeError('ClientSession requires a MongoClient');
        }
        if (sessionPool ***REMOVED*** null || !(sessionPool instanceof ServerSessionPool)) {
            // TODO(NODE-3483)
            throw new error_1.MongoRuntimeError('ClientSession requires a ServerSessionPool');
        }
        options = options ?? {};
        this.snapshotEnabled = options.snapshot ***REMOVED***= true;
        if (options.causalConsistency ***REMOVED***= true && this.snapshotEnabled) {
        ...
        const defaultCausalConsistencyValue = this.explicit && options.snapshot !***REMOVED*** true;
        this.supports = {
    ...
    get loadBalanced() {
        return this.client.topology?.description.type ***REMOVED***= common_1.TopologyType.LoadBalanced;
    }
    /** @internal */
    pin(conn) {
    ...
    async endSession(options) {
        try {
            ...
            if (error.name ***REMOVED***= 'MongoOperationTimeoutError')
                throw error;
    ...
    advanceOperationTime(operationTime) {
        if (this.operationTime ***REMOVED*** null) {
    ...
    advanceClusterTime(clusterTime) {
        if (!clusterTime || typeof clusterTime !***REMOVED*** 'object') {
            throw new error_1.MongoInvalidArgumentError('input cluster time must be an object');
        }
        if (!clusterTime.clusterTime || clusterTime.clusterTime._bsontype !***REMOVED*** 'Timestamp') {
            throw new error_1.MongoInvalidArgumentError('input cluster time "clusterTime" property must be a valid BSON Timestamp');
        }
        if (!clusterTime.signature ||
            clusterTime.signature.hash?._bsontype !***REMOVED*** 'Binary' ||
            (typeof clusterTime.signature.keyId !***REMOVED*** 'bigint' &&
                typeof clusterTime.signature.keyId !***REMOVED*** 'number' &&
                clusterTime.signature.keyId?._bsontype !***REMOVED*** 'Long') // apparently we decode the key to number?
        ) {
    ...
    equals(session) {
        ...
        if (this.id ***REMOVED*** null || session.id ***REMOVED*** null) {
            return false;
        }
    ...
    async commitTransaction(options) {
        if (this.transaction.state ***REMOVED***= transactions_1.TxnState.NO_TRANSACTION) {
            throw new error_1.MongoTransactionError('No transaction started');
        }
        if (this.transaction.state ***REMOVED***= transactions_1.TxnState.STARTING_TRANSACTION ||
            this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_COMMITTED_EMPTY) {
            // the transaction was never started, we can safely exit here
            this.transaction.transition(transactions_1.TxnState.TRANSACTION_COMMITTED_EMPTY);
            return;
        }
        if (this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_ABORTED) {
        ...
        const timeoutMS = typeof options?.timeoutMS ***REMOVED***= 'number'
            ? options.timeoutMS
            : typeof this.timeoutMS ***REMOVED***= 'number'
                ? this.timeoutMS
                : null;
        const wc = this.transaction.options.writeConcern ?? this.clientOptions?.writeConcern;
        if (wc != null) {
            if (timeoutMS ***REMOVED*** null && this.timeoutContext ***REMOVED*** null) {
        ...
        if (this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_COMMITTED || this.commitAttempted) {
            if (timeoutMS ***REMOVED*** null && this.timeoutContext ***REMOVED*** null) {
        ...
        if (typeof this.transaction.options.maxTimeMS ***REMOVED***= 'number') {
        ...
        const timeoutContext = this.timeoutContext ??
            (typeof timeoutMS ***REMOVED***= 'number'
    ...
    async abortTransaction(options) {
        ...
        if (this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_COMMITTED ||
            this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_COMMITTED_EMPTY) {
        ...
        const timeoutContext = timeoutMS != null
            ...
            : null;
        const wc = this.transaction.options.writeConcern ?? this.clientOptions?.writeConcern;
        if (wc != null && timeoutMS ***REMOVED*** null) {
            write_concern_1.WriteConcern.apply(command, { wtimeoutMS: 10000, w: 'majority', ...wc });
        }
        if (this.transaction.recoveryToken) {
            command.recoveryToken = this.transaction.recoveryToken;
        ...
        try {
            ...
            this.unpin();
            if (firstAbortError.name ***REMOVED***= 'MongoRuntimeError')
                throw firstAbortError;
            if (options?.throwTimeout && firstAbortError.name ***REMOVED***= 'MongoOperationTimeoutError') {
                throw firstAbortError;
            }
            if (firstAbortError instanceof error_1.MongoError && (0, error_1.isRetryableWriteError)(firstAbortError)) {
                try {
                    ...
                    if (secondAbortError.name ***REMOVED***= 'MongoRuntimeError')
                        throw secondAbortError;
                    if (options?.throwTimeout && secondAbortError.name ***REMOVED***= 'MongoOperationTimeoutError') {
                        throw secondAbortError;
                    }
        ...
        try {
            while (!committed) {
                ...
                try {
                    ...
                    if (this.transaction.state ***REMOVED***= transactions_1.TxnState.NO_TRANSACTION ||
                        this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_COMMITTED ||
                        this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_ABORTED) {
                    ...
                    if (this.transaction.state ***REMOVED***= transactions_1.TxnState.STARTING_TRANSACTION ||
                        this.transaction.state ***REMOVED***= transactions_1.TxnState.TRANSACTION_IN_PROGRESS) {
                        await this.abortTransaction();
                    }
...
function isUnknownTransactionCommitResult(err) {
    ...
    return (isMaxTimeMSExpiredError(err) ||
        (!isNonDeterministicWriteConcernError &&
            err.code !***REMOVED*** error_1.MONGODB_ERROR_CODES.UnsatisfiableWriteConcern &&
            err.code !***REMOVED*** error_1.MONGODB_ERROR_CODES.UnknownReplWriteConcern));
}
function maybeClearPinnedConnection(session, options) {
    ...
    if (conn && topology != null) {
        ...
        if (options?.error ***REMOVED*** null || options?.force) {
            loadBalancer.pool.checkIn(conn);
            session.pinnedConnection = undefined;
            conn.emit(constants_1.UNPINNED, session.transaction.state !***REMOVED*** transactions_1.TxnState.NO_TRANSACTION
...
function isMaxTimeMSExpiredError(err) {
    if (err ***REMOVED*** null || !(err instanceof error_1.MongoServerError)) {
        return false;
    }
    return (err.code ***REMOVED***= error_1.MONGODB_ERROR_CODES.MaxTimeMSExpired ||
        err.writeConcernError?.code ***REMOVED***= error_1.MONGODB_ERROR_CODES.MaxTimeMSExpired);
}
/**
...
function applySession(session, command, options) {
    ...
    if (options.writeConcern?.w ***REMOVED***= 0) {
    ...
    if (!inTxnOrTxnCommand) {
        if (session.transaction.state !***REMOVED*** transactions_1.TxnState.NO_TRANSACTION) {
    ...
    if (session.transaction.state ***REMOVED***= transactions_1.TxnState.STARTING_TRANSACTION) {
...
function updateSessionFromResponse(session, document) {
    ...
    if (session?.snapshotEnabled && session.snapshotTime ***REMOVED*** null) {
