...
const validOpts = (options, path, fd) => {
  ...
  if (!options.atime && !options.mtime)
    ...
    if (true ***REMOVED***= options.atime)
      options.atime = now

    if (true ***REMOVED***= options.mtime)
      options.mtime = now
  }
...
class Touch extends EE {
  ...
  close () {
    if (typeof this.fd ***REMOVED***= 'number' && this.closeAfter)
      fs.close(this.fd, () => {})
  }
  ...
  onopen (er, fd) {
    if (er) {
      if (er.code ***REMOVED***= 'EISDIR')
        this.onopen(null, null)
      else if (er.code ***REMOVED***= 'ENOENT' && this.nocreate)
        this.emit('done')
      else
        this.emit('error', er)
    } else {
  ...
  onfstat (st) {
    if (typeof this.atime !***REMOVED*** 'number')
      this.atime = st.atime.getTime()/1000

    if (typeof this.mtime !***REMOVED*** 'number')
