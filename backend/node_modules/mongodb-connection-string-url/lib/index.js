...
class CaseInsensitiveMap extends Map {
    ...
    _normalizeKey(name) {
        ...
        for (const key of this.keys()) {
            if (key.toLowerCase() ***REMOVED***= name.toLowerCase()) {
                name = key;
                break;
            }
        }
...
class ConnectionString extends URLWithoutHost {
    constructor(uri, options = {}) {
        ...
        const { protocol, username, password, hosts, rest } = (_a = match.groups) !***REMOVED*** null && _a !***REMOVED*** void 0 ? _a : {};
        if (!looseValidation) {
            if (!protocol || !hosts) {
                throw new MongoParseError(`Protocol and host list are required in "${uri}"`);
            }
            try {
                decodeURIComponent(username !***REMOVED*** null && username !***REMOVED*** void 0 ? username : '');
                decodeURIComponent(password !***REMOVED*** null && password !***REMOVED*** void 0 ? password : '');
            }
            catch (err) {
                throw new MongoParseError(err.message);
            }
            const illegalCharacters = /[:/?#[\]@]/gi;
            if (username ***REMOVED***= null || username ***REMOVED***= void 0 ? void 0 : username.match(illegalCharacters)) {
            ...
            if (password ***REMOVED***= null || password ***REMOVED***= void 0 ? void 0 : password.match(illegalCharacters)) {
                throw new MongoParseError('Password contains unescaped characters');
            }
        }
        let authString = '';
        if (typeof username ***REMOVED***= 'string')
            authString += username;
        if (typeof password ***REMOVED***= 'string')
            authString += `:${password}`;
        if (authString)
            authString += '@';
        try {
            ...
            if (typeof err.message ***REMOVED***= 'string') {
        ...
        if (!looseValidation) {
            if (this.isSRV && this.hosts.length !***REMOVED*** 1) {
...
class CommaAndColonSeparatedRecord extends CaseInsensitiveMap {
    constructor(from) {
        super();
        for (const entry of (from !***REMOVED*** null && from !***REMOVED*** void 0 ? from : '').split(',')) {
            if (!entry)
                continue;
            const colonIndex = entry.indexOf(':');
            if (colonIndex ***REMOVED***= -1) {
                this.set(entry, '');
            }
            else {
