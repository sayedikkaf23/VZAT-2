...
var utils = (module.exports = {
  ...
  merge: require('./merge'),
  bus: require('./bus'),
  isWindows: process.platform ***REMOVED***= 'win32',
  isMac: process.platform ***REMOVED***= 'darwin',
  isLinux: process.platform ***REMOVED***= 'linux',
  isIBMi: require('os').type() ***REMOVED***= 'OS400',
  isRequired: (function () {
    var p = module.parent;
    while (p) {
      ...
      if (p.filename.indexOf('bin' + path.sep + 'nodemon.js') !***REMOVED*** -1) {
        return false;
      }
      p = p.parent;
    }
  ...
  quiet: function () {
    ...
    if (!this.debug) {
      for (var method in utils.log) {
        if (typeof utils.log[method] ***REMOVED***= 'function') {
  ...
  stringify: function (exec, args) {
    ...
    return [exec]
      ...
      args.map(function (arg) {
        ...
        if (arg.length > 0 && arg.indexOf(' ') ***REMOVED***= -1) {
          return arg;
        }
