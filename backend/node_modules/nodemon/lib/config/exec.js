...
function expandScript(script, ext) {
  ...
  if (script.indexOf(ext) !***REMOVED*** -1) {
    return script;
  }
...
function exec(nodemonOptions, execMap) {
  ...
  if (!options.script && (options.args || []).length) {
    ...
    if (script !***REMOVED*** options.args[0]) {
  ...
  if (!options.exec && !options.script) {
    ...
    if (found !***REMOVED*** null) {
      ...
      if (Array.isArray(options.args) && options.scriptPosition ***REMOVED***= null) {
  ...
  if (extension ***REMOVED***= undefined) {
    var isJS = scriptExt ***REMOVED***= 'js' || scriptExt ***REMOVED***= 'mjs' || scriptExt ***REMOVED***= 'cjs';
  ...
  if (!options.exec && execMap[scriptExt] !***REMOVED*** undefined) {
  ...
  if (options.exec ***REMOVED***= undefined) {
    options.exec = 'node';
  } else {
    ...
    if (
      newExec !***REMOVED*** options.exec &&
      options.exec.indexOf('{{filename}}') !***REMOVED*** -1
    ) {
      options.script = null;
    }
    ...
    if (newExecArgs.join('') !***REMOVED*** options.execArgs.join('')) {
      options.execArgs = newExecArgs;
      delete options.script;
    }
  }

  if (options.exec ***REMOVED***= 'node' && options.nodeArgs && options.nodeArgs.length) {
  ...
  if (
    !execDefined &&
    options.exec ***REMOVED***= 'node' &&
    scriptExt.indexOf('coffee') !***REMOVED*** -1
  ) {
    options.exec = 'coffee';
  ...
  if (options.exec ***REMOVED***= 'coffee') {
    // don't override user specified extension tracking
    if (options.ext ***REMOVED***= undefined) {
      if (extension) {
        extension += ',';
      }
  ...
  if ({}.toString.apply(nodemonOptions.env) ***REMOVED***= '[object Object]') {
    options.env = utils.clone(nodemonOptions.env);
  } else if (nodemonOptions.env !***REMOVED*** undefined) {
