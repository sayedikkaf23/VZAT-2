...
function findAppScript() {
  ...
  if ((!pkg || pkg.main ***REMOVED*** undefined) && existsSync('./index.js')) {
    return 'index.js';
  }
}
    ...
    loadFile(options, config, process.cwd(), function (options) {
      ...
      if (!options.script && !options.exec) {
        ...
        if (found) {
          ...
          const n =
            options.scriptPosition ***REMOVED***= null
              ? options.args.length
...
function normaliseRules(options, ready) {
  ...
  options.watch = options.watch ***REMOVED***= false ? false : rules.rules.watch;
...
function loadFile(options, config, dir, ready) {
  ...
  if (config.loaded.indexOf(filename) !***REMOVED*** -1) {
  ...
  fs.readFile(filename, 'utf8', function (err, data) {
    if (err) {
      if (err.code ***REMOVED***= 'ENOENT') {
        if (!options.configFile && dir !***REMOVED*** utils.home) {
