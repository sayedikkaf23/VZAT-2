...
function nodemon(settings) {
  ...
  if (typeof settings ***REMOVED***= 'string') {
    settings = settings.trim();
    if (settings.indexOf('node') !***REMOVED*** 0) {
      if (settings.indexOf('nodemon') !***REMOVED*** 0) {
  ...
  if (options.cwd) {
    ...
    if (process.cwd() !***REMOVED*** path.resolve(config.system.cwd, options.cwd)) {
  ...
  config.load(options, function (config) {
    if (!config.options.dump && !config.options.execOptions.script &&
      config.options.execOptions.exec ***REMOVED***= 'node') {
      if (!config.required) {
    ...
    if (config.options.stdin && config.options.restartable) {
      ...
      process.stdin.on('data', data => {
        ...
        if (str ***REMOVED***= config.options.restartable) {
          bus.emit('restart');
        } else if (data.charCodeAt(0) ***REMOVED***= 12) { // ctrl+l
          console.clear();
        }
      });
      ...
      process.stdin.on('data', function (data) {
        ...
        if (chr ***REMOVED***= 3) {
          ...
          ctrlC = true;
          return;
        } else if (buffer ***REMOVED***= '.exit' || chr ***REMOVED***= 4) { // ctrl+d
          process.exit();
        } else if (chr ***REMOVED***= 13 || chr ***REMOVED***= 10) { // enter / carriage return
          buffer = '';
        } else if (chr ***REMOVED***= 12) { // ctrl+l
          console.clear();
          buffer = '';
        }
        ctrlC = false;
    ...
    if (!config.required) {
      const restartSignal = config.options.signal ***REMOVED***= 'SIGUSR2' ? 'SIGHUP' : 'SIGUSR2';
    ...
    const ignoring = config.options.monitor.map(function (rule) {
      ...
      if (defaults.ignoreRoot.indexOf(rule) !***REMOVED*** -1) {
        return false;
    ...
    if (config.options.stdout ***REMOVED***= false) {
...
nodemon.removeAllListeners = function (event) {
  ...
  Object.keys(eventHandlers).filter(function (e) {
    return event ? e ***REMOVED***= event : true;
