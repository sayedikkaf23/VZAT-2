...
function watch() {
  ...
  const promise = new Promise(function (resolve) {
    ...
    if (addDotFile.length ***REMOVED***= 0) {
  ...
  return promise.catch(e => {
    ...
    utils.log.detail(`watching ${watchedFiles.length} file${
      watchedFiles.length ***REMOVED***= 1 ? '' : 's'}`);
    return watchedFiles;
  });
}
...
function filterAndRestart(files) {
  ...
  if (files.length) {
    ...
    if (utils.isWindows) {
      ...
      files = files.map(f => {
        if (f.indexOf(':') ***REMOVED***= -1) { return f; }
    ...
    if (config.options.execOptions && config.options.execOptions.script) {
      ...
      if (matched.result.length ***REMOVED***= 0 && script) {
        const length = script.length;
        files.find(file => {
          if (file.substr(-length, length) ***REMOVED***= script) {
            matched = {
              result: [file],
              total: 1,
            };
    ...
    if (matched.result.length) {
      if (config.options.delay > 0) {
        ...
        if (debouncedBus ***REMOVED***= undefined) {
