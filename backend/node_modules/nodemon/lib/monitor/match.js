...
function rulesToMonitor(watch, ignore, config) {
  ...
  monitor = monitor.map(function (rule) {
    var not = rule.slice(0, 1) ***REMOVED***= '!';

    if (not) {
      rule = rule.slice(1);
    }

    if (rule ***REMOVED***= '.' || rule ***REMOVED***= '.*') {
      rule = '*.*';
    }
    ...
    try {
      ...
      if (!not && base) {
        if (config.dirs.indexOf(base) ***REMOVED***= -1) {
    ...
    if (
      rule.slice(-4) !***REMOVED*** '**/*' &&
      rule.slice(-1) ***REMOVED***= '*' &&
      rule.indexOf('*.') ***REMOVED***= -1
    ) {
...
function match(files, monitor, ext) {
  ...
  var rules = monitor
    .sort(function (a, b) {
      ...
      var aIsIgnore = a.slice(0, 1) ***REMOVED***= '!';
      var bIsIgnore = b.slice(0, 1) ***REMOVED***= '!';
    ...
    .map(function (s) {
      ...
      if (prefix ***REMOVED***= '!') {
        if (s.indexOf('!' + cwd) ***REMOVED***= 0) {
          return s;
        }

        // if it starts with a period, then let's get the relative path
        if (s.indexOf('!.') ***REMOVED***= 0) {
          return '!' + path.resolve(cwd, s.substring(1));
        }

        return '!**' + (prefix !***REMOVED*** path.sep ? path.sep : '') + s.slice(1);
      }
      ...
      return '**' + (prefix !***REMOVED*** path.sep ? path.sep : '') + s;
    });
    ...
    for (var i = 0; i < rules.length; i++) {
      if (rules[i].slice(0, 1) ***REMOVED***= '!') {
        ...
        if (minimatch(file, rules[i], minimatchOpts)) {
          ...
          if (usedRules.indexOf(rules[i]) ***REMOVED***= -1) {
          ...
          if (
            rules[i] !***REMOVED*** '**' + path.sep + '*.*' &&
            rules[i].slice(-3) ***REMOVED***= '*.*'
          ) {
            whitelist.push(file);
          } else if (path.basename(file) ***REMOVED***= path.basename(rules[i])) {
