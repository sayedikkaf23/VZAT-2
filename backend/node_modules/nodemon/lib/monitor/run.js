...
function run(options) {
  ...
  var runCmd = !options.runOnChangeOnly || config.lastStarted !***REMOVED*** 0;
  if (runCmd) {
    ...
    if (config.options.watch !***REMOVED*** false) {
      watch();
      return;
    }
  }
  ...
  if (config.options.stdin ***REMOVED***= false) {
  ...
  if (utils.isWindows) {
    ...
    // ref #1251 and #1236
    if (executable.indexOf('/') !***REMOVED*** -1) {
  ...
  const shouldFork =
    ...
    !inBinPath &&
    !(firstArg.indexOf('-') ***REMOVED***= 0) && // don't fork if there's a node exec arg
    firstArg !***REMOVED*** 'inspect' && // don't fork it's `inspect` debugger
    executable ***REMOVED***= 'node' && // only fork if node
  ...
  child.on('exit', function (code, signal) {
    ...
    if (code ***REMOVED***= 2 && Date.now() < config.lastStarted + 500) {
    ...
    if (utils.isWindows && signal ***REMOVED***= 'SIGTERM') {
      signal = config.signal;
    }

    if (signal ***REMOVED***= config.signal || code ***REMOVED***= 0) {
      // this was a clean exit, so emit exit, rather than crash
      debug('bus.emit(exit) via ' + config.signal);
      bus.emit('exit', signal);

      // exit the monitor, but do it gracefully
      if (signal ***REMOVED***= config.signal) {
        return restart();
      }
...
function kill(child, signal, callback) {
  ...
  if (utils.isWindows) {
    ...
    if (
      signal ***REMOVED***= 'SIGKILL' ||
      osRelease < 10 ||
      signal ***REMOVED***= 'SIGUSR2' ||
      signal ***REMOVED***= 'SIGUSR1'
    ) {
...
run.kill = function (noRestart, callback) {
  ...
  if (typeof noRestart ***REMOVED***= 'function') {
    callback = noRestart;
    noRestart = false;
  }
  ...
  if (child !***REMOVED*** null) {
    ...
    if (child) {
      kill(child, config.signal, function () {
        ...
        if (child && run.options.stdin && child.stdin && oldPid ***REMOVED***= child.pid) {
          child.stdin.end();
        }
        callback();
      });
