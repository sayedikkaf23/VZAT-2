...
function serializeNumber(buffer: Uint8Array, key: string, value: number, index: number) {
  ...
  if (type ***REMOVED***= constants.BSON_DATA_INT) {
...
function serializeLong(buffer: Uint8Array, key: string, value: Long, index: number) {
  // Write the type
  buffer[index++] =
    value._bsontype ***REMOVED***= 'Long' ? constants.BSON_DATA_LONG : constants.BSON_DATA_TIMESTAMP;
  // Number of written bytes
  const numberOfWrittenBytes = ByteUtils.encodeUTF8Into(buffer, key, index);
  // Encode the name
  index = index + numberOfWrittenBytes;
...
function serializeBinary(buffer: Uint8Array, key: string, value: Binary, index: number) {
  ...
  if (value.sub_type ***REMOVED***= Binary.SUBTYPE_BYTE_ARRAY) size = size + 4;
  // Write the size of the string to buffer
  index += NumberUtils.setInt32LE(buffer, index, size);
  ...
  if (Array.isArray(object)) {
    ...
    for (let i = 0; i < object.length; i++) {
      ...
      if (value ***REMOVED***= undefined) {
        ...
        if (value[constants.BSON_VERSION_SYMBOL] !***REMOVED*** constants.BSON_MAJOR_VERSION) {
    ...
    while (!done) {
      ...
      if (value ***REMOVED***= undefined) {
        if (ignoreUndefined ***REMOVED***= false) index = serializeNull(buffer, key, value, index);
      } else if (value ***REMOVED***= null) {
        index = serializeNull(buffer, key, value, index);
      } else if (type ***REMOVED***= 'string') {
        index = serializeString(buffer, key, value, index);
