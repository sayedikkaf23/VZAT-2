 ...
 * (-2^63) because -MIN_VALUE ***REMOVED*** MIN_VALUE (since 2^63 cannot be represented as
...
export class Long extends BSONValue {
  ...
  constructor(
    ...
    const res =
      typeof lowOrValue ***REMOVED***= 'string'
  ...
  private static _fromString(str: string, unsigned: boolean, radix: number): Long {
    if (str.length ***REMOVED***= 0) throw new BSONError('empty string');
  ...
  static fromValue(
    ...
    if (typeof val ***REMOVED***= 'number') return Long.fromNumber(val, unsigned);
    if (typeof val ***REMOVED***= 'string') return Long.fromString(val, unsigned);
    // Throws for non-objects, converts non-instanceof Long:
    return Long.fromBits(
      val.low,
      val.high,
      typeof unsigned ***REMOVED***= 'boolean' ? unsigned : val.unsigned
    );
  }
  ...
  divide(divisor: string | number | Long | Timestamp): Long {
    ...
    if (!this.unsigned) {
      ...
      if (this.eq(Long.MIN_VALUE)) {
        ...
        // recall that -MIN_VALUE ***REMOVED*** MIN_VALUE
  ...
  getNumBitsAbs(): number {
    ...
    let bit: number;
    for (bit = 31; bit > 0; bit--) if ((val & (1 << bit)) !***REMOVED*** 0) break;
  ...
  shiftLeft(numBits: number | Long): Long {
    ...
    if ((numBits &= 63) ***REMOVED***= 0) return this;
    else if (numBits < 32)
      return Long.fromBits(
        this.low << numBits,
  ...
  shiftRightUnsigned(numBits: Long | number): Long {
    if (Long.isLong(numBits)) numBits = numBits.toInt();
    numBits &= 63;
    if (numBits ***REMOVED***= 0) return this;
    else {
      ...
      } else if (numBits ***REMOVED***= 32) return Long.fromBits(high, 0, this.unsigned);
