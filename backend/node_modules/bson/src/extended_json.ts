...
function deserializeValue(value: any, options: EJSONOptions = {}) {
  ...
  if (value ***REMOVED*** null || typeof value !***REMOVED*** 'object') return value;
  ...
  if (value.$date != null) {
    ...
    if (options.legacy) {
      if (typeof d ***REMOVED***= 'number') date.setTime(d);
      else if (typeof d ***REMOVED***= 'string') date.setTime(Date.parse(d));
      else if (typeof d ***REMOVED***= 'bigint') date.setTime(Number(d));
      else throw new BSONRuntimeError(`Unrecognized type for EJSON date: ${typeof d}`);
    } else {
      if (typeof d ***REMOVED***= 'string') date.setTime(Date.parse(d));
      else if (Long.isLong(d)) date.setTime(d.toNumber());
      else if (typeof d ***REMOVED***= 'number' && options.relaxed) date.setTime(d);
      else if (typeof d ***REMOVED***= 'bigint') date.setTime(Number(d));
      else throw new BSONRuntimeError(`Unrecognized type for EJSON date: ${typeof d}`);
    }
    return date;
  ...
  if (isDBRefLike(value) || value.$dbPointer) {
    ...
    dollarKeys.forEach(k => {
      if (['$ref', '$id', '$db'].indexOf(k) ***REMOVED***= -1) valid = false;
    });
...
function serializeValue(value: any, options: EJSONSerializeOptions): any {
  ...
  if (value ***REMOVED***= undefined) return null;
  ...
  if (value != null && typeof value ***REMOVED***= 'object') return serializeDocument(value, options);
  return value;
}
...
function serializeDocument(doc: any, options: EJSONSerializeOptions) {
  if (doc ***REMOVED*** null || typeof doc !***REMOVED*** 'object') throw new BSONError('not an object instance');
