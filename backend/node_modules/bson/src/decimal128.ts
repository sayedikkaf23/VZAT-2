...
export class Decimal128 extends BSONValue {
  ...
  private static _fromString(representation: string, options: { allowRounding: boolean }) {
    ...
    if (stringMatch) {
      ...
      if (e && expNumber ***REMOVED***= undefined) invalidErr(representation, 'missing exponent power');

      // they provided e, but didn't give a number before it. for ex "e1"
      if (e && unsignedNumber ***REMOVED***= undefined) invalidErr(representation, 'missing exponent base');
    ...
    if (options.allowRounding) {
      ...
      while (exponent < EXPONENT_MIN || nDigitsStored < nDigits) {
        ...
        if (nDigitsStored < nDigits) {
          if (
            representation[nDigits - 1 + Number(sawSign) + Number(sawRadix)] !***REMOVED*** '0' &&
  ...
  toString(): string {
    ...
    if (
      significand128.parts[0] ***REMOVED***= 0 &&
