...
SchemaSubdocument.prototype.cast = function(val, doc, init, priorVal, options) {
  if (val && val.$isSingleNested && val.parent ***REMOVED***= doc) {
    return val;
  }

  if (val != null && (typeof val !***REMOVED*** 'object' || Array.isArray(val))) {
    throw new ObjectExpectedError(this.path, val);
  }

  const discriminatorKeyPath = this.schema.path(this.schema.options.discriminatorKey);
  const defaultDiscriminatorValue = discriminatorKeyPath ***REMOVED*** null ? null : discriminatorKeyPath.getDefault(doc);
  ...
  const selected = parentSelected ***REMOVED*** null ? null : Object.keys(parentSelected).reduce((obj, key) => {
  ...
  if (init) {
    ...
    if (Object.keys(val).length ***REMOVED***= 0) {
...
SchemaSubdocument.prototype.discriminator = function(name, schema, options) {
  ...
  const clone = typeof options.clone ***REMOVED***= 'boolean'
    ? options.clone
    : true;
