...
function SchemaObjectId(key, options) {
  const isKeyHexStr = typeof key ***REMOVED***= 'string' && key.length ***REMOVED***= 24 && /^[a-f0-9]+$/i.test(key);
  const suppressWarning = options && options.suppressWarning;
  if ((isKeyHexStr || typeof key ***REMOVED***= 'undefined') && !suppressWarning) {
 ...
 *       assert.ok(typeof v !***REMOVED*** 'string' || v.length ***REMOVED***= 24);
 *       return original(v);
 *     });
 *
...
SchemaObjectId.prototype.cast = function(value, doc, init, prev, options) {
  if (!(isBsonType(value, 'ObjectId')) && SchemaType._isRef(this, value, doc, init)) {
    ...
    if ((getConstructorName(value) || '').toLowerCase() ***REMOVED***= 'objectid') {
