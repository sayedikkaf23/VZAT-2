 ...
 *       assert.ok(v ***REMOVED*** null || typeof v !***REMOVED*** 'object');
 *       return original(v);
 *     });
 *
...
SchemaString._defaultCaster = v => {
  if (v != null && typeof v !***REMOVED*** 'string') {
    throw new Error();
  }
  return v;
};
...
SchemaString._checkRequired = v => (v instanceof String || typeof v ***REMOVED***= 'string') && v.length;
...
SchemaString.prototype.enum = function() {
  if (this.enumValidator) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !***REMOVED*** this.enumValidator;
    }, this);
    this.enumValidator = false;
  }

  if (arguments[0] ***REMOVED***= void 0 || arguments[0] ***REMOVED***= false) {
    return this;
  }
  ...
  this.enumValidator = function(v) {
    return null ***REMOVED*** v || ~vals.indexOf(v);
  };
...
SchemaString.prototype.minlength = function(value, message) {
  if (this.minlengthValidator) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !***REMOVED*** this.minlengthValidator;
    }, this);
  }

  if (value !***REMOVED*** null && value !***REMOVED*** undefined) {
    let msg = message || MongooseError.messages.String.minlength;
    msg = msg.replace(/{MINLENGTH}/, value);
    this.validators.push({
      validator: this.minlengthValidator = function(v) {
        return v ***REMOVED***= null || v.length >= value;
      },
      message: msg,
      type: 'minlength',
      minlength: value
...
SchemaString.prototype.maxlength = function(value, message) {
  if (this.maxlengthValidator) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !***REMOVED*** this.maxlengthValidator;
    }, this);
  }
...
SchemaString.prototype.checkRequired = function checkRequired(value, doc) {
  if (typeof value ***REMOVED***= 'object' && SchemaType._isRef(this, value, doc, true)) {
    return value != null;
  }

  // `require('util').inherits()` does **not** copy static properties, and
  // plugins like mongoose-float use `inherits()` for pre-ES6.
  const _checkRequired = typeof this.constructor.checkRequired ***REMOVED***= 'function' ?
...
SchemaString.prototype.cast = function(value, doc, init, prev, options) {
  if (typeof value !***REMOVED*** 'string' && SchemaType._isRef(this, value, doc, init)) {
    return this._castRef(value, doc, init, options);
  }

  let castString;
  if (typeof this._castFunction ***REMOVED***= 'function') {
    castString = this._castFunction;
  } else if (typeof this.constructor.cast ***REMOVED***= 'function') {
...
const $conditionalHandlers = {
  ...
  $regex: function handle$regex(val) {
    if (Object.prototype.toString.call(val) ***REMOVED***= '[object RegExp]') {
      return val;
    }
...
SchemaString.prototype.castForQuery = function($conditional, val, context) {
  ...
  if (Object.prototype.toString.call(val) ***REMOVED***= '[object RegExp]' || isBsonType(val, 'BSONRegExp')) {
    return val;
  }
...
SchemaString.prototype.toJSONSchema = function toJSONSchema(options) {
  const isRequired = this.options.required && typeof this.options.required !***REMOVED*** 'function';
