...
module.exports = function trackTransaction(schema) {
  schema.pre('save', function trackTransactionPreSave() {
    ...
    if (session.transaction ***REMOVED*** null || session[sessionNewDocuments] ***REMOVED*** null) {
      return;
    }
...
function _getAtomics(doc, previous) {
  ...
  for (const path of pathsToCheck) {
    ...
    if (val != null &&
        ...
        Object.keys(val[arrayAtomicsSymbol]).length !***REMOVED*** 0) {
  ...
  for (const dirt of dirty) {
    ...
    if (val != null && val[arrayAtomicsSymbol] != null && Object.keys(val[arrayAtomicsSymbol]).length !***REMOVED*** 0) {
      const existing = previous.get(path) || {};
      pathToAtomics.set(path, mergeAtomics(existing, val[arrayAtomicsSymbol]));
    }
  }
