...
Mongoose.prototype.setDriver = function setDriver(driver) {
  ...
  if (_mongoose.__driver ***REMOVED***= driver) {
    return _mongoose;
  }

  const openConnection = _mongoose.connections && _mongoose.connections.find(conn => conn.readyState !***REMOVED*** STATES.disconnected);
  if (openConnection) {
  ...
  if (Array.isArray(driver.plugins)) {
    for (const plugin of driver.plugins) {
      if (typeof plugin ***REMOVED***= 'function') {
  ...
  if (oldDefaultConnection ***REMOVED*** null) {
    return _mongoose;
  }
  ...
  for (const model of Object.values(_mongoose.models)) {
    if (model.db !***REMOVED*** oldDefaultConnection) {
      continue;
    }
...
Mongoose.prototype.set = function getsetOptions(key, value) {
  ...
  if (arguments.length ***REMOVED***= 1 && typeof key !***REMOVED*** 'object') {
    if (VALID_OPTIONS.indexOf(key) ***REMOVED***= -1) {
  ...
  for (const [optionKey, optionValue] of Object.entries(options)) {
    if (VALID_OPTIONS.indexOf(optionKey) ***REMOVED***= -1) {
      if (!error) {
    ...
    if (optionKey ***REMOVED***= 'objectIdGetter') {
      if (optionValue) {
    ...
    } else if (optionKey ***REMOVED***= 'transactionAsyncLocalStorage') {
    ...
    } else if (optionKey ***REMOVED***= 'createInitialConnection') {
      if (optionValue && !_mongoose.connection) {
        _createDefaultConnection(_mongoose);
      } else if (optionValue ***REMOVED***= false && _mongoose.connection && _mongoose.connection[defaultConnectionSymbol]) {
        if (_mongoose.connection.readyState ***REMOVED***= STATES.disconnected && Object.keys(_mongoose.connection.models).length ***REMOVED***= 0) {
...
Mongoose.prototype.connect = async function connect(uri, options) {
  ...
  const _mongoose = this instanceof Mongoose ? this : mongoose;
  if (_mongoose.connection ***REMOVED*** null) {
    _createDefaultConnection(_mongoose);
  }
 ...
 *     conn.model('Actor') ***REMOVED***= Actor; // true
 *     conn.model('Actor', schema) ***REMOVED***= Actor; // true, same schema
 *     conn.model('Actor', schema, 'actors') ***REMOVED***= Actor; // true, same schema and collection name
 *
...
Mongoose.prototype.model = function model(name, schema, collection, options) {
  ...
  if (_mongoose.models.hasOwnProperty(name) && options.cache !***REMOVED*** false && overwriteModels !***REMOVED*** true) {
    if (originalSchema &&
        originalSchema.instanceOfSchema &&
        originalSchema !***REMOVED*** _mongoose.models[name].schema) {
      throw new _mongoose.Error.OverwriteModelError(name);
    }
    if (collection && collection !***REMOVED*** _mongoose.models[name].collection.name) {
...
Mongoose.prototype._model = function _model(name, schema, collection, options) {
  ...
  if (schema ***REMOVED*** null || !('pluralization' in schema.options)) {
...
Mongoose.prototype._applyPlugins = function _applyPlugins(schema, options) {
  ...
  options.applyPluginsToChildSchemas = typeof (_mongoose.options && _mongoose.options.applyPluginsToChildSchemas) ***REMOVED***= 'boolean' ?
 ...
 *     mongoose.connections[0] ***REMOVED***= mongoose.connection; // true
 *
...
Object.defineProperty(Mongoose.prototype, 'Connection', {
  ...
  set: function(Connection) {
    if (Connection ***REMOVED***= this.__driver.Connection) {
      return;
    }
...
Mongoose.prototype.isObjectIdOrHexString = function isObjectIdOrHexString(v) {
  return isBsonType(v, 'ObjectId') || (typeof v ***REMOVED***= 'string' && objectIdHexRegexp.test(v));
};
