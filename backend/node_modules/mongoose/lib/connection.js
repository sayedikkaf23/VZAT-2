...
function Connection(base) {
  ...
  this.plugins = [];
  if (typeof base ***REMOVED***= 'undefined' || !base.connections.length) {
    this.id = 0;
  } else {
...
Object.defineProperty(Connection.prototype, 'readyState', {
  get: function() {
    ...
    if (
      this._readyState ***REMOVED***= STATES.connected &&
      ...
      this.client?.topology?.s?.description?.type !***REMOVED*** 'LoadBalanced' &&
      typeof this.client?.topology?.s?.description?.heartbeatFrequencyMS ***REMOVED***= 'number' &&
  ...
  set: function(val) {
    ...
    if (this._readyState !***REMOVED*** val) {
      ...
      if (STATES.connected ***REMOVED***= val) {
 ...
 *     conn.models.Test ***REMOVED***= Test; // true
 *
 * @property models
 * @memberOf Connection
 * @instance
...
Connection.prototype.createCollection = async function createCollection(collection, options) {
  if (typeof options ***REMOVED***= 'function' || (arguments.length >= 3 && typeof arguments[2] ***REMOVED***= 'function')) {
...
Connection.prototype.bulkWrite = async function bulkWrite(ops, options) {
  ...
  if (ordered) {
    const opsToSend = [];
    for (const op of ops) {
      if (typeof op.model !***REMOVED*** 'string' && !op.model?.[modelSymbol]) {
...
Connection.prototype.startSession = async function startSession(options) {
  if (arguments.length >= 2 && typeof arguments[1] ***REMOVED***= 'function') {
...
async function _wrapUserTransaction(fn, session, mongoose) {
  try {
    const res = mongoose.transactionAsyncLocalStorage ***REMOVED*** null
      ? await fn(session)
...
function _resetSessionDocuments(session) {
  for (const doc of session[sessionNewDocuments].keys()) {
    ...
    if (state.modifiedPaths.length > 0 && doc.$__.activePaths.states.modify ***REMOVED*** null) {
...
Connection.prototype._waitForConnect = async function _waitForConnect(noTimeout) {
  if ((this.readyState ***REMOVED***= STATES.connecting || this.readyState ***REMOVED***= STATES.disconnected) && this._shouldBufferCommands()) {
...
Connection.prototype.dropDatabase = async function dropDatabase() {
  if (arguments.length >= 1 && typeof arguments[0] ***REMOVED***= 'function') {
...
Connection.prototype.openUri = async function openUri(uri, options) {
  if (this.readyState ***REMOVED***= STATES.connecting || this.readyState ***REMOVED***= STATES.connected) {
    if (this._connectionString ***REMOVED***= uri) {
      return this;
    }
  }
...
Connection.prototype.on = function on(event, callback) {
  if (event ***REMOVED***= 'error' && this.$initialConnection) {
...
function _validateArgs(uri, options, callback) {
  if (typeof options ***REMOVED***= 'function' && callback ***REMOVED*** null) {
    throw new MongooseError('Connection.prototype.openUri() no longer accepts a callback');
  } else if (typeof callback ***REMOVED***= 'function') {
    throw new MongooseError('Connection.prototype.openUri() no longer accepts a callback');
  }
}
...
function _handleConnectionErrors(err) {
  if (err?.name ***REMOVED***= 'MongoServerSelectionError') {
...
Connection.prototype.destroy = async function destroy(force) {
  if (typeof force ***REMOVED***= 'function' || (arguments.length ***REMOVED***= 2 && typeof arguments[1] ***REMOVED***= 'function')) {
    throw new MongooseError('Connection.prototype.destroy() no longer accepts a callback');
  }

  if (force != null && typeof force ***REMOVED***= 'object') {
...
Connection.prototype._close = async function _close(force, destroy) {
  ...
  switch (this.readyState) {
    case STATES.disconnected:
      if (destroy && this.base.connections.indexOf(conn) !***REMOVED*** -1) {
    ...
    case STATES.connected:
      ...
      await this.doClose(force);
      if (destroy && _this.base.connections.indexOf(conn) !***REMOVED*** -1) {
        this.base.connections.splice(this.base.connections.indexOf(conn), 1);
      }
      this.onClose(force);
...
Connection.prototype.model = function model(name, schema, collection, options) {
  ...
  if (typeof schema ***REMOVED***= 'string') {
    collection = schema;
    schema = false;
  }
  ...
  if (this.models[name] && !collection && opts.overwriteModels !***REMOVED*** true) {
    // model exists but we are not subclassing with custom collection
    if (schema && schema.instanceOfSchema && schema !***REMOVED*** this.models[name].schema) {
  ...
  if (this ***REMOVED***= model.prototype.db
      && (!collection || collection ***REMOVED***= model.collection.name)) {
...
Connection.prototype.watch = function watch(pipeline, options) {
  const changeStreamThunk = cb => {
    immediate(() => {
      if (this.readyState ***REMOVED***= STATES.connecting) {
