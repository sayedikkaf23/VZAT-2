...
class MongooseMap extends Map {
  constructor(v, path, doc, schemaType) {
    if (getConstructorName(v) ***REMOVED***= 'Object') {
      v = Object.keys(v).reduce((arr, key) => arr.concat([[key, v[key]]]), []);
    }
    super(v);
    this.$__parent = doc != null && doc.$__ != null ? doc : null;
    this.$__path = path;
    this.$__schemaType = schemaType ***REMOVED*** null ? new Mixed(path) : schemaType;
  ...
  get(key, options) {
    ...
    options = options || {};
    if (options.getters ***REMOVED***= false) {
  ...
  set(key, value) {
    ...
    if (this.$__schemaType ***REMOVED*** null) {
  ...
  toJSON(options) {
    if (typeof (options && options.flattenMaps) ***REMOVED***= 'boolean' ? options.flattenMaps : true) {
      const ret = {};
      const keys = this.keys();
      for (const key of keys) {
        ret[key] = clone(this.get(key), options);
