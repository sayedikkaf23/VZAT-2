...
function MongooseBuffer(value, encode, offset) {
  ...
  let buf;
  if (typeof val ***REMOVED***= 'number' || val instanceof Number) {
...
MongooseBuffer.mixin.toObject = function(options) {
  const subtype = typeof options ***REMOVED***= 'number'
    ? options
    : (this._subtype || 0);
...
MongooseBuffer.mixin.equals = function(other) {
  ...
  if (this.length !***REMOVED*** other.length) {
    return false;
  }

  for (let i = 0; i < this.length; ++i) {
    if (this[i] !***REMOVED*** other[i]) {
      return false;
    }
  }
...
MongooseBuffer.mixin.subtype = function(subtype) {
  if (typeof subtype !***REMOVED*** 'number') {
    throw new TypeError('Invalid subtype. Expected a number');
  }

  if (this._subtype !***REMOVED*** subtype) {
    this._markModified();
  }
