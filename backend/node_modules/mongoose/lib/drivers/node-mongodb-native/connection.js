...
NativeConnection.prototype.useDb = function(name, options) {
  ...
  if (this.db && this._readyState ***REMOVED***= STATES.connected) {
    wireup();
  } else {
  ...
  if (options.noListener !***REMOVED*** true) {
...
NativeConnection.prototype.removeDb = function removeDb(name) {
  const dbs = this.otherDbs.filter(db => db.name ***REMOVED***= name);
  if (!dbs.length) {
  ...
  this.otherDbs = this.otherDbs.filter(db => db.name !***REMOVED*** name);
};
...
NativeConnection.prototype.createClient = async function createClient(uri, options) {
  ...
  if (options) {
    ...
    const autoIndex = options.config && options.config.autoIndex != null ?
      ...
      options.autoIndex;
    if (autoIndex != null) {
      this.config.autoIndex = autoIndex !***REMOVED*** false;
...
NativeConnection.prototype._buildEncryptionSchemas = function() {
  ...
  for (const model of encryptedModels) {
    ...
    const mappings = schema.encryptionType() ***REMOVED***= 'csfle' ? csfleMappings : qeMappings;
...
NativeConnection.prototype.setClient = function setClient(client) {
  ...
  if (this.readyState !***REMOVED*** STATES.disconnected) {
    throw new MongooseError('Cannot call `setClient()` on a connection that is already connected.');
  }
  if (client.topology ***REMOVED*** null) {
...
function _setClient(conn, client, options, dbName) {
  ...
  const _handleReconnect = () => {
    ...
    // this event.
    if (conn.readyState !***REMOVED*** STATES.connected) {
  ...
  if (type ***REMOVED***= 'Single') {
    client.on('serverDescriptionChanged', ev => {
      const newDescription = ev.newDescription;
      if (newDescription.type ***REMOVED***= 'Unknown') {
    ...
    client.on('topologyDescriptionChanged', ev => {
      ...
      if (conn.readyState ***REMOVED***= STATES.connected && description.type !***REMOVED*** 'ReplicaSetWithPrimary') {
        // Implicitly emits 'disconnected'
        conn.readyState = STATES.disconnected;
      } else if (conn.readyState ***REMOVED***= STATES.disconnected && description.type ***REMOVED***= 'ReplicaSetWithPrimary') {
        _handleReconnect();
      }
    });
  }
