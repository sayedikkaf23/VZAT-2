...
function iter(i) {
  NativeCollection.prototype[i] = function() {
    ...
    const debug = connectionDebug ***REMOVED*** null ? globalDebug : connectionDebug;
    ...
    if (this.conn.$wasForceClosed) {
      ...
      if (args.length > 0 &&
        typeof args[args.length - 1] ***REMOVED***= 'function') {
    ...
    if (this._shouldBufferCommands() && this.buffer) {
      ...
      let promise = null;
      let timeout = null;
      if (syncCollectionMethods[i] && typeof lastArg ***REMOVED***= 'function') {
      ...
      } else if (typeof lastArg ***REMOVED***= 'function') {
      ...
      return promise;
    } else if (!syncCollectionMethods[i] && typeof lastArg ***REMOVED***= 'function') {
      callback = function collectionOperationCallback(err, res) {
        if (err != null) {
    ...
    if (debug) {
      if (typeof debug ***REMOVED***= 'function') {
        let argsToAdd = null;
        ...
        const color = debug.color ***REMOVED*** null ? true : debug.color;
        const shell = debug.shell ***REMOVED*** null ? false : debug.shell;
    ...
    try {
      ...
      if (ret != null && typeof ret.then ***REMOVED***= 'function') {
        return ret.then(
          result => {
            if (typeof lastArg ***REMOVED***= 'function') {
              lastArg(null, result);
            } else {
              this.conn.emit('operation-end', { _id: opId, modelName: _this.modelName, collectionName: this.name, method: i, result });
            }
...
for (const key of Object.getOwnPropertyNames(Collection.prototype)) {
  ...
  if (descriptor.get !***REMOVED*** undefined) {
    continue;
  }
  if (typeof Collection.prototype[key] !***REMOVED*** 'function') {
    continue;
  }
...
NativeCollection.prototype.$format = function(arg, color, shell) {
  ...
  if (type ***REMOVED***= 'function' || type ***REMOVED***= 'undefined') return '';
...
function format(obj, sub, color, shell) {
  if (obj && typeof obj.toBSON ***REMOVED***= 'function') {
    obj = obj.toBSON();
  }
  if (obj ***REMOVED*** null) {
    return obj;
  ...
  if (constructorName ***REMOVED***= 'Binary') {
    x = 'BinData(' + x.sub_type + ', "' + x.toString('base64') + '")';
  } else if (constructorName ***REMOVED***= 'ObjectId') {
    x = inspectable('ObjectId("' + x.toHexString() + '")');
  } else if (constructorName ***REMOVED***= 'Date') {
    x = inspectable('new Date("' + x.toUTCString() + '")');
  } else if (constructorName ***REMOVED***= 'Object') {
    ...
    for (let i = 0; i < numKeys; ++i) {
      ...
      if (x[key]) {
        let error;
        if (typeof x[key].toBSON ***REMOVED***= 'function') {
          try {
        ...
        if (_constructorName ***REMOVED***= 'Binary') {
          x[key] = 'BinData(' + x[key].sub_type + ', "' +
            x[key].buffer.toString('base64') + '")';
        } else if (_constructorName ***REMOVED***= 'Object') {
          x[key] = format(x[key], true);
        } else if (_constructorName ***REMOVED***= 'ObjectId') {
          formatObjectId(x, key);
        } else if (_constructorName ***REMOVED***= 'Date') {
          formatDate(x, key, shell);
        } else if (_constructorName ***REMOVED***= 'ClientSession') {
