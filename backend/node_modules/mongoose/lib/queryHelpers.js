...
exports.preparePopulationOptionsMQ = function preparePopulationOptionsMQ(query, options) {
  ...
  if (options.lean != null) {
    pop
      .filter(p => (p && p.options && p.options.lean) ***REMOVED*** null)
...
exports.applyPaths = function applyPaths(fields, schema, sanitizeProjection) {
  ...
  if (fields) {
    ...
    for (let keyIndex = 0; keyIndex < keys.length; ++keyIndex) {
      ...
      if (keys[keyIndex] ***REMOVED***= '_id' && keys.length > 1) {
        continue;
      }
      if (keys[keyIndex] ***REMOVED***= schema.options.discriminatorKey && keys.length > 1 && field != null && !field) {
        continue;
      }
      exclude = !field;
      break;
    ...
    for (const path of minusPaths) {
      ...
      if ((!type || !type.selected) || exclude !***REMOVED*** false) {
        fields[path] = 0;
        exclude = true;
      } else if (type && type.selected && exclude ***REMOVED***= false) {
  ...
  switch (exclude) {
    ...
    case false:
      if (schema &&
          ...
          schema.paths['_id'].options.select ***REMOVED***= false) {
        fields._id = 0;
      }
  ...
  function analyzeSchema(schema, prefix) {
    ...
    // avoid recursion
    if (stack.indexOf(schema) !***REMOVED*** -1) {
      return [];
    }
    stack.push(schema);
    ...
    schema.eachPath(function(path, type) {
      ...
      if (type.$isSchemaMap || path.endsWith('.$*')) {
        ...
        if (type.options && type.options.select ***REMOVED***= false && !hasPlusPath) {
          excluded.push(path);
        }
        return;
      }
      ...
      // arrays
      if (addedPath ***REMOVED*** null && !Array.isArray(type) && type.$isMongooseArray && !type.$isMongooseDocumentArray) {
  ...
  function analyzePath(path, type) {
    ...
    if (typeof type.selected !***REMOVED*** 'boolean') {
      return;
    }

    // User overwriting default exclusion
    if (type.selected ***REMOVED***= false && fields[path]) {
    ...
    if (!exclude && type.selected && path ***REMOVED***= schema.options.discriminatorKey && fields[path] != null && !fields[path]) {
      delete fields[path];
      return;
    }

    if (exclude ***REMOVED***= false && type.selected && fields[path] != null && !fields[path]) {
      delete fields[path];
      return;
    }
    ...
    if (hasPlusPath) {
      ...
      if (exclude ***REMOVED***= false && keys.length > 1 && !~keys.indexOf(path) && !sanitizeProjection) {
        fields[path] = 1;
      } else if (exclude ***REMOVED*** null && sanitizeProjection && type.selected ***REMOVED***= false) {
        fields[path] = 0;
      }
    ...
    for (let i = 0; i < pieces.length; ++i) {
      ...
      if (excluded.indexOf(cur) !***REMOVED*** -1) {
        return;
      }
    }
    ...
    if (!exclude && (type && type.options && type.options.$skipDiscriminatorCheck || false)) {
      let cur = '';
      for (let i = 0; i < pieces.length; ++i) {
        cur += (cur.length ***REMOVED***= 0 ? '' : '.') + pieces[i];
        const projection = get(fields, cur, false) || get(fields, cur + '.$', false);
        if (projection && typeof projection !***REMOVED*** 'object') {
          return;
        }
      }
    }
  ...
  return function(option) {
    ...
    if (val != null && Array.isArray(val.virtuals)) {
      ...
      val.virtuals = val.virtuals.
        filter(path => typeof path ***REMOVED***= 'string' && path.startsWith(option.path + '.')).
