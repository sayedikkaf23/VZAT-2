...
function aliasFields(schema, paths) {
  for (const path of Object.keys(paths)) {
    ...
    if (Array.isArray(alias)) {
      for (const a of alias) {
        ...
        schema.
          ...
          get((function(p) {
            return function() {
              if (typeof this.get ***REMOVED***= 'function') {
                return this.get(p);
              }
              return this[p];
            };
    ...
    if (typeof alias !***REMOVED*** 'string') {
...
Schema.prototype._clone = function _clone(Constructor) {
  Constructor = Constructor || (this.base ***REMOVED*** null ? Schema : this.base.Schema);
...
Schema.prototype.defaultOptions = function(options) {
  this._userProvidedOptions = options ***REMOVED*** null ? {} : clone(options);
  ...
  if (options.versionKey && typeof options.versionKey !***REMOVED*** 'string') {
    throw new MongooseError('`versionKey` must be falsy or string, got `' + (typeof options.versionKey) + '`');
  }

  if (typeof options.read ***REMOVED***= 'string') {
    options.read = handleReadPreferenceAliases(options.read);
  } else if (Array.isArray(options.read) && typeof options.read[0] ***REMOVED***= 'string') {
    options.read = {
...
Schema.prototype.encryptionType = function encryptionType(encryptionType) {
  ...
  if (!(typeof encryptionType ***REMOVED***= 'string' || encryptionType ***REMOVED***= null)) {
...
Schema.prototype.add = function add(obj, prefix) {
  ...
  if (obj._id ***REMOVED***= false && prefix ***REMOVED*** null) {
    this.options._id = false;
  }

  prefix = prefix || '';
  // avoid prototype pollution
  if (prefix ***REMOVED***= '__proto__.' || prefix ***REMOVED***= 'constructor.' || prefix ***REMOVED***= 'prototype.') {
    return this;
  }
  ...
  for (const key of keys) {
    ...
    if (key ***REMOVED***= '_id' && val ***REMOVED***= false) {
      continue;
    }
    ...
    if (
      key !***REMOVED*** '_id' &&
      ((typeof val !***REMOVED*** 'object' && typeof val !***REMOVED*** 'function' && !isMongooseTypeString) ||
      val ***REMOVED*** null)
    ) {
    ...
    if (val instanceof VirtualType || (val.constructor && val.constructor.name || null) ***REMOVED***= 'VirtualType') {
      this.virtual(val);
      continue;
    }

    if (Array.isArray(val) && val.length ***REMOVED***= 1 && val[0] ***REMOVED*** null) {
    ...
    } else if (!val[typeKey] || (typeKey ***REMOVED***= 'type' && isPOJO(val.type) && val.type.type)) {
    ...
    } else if (typeof val ***REMOVED***= 'object' && 'encrypt' in val) {
      // schema.add({ field: { type: <schema type>, encrypt: { ... }}})
      const { encrypt } = val;

      if (this.encryptionType() ***REMOVED*** null) {
...
Schema.prototype._buildSchemaMap = function() {
  ...
  function buildNestedPath(path, object, value) {
    ...
    for (; i < path.length - 1; ++i, component = path[i]) {
      object[component] = object[component] ***REMOVED*** null ? {
        bsonType: 'object',
        properties: {}
      } : object[component];
...
Schema.prototype.removeIndex = function removeIndex(index) {
  ...
  if (typeof index !***REMOVED*** 'object' && typeof index !***REMOVED*** 'string') {
    throw new Error('removeIndex() may only take either an object or a string as an argument');
  }

  if (typeof index ***REMOVED***= 'object') {
    ...
    for (let i = this._indexes.length - 1; i >= 0; --i) {
      if (this._indexes[i][1] != null && this._indexes[i][1].name ***REMOVED***= index) {
        this._indexes.splice(i, 1);
      }
    }
  }
...
Schema.prototype.path = function(path, obj) {
  ...
  if (typeof obj ***REMOVED***= 'object' && utils.hasUserDefinedProperty(obj, 'ref')) {
  ...
  for (const sub of subpaths) {
    ...
    if (typeof branch[sub] !***REMOVED*** 'object') {
  ...
  this.childSchemas = this.childSchemas.filter(childSchema => childSchema.path !***REMOVED*** path);
  ...
  if (schemaType.$isMongooseDocumentArray) {
    for (const key of Object.keys(schemaType.schema.paths)) {
      ...
      if (typeof _schemaType ***REMOVED***= 'object' && _schemaType != null && _schemaType.$parentSchemaDocArray ***REMOVED*** null) {
...
function _getPath(schema, path, cleanPath) {
  ...
  if (schema.subpaths.hasOwnProperty(cleanPath)) {
    ...
    if (subpath ***REMOVED***= 'nested') {
      return undefined;
    }
    return subpath;
  }
  if (schema.singleNestedPaths.hasOwnProperty(cleanPath) && typeof schema.singleNestedPaths[cleanPath] ***REMOVED***= 'object') {
    const singleNestedPath = schema.singleNestedPaths[cleanPath];
    if (singleNestedPath ***REMOVED***= 'nested') {
      return undefined;
    }
...
function getMapPath(schema, path) {
  if (schema.mapPaths.length ***REMOVED***= 0) {
    return null;
  }
  for (const val of schema.mapPaths) {
    const cleanPath = val.path.replace(/\.\$\*/g, '');
    if (path ***REMOVED***= cleanPath || (path.startsWith(cleanPath + '.') && path.slice(cleanPath.length + 1).indexOf('.') ***REMOVED***= -1)) {
      return val;
      ...
      if (remnant.indexOf('.') ***REMOVED***= -1 && presplitPath[presplitPath.length - 1] ***REMOVED***= '$*') {
        // Handle map of map of primitives
        return val.$__schemaType;
      } else if (remnant.indexOf('.') !***REMOVED*** -1 && val.$__schemaType.schema && presplitPath[presplitPath.length - 1] ***REMOVED***= '$*') {
...
Schema.prototype.interpretAsType = function(path, obj, options) {
  ...
  if (!utils.isPOJO(obj) && !(obj instanceof SchemaTypeOptions)) {
    ...
    if (constructorName !***REMOVED*** 'Object') {
      const oldObj = obj;
      obj = {};
  ...
  let type = obj[options.typeKey] && (obj[options.typeKey] instanceof Function || options.typeKey !***REMOVED*** 'type' || !obj.type.type)
    ? obj[options.typeKey]
    : {};

  if (type instanceof SchemaType) {
    if (type.path ***REMOVED***= path) {
      return type;
    }
  ...
  if (utils.isPOJO(type) || type ***REMOVED***= 'mixed') {
    return new MongooseTypes.Mixed(path, obj);
  }

  if (Array.isArray(type) || type ***REMOVED***= Array || type ***REMOVED***= 'array' || type ***REMOVED***= MongooseTypes.Array) {
    // if it was specified through { type } look for `cast`
    let cast = (type ***REMOVED***= Array || type ***REMOVED***= 'array')
      ? obj.cast || obj.of
      : type[0];
    ...
    if (typeof cast !***REMOVED*** 'undefined') {
      if (Array.isArray(cast) || cast.type ***REMOVED***= Array || cast.type ***REMOVED*** 'Array') {
        if (cast && cast.type ***REMOVED*** 'Array') {
          cast.type = Array;
        }
    ...
    const castFromTypeKey = (cast != null && cast[options.typeKey] && (options.typeKey !***REMOVED*** 'type' || !cast.type.type)) ?
      cast[options.typeKey] :
      cast;
    if (typeof cast ***REMOVED***= 'string') {
    ...
    if (cast) {
      type = cast[options.typeKey] && (options.typeKey !***REMOVED*** 'type' || !cast.type.type)
        ? cast[options.typeKey]
        : cast;
      ...
      name = typeof type ***REMOVED***= 'string'
        ? type
        : type.schemaName || utils.getFunctionName(type);

      // For Jest 26+, see #10296
      if (name ***REMOVED***= 'ClockDate') {
        name = 'Date';
      }

      if (name ***REMOVED***= void 0) {
  ...
  if (Buffer.isBuffer(type)) {
    name = 'Buffer';
  } else if (typeof type ***REMOVED***= 'function' || typeof type ***REMOVED***= 'object') {
    name = type.schemaName || utils.getFunctionName(type);
  } else if (type ***REMOVED***= Types.ObjectId) {
    name = 'ObjectId';
  } else if (type ***REMOVED***= Types.Decimal128) {
    name = 'Decimal128';
  } else {
    name = type ***REMOVED*** null ? '' + type : type.toString();
  }
  ...
  if (MongooseTypes[name] ***REMOVED*** null) {
  ...
  if (utils.hasUserDefinedProperty(obj, 'of')) {
    ...
    if (_mapType[schema.options.typeKey] && _mapType[schema.options.typeKey].instanceOfSchema) {
      ...
      subdocumentSchema.eachPath((subpath, type) => {
        if (type.options.select ***REMOVED***= true || type.options.select ***REMOVED***= false) {
...
Schema.prototype.pathType = function(path) {
  ...
  if (singleNestedPath) {
    return singleNestedPath ***REMOVED***= 'nested' ? 'nested' : 'real';
  }
...
function getPositionalPathType(self, path, cleanPath) {
  ...
  for (let i = 1; i < subpaths.length; ++i) {
    ...
    if (i ***REMOVED***= last && val && !/\D/.test(subpath)) {
    ...
    if (!/\D/.test(subpath)) {
      // Nested array
      if (val instanceof MongooseTypes.Array && i !***REMOVED*** last) {
        val = val.caster;
      }
      continue;
    }
 ...
 *       if (this.name !***REMOVED*** 'Woody') this.name = 'Woody';
 *       next();
 *     });
 *
 ...
 * @param {Boolean} [opts.deduplicate=false] If true, ignore duplicate plugins (same `fn` argument using `***REMOVED***=`)
 * @see plugins https://mongoosejs.com/docs/plugins.html
 * @api public
 */

Schema.prototype.plugin = function(fn, opts) {
  if (typeof fn !***REMOVED*** 'function') {
...
Schema.prototype.index = function(fields, options) {
  ...
  for (const field of Object.keys(fields)) {
    if (fields[field] ***REMOVED***= 'ascending' || fields[field] ***REMOVED***= 'asc') {
      fields[field] = 1;
    } else if (fields[field] ***REMOVED***= 'descending' || fields[field] ***REMOVED***= 'desc') {
      fields[field] = -1;
    }
  }

  for (const existingIndex of this.indexes()) {
    if (options.name ***REMOVED*** null && existingIndex[1].name ***REMOVED*** null && isIndexSpecEqual(existingIndex[0], fields)) {
 ...
 * @param {Array<string>} [tags] tags to add to read preference if key ***REMOVED***= 'read'
...
Schema.prototype.set = function(key, value, tags) {
  ...
  switch (key) {
    case 'read':
      ...
      } else if (Array.isArray(value) && typeof value[0] ***REMOVED***= 'string') {
        this.options[key] = {
 ...
 *         if (index[1].unique ***REMOVED***= undefined) {
 *           index[1].unique = true;
 *         }
 *       }
 *     }
...
Schema.prototype.virtual = function(name, options) {
  if (name instanceof VirtualType || getConstructorName(name) ***REMOVED***= 'VirtualType') {
    return this.virtual(name.path, name.options);
  }
  options = new VirtualOptions(options);

  if (utils.hasUserDefinedProperty(options, ['ref', 'refPath'])) {
    if (options.localField ***REMOVED*** null) {
      throw new Error('Reference virtuals require `localField` option');
    }

    if (options.foreignField ***REMOVED*** null) {
    ...
    this.pre('init', function virtualPreInit(obj, opts) {
      if (mpath.has(name, obj)) {
        ...
        if (opts?.hydratedPopulatedDocs && !options.count) {
          ...
          if (!Array.isArray(populatedVal) && !populatedVal.$__ && modelNames?.length ***REMOVED***= 1) {
            const PopulateModel = this.db.model(modelNames[0]);
            this.$$populatedVirtuals[name] = PopulateModel.hydrate(populatedVal);
          } else if (Array.isArray(populatedVal) && modelNames?.length ***REMOVED***= 1) {
            const PopulateModel = this.db.model(modelNames[0]);
            ...
            this.$populated(
              name,
              populatedVal.map(doc => doc ***REMOVED*** null ? doc : doc.get(typeof foreignField ***REMOVED***= 'function' ? foreignField.call(doc, doc) : foreignField)),
    ...
    if (typeof options.get ***REMOVED***= 'function') {
    ...
    for (let i = 0; i < parts.length - 1; ++i) {
      if (this.paths[cur] ***REMOVED*** null) {
        continue;
      }
  ...
  if (this.pathType(name) ***REMOVED***= 'real') {
    throw new Error('Virtual path "' + name + '"' +
      ' conflicts with a real path in the schema');
  }

  virtuals[name] = parts.reduce(function(mem, part, i) {
    mem[part] || (mem[part] = (i ***REMOVED***= parts.length - 1)
...
Schema.prototype.remove = function(path) {
  ...
  if (Array.isArray(path)) {
    path.forEach(function(name) {
      if (this.path(name) ***REMOVED*** null && !this.nested[name]) {
        return;
      }
      if (this.nested[name]) {
...
Schema.prototype.removeVirtual = function(path) {
  ...
  if (Array.isArray(path)) {
    for (const virtual of path) {
      if (this.virtuals[virtual] ***REMOVED*** null) {
...
Schema.prototype.loadClass = function(model, virtualsOnly) {
  ...
  if (model ***REMOVED***= Object.prototype ||
      model ***REMOVED***= Function.prototype ||
  ...
  Object.getOwnPropertyNames(model.prototype).forEach(function(name) {
    ...
    if (!virtualsOnly) {
      if (typeof method.value ***REMOVED***= 'function') {
        this.method(name, method.value);
      }
    }
    if (typeof method.get ***REMOVED***= 'function') {
      if (this.virtuals[name]) {
        this.virtuals[name].getters = [];
      }
      this.virtual(name).get(method.get);
    }
    if (typeof method.set ***REMOVED***= 'function') {
      if (this.virtuals[name]) {
...
Schema.prototype._getSchema = function(path) {
    ...
    while (p--) {
      ...
      if (foundschema) {
        ...
        if (foundschema.caster) {
          ...
          // a path like array.$
          if (p !***REMOVED*** parts.length) {
            if (p + 1 ***REMOVED***= parts.length && foundschema.$embeddedSchemaType && (parts[p] ***REMOVED***= '$' || isArrayFilter(parts[p]))) {
              return foundschema.$embeddedSchemaType;
            }

            if (foundschema.schema) {
              let ret;
              if (parts[p] ***REMOVED***= '$' || isArrayFilter(parts[p])) {
                if (p + 1 ***REMOVED***= parts.length) {
                  // comments.$
                  return foundschema.$embeddedSchemaType;
                }
  ...
  for (let i = 0; i < parts.length; ++i) {
    if (parts[i] ***REMOVED***= '$' || isArrayFilter(parts[i])) {
    ...
    while (p--) {
      ...
      if (foundschema) {
        if (foundschema.caster) {
          ...
          // If there is no foundschema.schema we are dealing with
          // a path like array.$
          if (p !***REMOVED*** parts.length && foundschema.schema) {
            if (parts[p] ***REMOVED***= '$' || isArrayFilter(parts[p])) {
              if (p ***REMOVED***= parts.length - 1) {
      ...
      } else if (p ***REMOVED***= parts.length && schema.nested[trypath]) {
...
Schema.prototype._transformDuplicateKeyError = function _transformDuplicateKeyError(error) {
  ...
  if (error.code !***REMOVED*** 11000 && error.code !***REMOVED*** 11001) {
    return error;
  }
...
Schema.prototype.toJSONSchema = function toJSONSchema(options) {
  ...
  for (const path of Object.keys(this.paths)) {
    ...
    if (schemaType._presplitPath.indexOf('$*') !***REMOVED*** -1) {
      continue;
    }
    ...
    if (isNested) {
      for (let i = 0; i < schemaType._presplitPath.length - 1; ++i) {
        ...
        if (jsonSchemaForPath.properties[subpath] ***REMOVED*** null) {
    ...
    if (path ***REMOVED***= '_id') {
      ...
      isRequired = true;
    } else if (schemaType.options.required && typeof schemaType.options.required !***REMOVED*** 'function') {
      if (!jsonSchemaForPath.required) {
        jsonSchemaForPath.required = [];
      }
  ...
  if (result.required.length ***REMOVED***= 0) {
