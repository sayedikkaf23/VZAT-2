...
module.exports = function(query, schema, castedDoc, options, callback) {
  ...
  for (i = 0; i < numKeys; ++i) {
    if (keys[i].startsWith('$')) {
      ...
      if (keys[i] ***REMOVED***= '$push' || keys[i] ***REMOVED***= '$addToSet') {
      ...
      for (let j = 0; j < numPaths; ++j) {
        ...
        if (key ***REMOVED***= '$set' || key ***REMOVED***= '$setOnInsert' ||
            key ***REMOVED***= '$pull' || key ***REMOVED***= '$pullAll') {
          updatedValues[updatedPath] = flat[paths[j]];
          isPull[updatedPath] = key ***REMOVED***= '$pull' || key ***REMOVED***= '$pullAll';
        } else if (key ***REMOVED***= '$unset') {
  ...
  function iter(i, v) {
    ...
    if (schemaPath ***REMOVED*** null) {
      return;
    }
    if (schemaPath.instance ***REMOVED***= 'Mixed' && schemaPath.path !***REMOVED*** updates[i]) {
      return;
    }
        ...
        validatorsToExecute.push(function(callback) {
          ...
          schemaPath.doValidate(v, function(err) {
            if (schemaPath.schema != null &&
                schemaPath.schema.options.storeSubdocValidationError ***REMOVED***= false &&
    ...
    if (schemaPath && schemaPath.$isMongooseDocumentArray) {
      validatorsToExecute.push(function(callback) {
        schemaPath.doValidate(
          ...
          options && options.context ***REMOVED***= 'query' ? query : null);
      });
    } else {
