...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  const virtual = _virtualRes ***REMOVED*** null ? null : _virtualRes.virtual;
  if (virtual != null) {
  ...
  const isStrictPopulateDisabled = options.strictPopulate ***REMOVED***= false || options.options?.strictPopulate ***REMOVED***= false;
  if (!isStrictPopulateDisabled && allSchemaTypes.length ***REMOVED***= 0 && options._localModel != null) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (doc.$__ != null && doc.populated(options.path)) {
      ...
      if (forceRepopulate ***REMOVED***= false) {
        continue;
      }
    }
    ...
    if (schema != null &&
        ...
        schema.options.ref ***REMOVED*** null &&
        schema.options.refPath ***REMOVED*** null) {
      continue;
    }
    ...
    if (schema != null && schema.instance ***REMOVED***= 'Embedded') {
    ...
    if (Array.isArray(schema)) {
      ...
      for (const _schema of schemasArray) {
        ...
        for (const modelName of _modelNames) {
          if (modelNames.indexOf(modelName) ***REMOVED***= -1) {
    ...
    if ('justOne' in options && options.justOne !***REMOVED*** void 0) {
      justOne = options.justOne;
    } else if (schema && !schema[schemaMixedSymbol]) {
      // Skip Mixed types because we explicitly don't do casting on those.
      if (options.path.endsWith('.' + schema.path) || options.path ***REMOVED***= schema.path) {
    ...
    const hasMatchFunction = typeof match ***REMOVED***= 'function';
    if (hasMatchFunction) {
  ...
  function _getModelNames(doc, schema, modelNameFromQuery, model) {
    ...
    if (schema && schema.instance ***REMOVED***= 'Array') {
    ...
    if (modelNameFromQuery) {
      ...
      if (typeof refPath ***REMOVED***= 'function') {
      ...
      if ((ref = get(schemaForCurrentDoc, 'options.ref')) != null) {
        if (schemaForCurrentDoc != null &&
          ...
          if (subdocsBeingPopulated.length ***REMOVED***= 0) {
    ...
    if (!modelNames) {
      ...
      if (options._localModel ***REMOVED*** null) {
...
function _virtualPopulate(model, docs, options, _virtualRes) {
  ...
  for (const doc of docs) {
    ...
    if (typeof options.localField ***REMOVED***= 'string') {
      localField = options.localField;
    } else if (typeof virtual.options.localField ***REMOVED***= 'function') {
    ...
    if (typeof localField ***REMOVED***= 'function') {
      localField = localField.call(doc, doc);
    }
    if (typeof foreignField ***REMOVED***= 'function') {
    ...
    let hasMatchFunction = typeof match ***REMOVED***= 'function';
    if (hasMatchFunction) {
      match = match.call(doc, doc, data.virtual);
    }

    if (Array.isArray(localField) && Array.isArray(foreignField) && localField.length ***REMOVED***= foreignField.length) {
...
function addModelNamesToMap(model, map, available, modelNames, options, data, ret, doc, schemaOptions, unpopulatedValue) {
  ...
  unpopulatedValue = unpopulatedValue ***REMOVED***= void 0 ? ret : unpopulatedValue;
  if (Array.isArray(unpopulatedValue)) {
    unpopulatedValue = utils.cloneArrays(unpopulatedValue);
  }

  if (modelNames ***REMOVED*** null) {
    return;
  }
  ...
  while (k--) {
    ...
    if (data.isRefPath && Array.isArray(ret) && ret.length ***REMOVED***= modelNamesForRefPath.length) {
      ids = matchIdsToRefPaths(ret, modelNamesForRefPath, modelName);
    }

    const perDocumentLimit = options.perDocumentLimit ***REMOVED*** null ?
...
function _getModelFromConn(conn, modelName) {
  ...
  if (conn.models[modelName] ***REMOVED*** null && conn._parent != null) {
...
function matchIdsToRefPaths(ids, refPaths, refPathToFind) {
  if (!Array.isArray(refPaths)) {
    return refPaths ***REMOVED***= refPathToFind
      ? Array.isArray(ids)
...
function handleRefFunction(ref, doc) {
  if (typeof ref ***REMOVED***= 'function' && !ref[modelSymbol]) {
...
function _getLocalFieldValues(doc, localField, model, options, virtual, schema) {
  ...
  const localFieldPath = localFieldPathType ***REMOVED***= 'real' ?
  ...
  localField = localFieldPath != null && localFieldPath.instance ***REMOVED***= 'Embedded' ? localField + '._id' : localField;
  ...
  if (localFieldGetters.length !***REMOVED*** 0 && getters) {
...
function convertTo_id(val, schema) {
  ...
  if (val != null && val._id != null && (schema ***REMOVED*** null || !schema.$isSchemaMap)) {
    return val._id;
  }
  ...
  // been hydrated yet
  if (getConstructorName(val) ***REMOVED***= 'Object' &&
      // The intent here is we should only flatten the object if we expect
      // to get a Map in the end. Avoid doing this for mixed types.
      (schema ***REMOVED*** null || schema[schemaMixedSymbol] ***REMOVED*** null)) {
    const ret = [];
...
function _findRefPathForDiscriminators(doc, modelSchema, data, options, normalizedRefPath, ret) {
  ...
  // populated path.
  if (!data.isRefPath || normalizedRefPath ***REMOVED*** null) {
    return;
  }
  ...
  for (let i = 0; i < pieces.length; ++i) {
    ...
    cur = cur + (cur.length ***REMOVED***= 0 ? '' : '.') + piece;
    const schematype = modelSchema.path(cur);
    if (schematype != null &&
        schematype.$isMongooseArray &&
        schematype.caster.discriminators != null &&
        Object.keys(schematype.caster.discriminators).length !***REMOVED*** 0) {
      ...
      for (const subdoc of subdocs) {
        ...
        if (discriminatorSchema ***REMOVED*** null) {
          continue;
        }
        const _path = discriminatorSchema.path(remnant);
        if (_path ***REMOVED*** null || _path.options.refPath ***REMOVED*** null) {
          const docValue = utils.getValue(data.localField.substring(cur.length + 1), subdoc);
          ret.forEach((v, i) => {
            if (v ***REMOVED***= docValue) {
...
function throwOn$where(match) {
  ...
  if (typeof match !***REMOVED*** 'object') {
    return;
  }
  for (const key of Object.keys(match)) {
    if (key ***REMOVED***= '$where') {
      throw new MongooseError('Cannot use $where filter with populate() match');
    }
    if (match[key] != null && typeof match[key] ***REMOVED***= 'object') {
