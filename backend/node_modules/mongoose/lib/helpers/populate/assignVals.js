...
module.exports = function assignVals(o) {
  ...
  function setValue(val) {
    ...
    if (o.justOne ***REMOVED***= true && Array.isArray(val)) {
      // Might be an embedded discriminator (re: gh-9244) with multiple models, so make sure to pick the right
      // model before assigning.
      const ret = [];
      for (const doc of val) {
        const _docPopulatedModel = leanPopulateMap.get(doc);
        if (_docPopulatedModel ***REMOVED*** null || _docPopulatedModel ***REMOVED***= populatedModel) {
          ret.push(doc);
        }
      }
    ...
    } else if (o.justOne ***REMOVED***= false && !Array.isArray(val)) {
      return valueFilter([val], options, populateOptions, _allIds);
    } else if (o.justOne ***REMOVED***= true && !Array.isArray(val) && Array.isArray(_allIds)) {
      return valueFilter(val, options, populateOptions, val ***REMOVED*** null ? val : _allIds[setValueIndex - 1]);
    }
    return valueFilter(val, options, populateOptions, _allIds);
  }
  ...
  for (i = 0; i < docs.length; ++i) {
    ...
    if (existingVal ***REMOVED*** null && !getVirtual(o.originalModel.schema, _path)) {
      continue;
    }
    ...
    if (count) {
      ...
      valueToSet = Array.isArray(rawIds[i]) ?
        rawIds[i].filter(v => v ***REMOVED*** null || sift(o.match[i])(v)) :
        [rawIds[i]].filter(v => v ***REMOVED*** null || sift(o.match[i])(v))[0];
    } else {
      valueToSet = rawIds[i];
    }
    ...
    if (o.isVirtual && isDoc) {
      ...
      if (Array.isArray(valueToSet)) {
        valueToSet = valueToSet.map(v => v ***REMOVED*** null ? void 0 : v);
      }
      mpath.set(
        _path,
        valueToSet,
        ...
        function lookup(obj, part, val) {
          ...
          if (obj instanceof Map && part ***REMOVED***= '$*') {
            return [...obj.values()];
          }
          return obj[part];
        },
    ...
    for (let j = 0; j < parts.length - 1; ++j) {
      ...
      if (cur[parts[j]] ***REMOVED*** null) {
        ...
        if (valueToSet ***REMOVED*** null && schematype != null && schematype.$isMongooseArray) {
          break;
        }
        cur[parts[j]] = {};
      }
...
function numDocs(v) {
  if (Array.isArray(v)) {
    ...
    if (v.some(el => Array.isArray(el) || el ***REMOVED***= null)) {
      return v.map(el => {
        if (el ***REMOVED*** null) {
          return 0;
        }
...
function valueFilter(val, assignmentOpts, populateOptions, allIds) {
  const userSpecifiedTransform = typeof populateOptions.transform ***REMOVED***= 'function';
  ...
  if (Array.isArray(val)) {
    ...
    for (let i = 0; i < numValues; ++i) {
      ...
      if (!isPopulatedObject(subdoc) && (!populateOptions.retainNullValues || subdoc != null) && !userSpecifiedTransform) {
        continue;
      } else if (!populateOptions.retainNullValues && subdoc ***REMOVED*** null) {
        continue;
  ...
  if (populateOptions.justOne ***REMOVED***= false) {
    return [];
  }

  return val ***REMOVED*** null ? transform(val, allIds) : transform(null, allIds);
}
...
function maybeRemoveId(subdoc, assignmentOpts) {
  if (subdoc != null && assignmentOpts.excludeId) {
    if (typeof subdoc.$__setValue ***REMOVED***= 'function') {
