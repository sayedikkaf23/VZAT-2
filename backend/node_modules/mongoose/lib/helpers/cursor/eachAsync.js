...
module.exports = async function eachAsync(next, fn, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (batchSize != null) {
      if (typeof batchSize !***REMOVED*** 'number') {
    ...
    function createFetch() {
      ...
      function fetch(done) {
        ...
        next(function(err, doc) {
          ...
          if (err != null) {
            if (err.name ***REMOVED***= 'MongoCursorExhaustedError') {
          ...
          if (batchSize && documentsBatch.length !***REMOVED*** batchSize) {
  ...
  function handleNextResult(doc, i, callback) {
    ...
    if (maybePromise && typeof maybePromise.then ***REMOVED***= 'function') {
      maybePromise.then(
...
function asyncQueue() {
  ...
  return function enqueue(fn) {
    if (
      inProgress ***REMOVED***= null &&
      _queue.length ***REMOVED***= 0
    ) {
      inProgress = id++;
      return fn(_step);
    }
...
function createEachAsyncMultiError(aggregatedErrors) {
  if (aggregatedErrors.length ***REMOVED***= 0) {
    return null;
  }
