...
module.exports = function(filter, schema, castedDoc, options) {
  ...
  if (!options.upsert || shouldSetDefaultsOnInsert ***REMOVED***= false) {
    return castedDoc;
  }
  ...
  for (let i = 0; i < numPaths; ++i) {
    ...
    if (condition && typeof condition ***REMOVED***= 'object') {
  ...
  schema.eachPath(function(path, schemaType) {
    ...
    if (schemaType.path ***REMOVED***= '_id' && schemaType.options.auto) {
      return;
    }
    ...
    if (typeof def ***REMOVED***= 'undefined') {
      return;
    }
    ...
    if (get(castedDoc, path) ***REMOVED*** null) {
...
function isModified(modified, path) {
  ...
  if (modifiedKeys.length) {
    ...
    for (const modifiedPath of modifiedKeys) {
      if (modifiedPath.slice(0, path.length + 1) ***REMOVED***= parentPath) {
        return true;
      }
    }
  }
