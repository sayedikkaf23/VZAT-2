...
function applyVirtualsToChildren(schema, res, virtuals) {
  ...
  for (const childSchema of schema.childSchemas) {
    ...
    if (_obj ***REMOVED*** null || (Array.isArray(_obj) && _obj.flat(Infinity).length ***REMOVED***= 0)) {
      continue;
    }
    ...
    if (Array.isArray(virtuals)) {
      virtualsForChild = [];
      for (const virtual of virtuals) {
        if (virtual[0] ***REMOVED*** _path) {
          virtualsForChild.push(virtual.slice(1));
        }
      }

      if (virtualsForChild.length ***REMOVED***= 0) {
        continue;
      }
    }
...
function applyVirtualsToDoc(schema, obj, virtuals) {
  ...
  for (const virtual of virtuals) {
    ...
    if (isPopulateVirtual && val ***REMOVED***= undefined) {
