...
module.exports = function getEmbeddedDiscriminatorPath(doc, path, options) {
  ...
  const parts = Array.isArray(path) ?
    path :
    (path.indexOf('.') ***REMOVED***= -1 ? [path] : path.split('.'));
  let schemaType = null;
  ...
  for (let i = 0; i < parts.length; ++i) {
    ...
    if (schemaType.instance ***REMOVED***= 'Mixed') {
    ...
    schemaType.schema.discriminators != null) {
      ...
      if (discriminatorKey ***REMOVED*** null || discriminators[discriminatorKey] ***REMOVED*** null) {
        continue;
      }
