...
function validateDiscriminatorSchemasForEncryption(parentSchema, childSchema) {
  if (parentSchema.encryptionType() ***REMOVED*** null && childSchema.encryptionType() ***REMOVED*** null) return;
...
module.exports = function discriminator(model, name, schema, tiedValue, applyPlugins, mergeHooks, overwriteExisting) {
  ...
  mergeHooks = mergeHooks ***REMOVED*** null ? true : mergeHooks;
  ...
  if (schema.path(key) && schema.path(key).options.$skipDiscriminatorCheck !***REMOVED*** true) {
    throw new Error('Discriminator "' + name +
        '" cannot have field with name "' + key + '"');
  }

  let value = name;
  if ((typeof tiedValue ***REMOVED***= 'string' && tiedValue.length) || tiedValue != null) {
    value = tiedValue;
  }
  ...
  function merge(schema, baseSchema) {
    ...
    obj[key] = {
      ...
      set: function(newName) {
        if (newName ***REMOVED***= value || (Array.isArray(value) && utils.deepEqual(newName, value))) {
          return value;
        }
    ...
    if (typeof _id !***REMOVED*** 'undefined') {
