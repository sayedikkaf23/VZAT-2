...
function applyHooks(model, schema, options) {
  ...
  const middleware = schema.s.hooks.
    filter(hook => {
      if (hook.name ***REMOVED***= 'updateOne' || hook.name ***REMOVED***= 'deleteOne') {
        return !!hook['document'];
      }
      if (hook.name ***REMOVED***= 'remove' || hook.name ***REMOVED***= 'init') {
        return hook['document'] ***REMOVED*** null || !!hook['document'];
      }
      if (hook.query != null || hook.document != null) {
        return hook.document !***REMOVED*** false;
      }
      return true;
    }).
    filter(hook => {
  ...
  for (const method of internalMethodsToWrap) {
    const toWrap = method ***REMOVED***= 'validate' ? '$__originalValidate' : `$__${method}`;
  ...
  for (const method of customMethods) {
    ...
    objToDecorate[method] = function() {
      ...
      const argsWithoutCallback = typeof cb ***REMOVED***= 'function' ?
