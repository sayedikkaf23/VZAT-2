...
function applyTimestampsToChildren(now, update, schema) {
  ...
  const hasDollarKey = keys.some(key => key[0] ***REMOVED***= '$');
  ...
  const updateKeys = Object.keys(update).filter(key => key[0] !***REMOVED*** '$');
...
function applyTimestampsToUpdateKey(schema, key, update, now) {
  ...
  if (Array.isArray(update[key]) && path.$isMongooseDocumentArray) {
    ...
    for (const item of parentSchemaTypes) {
      ...
      if (!timestamps || updatedAt ***REMOVED*** null) {
        continue;
      }
      ...
      if (parentSchemaType.$isSingleNested) {
        ...
        childPath = firstDot !***REMOVED*** -1 ? childPath.substring(0, firstDot) : childPath;
