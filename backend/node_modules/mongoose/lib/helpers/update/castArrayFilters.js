...
function _castArrayFilters(arrayFilters, schema, strictQuery, updatedPathsByFilter, query) {
  ...
  for (const filter of arrayFilters) {
    ...
    if (keys.length ***REMOVED***= 0) {
      continue;
    }

    const firstKey = keys[0];
    if (firstKey ***REMOVED***= '$and' || firstKey ***REMOVED***= '$or') {
    ...
    const filterWildcardPath = dot ***REMOVED***= -1 ? firstKey : firstKey.substring(0, dot);
    if (updatedPathsByFilter[filterWildcardPath] ***REMOVED*** null) {
      continue;
    }
    ...
    for (const key of keys) {
      if (updatedPathsByFilter[key] ***REMOVED***= null) {
        continue;
      }
      if (Object.keys(updatedPathsByFilter).length ***REMOVED***= 0) {
        continue;
      }
      const dot = key.indexOf('.');

      let filterPathRelativeToBase = dot ***REMOVED***= -1 ? null : key.substring(dot);
      let schematype;
      if (filterPathRelativeToBase ***REMOVED*** null || filterBaseSchema ***REMOVED*** null) {
      ...
      if (schematype ***REMOVED*** null) {
        ...
        const filterPath = filterPathRelativeToBase ***REMOVED*** null ?
          baseFilterPath + '.0' :
      ...
      if (typeof filter[key] ***REMOVED***= 'object') {
