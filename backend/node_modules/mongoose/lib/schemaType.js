...
function SchemaType(path, options, instance) {
  ...
  if (options.select ***REMOVED*** null) {
  ...
  for (const prop of keys) {
    ...
    if (utils.hasUserDefinedProperty(this.options, prop) && typeof this[prop] ***REMOVED***= 'function') {
      // { unique: true, index: true }
      if (prop ***REMOVED***= 'index' && this._index) {
        if (options.index ***REMOVED***= false) {
          const index = this._index;
          if (typeof index ***REMOVED***= 'object' && index != null) {
            if (index.unique) {
...
SchemaType.prototype.splitPath = function() {
  ...
  if (this.path ***REMOVED*** null) {
    return undefined;
  }

  this._presplitPath = this.path.indexOf('.') ***REMOVED***= -1 ? [this.path] : this.path.split('.');
  return this._presplitPath;
};
 ...
 *       assert.ok(v ***REMOVED***= undefined || typeof v ***REMOVED***= 'number');
 *       return v;
 *     });
 *
...
SchemaType.prototype.castFunction = function castFunction(caster, message) {
  ...
  if (typeof caster ***REMOVED***= 'string') {
...
SchemaType.prototype.unique = function unique(value, message) {
  ...
  if (!this.options.hasOwnProperty('index') && value ***REMOVED***= false) {
    return this;
  }

  if (this._index ***REMOVED*** null || this._index ***REMOVED***= true) {
    this._index = {};
  } else if (typeof this._index ***REMOVED***= 'string') {
    this._index = { type: this._index };
  }

  this._index.unique = !!value;
  if (typeof message ***REMOVED***= 'string') {
...
SchemaType.prototype.text = function(bool) {
  ...
  if (!this.options.hasOwnProperty('index') && bool ***REMOVED***= false) {
    return this;
  }

  if (this._index ***REMOVED***= null || this._index ***REMOVED***= undefined ||
    typeof this._index ***REMOVED***= 'boolean') {
    this._index = {};
  } else if (typeof this._index ***REMOVED***= 'string') {
    this._index = { type: this._index };
  }
...
SchemaType.prototype.sparse = function(bool) {
  ...
  if (this._index ***REMOVED*** null || typeof this._index ***REMOVED***= 'boolean') {
    this._index = {};
  } else if (typeof this._index ***REMOVED***= 'string') {
    this._index = { type: this._index };
  }
 ...
 *       if (typeof val !***REMOVED*** 'string') val = '';
...
SchemaType.prototype.validate = function(obj, message, type) {
  if (typeof obj ***REMOVED***= 'function' || obj && utils.getFunctionName(obj.constructor) ***REMOVED***= 'RegExp') {
    let properties;
    if (typeof message ***REMOVED***= 'function') {
...
SchemaType.prototype.required = function(required, message) {
  ...
  if (arguments.length > 0 && required ***REMOVED*** null) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !***REMOVED*** this.requiredValidator;
    }, this);
  ...
  if (typeof required ***REMOVED***= 'object') {
    customOptions = required;
    message = customOptions.message || message;
    required = required.isRequired;
  }

  if (required ***REMOVED***= false) {
    this.validators = this.validators.filter(function(v) {
      return v.validator !***REMOVED*** this.requiredValidator;
    }, this);
  ...
  this.requiredValidator = function(v) {
    ...
    if (cachedRequired != null && _this.path in cachedRequired) {
      ...
      return res;
    } else if (typeof required ***REMOVED***= 'function') {
  ...
  if (typeof required ***REMOVED***= 'string') {
    message = required;
    required = undefined;
  }
...
SchemaType.prototype.getDefault = function(scope, init, options) {
  let ret;
  if (typeof this.defaultValue ***REMOVED***= 'function') {
    if (
      this.defaultValue ***REMOVED***= Date.now ||
      this.defaultValue ***REMOVED***= Array ||
      this.defaultValue.name.toLowerCase() ***REMOVED***= 'objectid'
    ) {
  ...
  if (ret !***REMOVED*** null && ret !***REMOVED*** undefined) {
    if (typeof ret ***REMOVED***= 'object' && (!this.options || !this.options.shared)) {
      ret = clone(ret);
    }
...
SchemaType.prototype.doValidate = function(value, fn, scope, options) {
  ...
  const validators = this.validators.
    filter(v => typeof v ***REMOVED***= 'object' && v !***REMOVED*** null);
  ...
  for (let i = 0, len = validators.length; i < len; ++i) {
    ...
    if (typeof validator !***REMOVED*** 'function') {
      continue;
    }

    if (value ***REMOVED***= undefined && validator !***REMOVED*** this.requiredValidator) {
    ...
    if (ok != null && typeof ok.then ***REMOVED***= 'function') {
      ok.then(
...
SchemaType.prototype.doValidateSync = function(value, scope, options) {
  ...
  if (value ***REMOVED***= void 0) {
    if (this.validators.length !***REMOVED*** 0 && this.validators[0].type ***REMOVED***= 'required') {
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (v ***REMOVED***= null || typeof v !***REMOVED*** 'object') {
      continue;
    }
...
SchemaType._isRef = function(self, value, doc, init) {
  ...
  if (ref) {
    ...
    if (!Buffer.isBuffer(value) && // buffers are objects too
      value._bsontype !***REMOVED*** 'Binary' // raw binary value from the db
...
function handle$in(val, context) {
  ...
  return val.map(function(m) {
    if (Array.isArray(m) && m.length ***REMOVED***= 0) {
      return m;
    }
    return _this.castForQuery(null, m, context);
  });
...
SchemaType.prototype.castForQuery = function($conditional, val, context) {
  ...
  try {
    ...
    if (err instanceof CastError && err.path ***REMOVED***= this.path && this.$fullPath != null) {
 ...
 *     mongoose.Schema.Types.String.checkRequired(v => typeof v ***REMOVED***= 'string');
 *
...
SchemaType.prototype.clone = function() {
  ...
  if (this.requiredValidator !***REMOVED*** undefined) schematype.requiredValidator = this.requiredValidator;
  if (this.defaultValue !***REMOVED*** undefined) schematype.defaultValue = this.defaultValue;
  if (this.$immutable !***REMOVED*** undefined && this.options.immutable ***REMOVED***= undefined) {
    schematype.$immutable = this.$immutable;

    handleImmutable(schematype);
  }
  if (this._index !***REMOVED*** undefined) schematype._index = this._index;
  if (this.selected !***REMOVED*** undefined) schematype.selected = this.selected;
  if (this.isRequired !***REMOVED*** undefined) schematype.isRequired = this.isRequired;
  if (this.originalRequiredValue !***REMOVED*** undefined) schematype.originalRequiredValue = this.originalRequiredValue;
