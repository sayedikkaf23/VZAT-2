...
module.exports = function cast(schema, obj, options, context) {
  ...
  while (i--) {
    ...
    if (path ***REMOVED***= '$or' || path ***REMOVED***= '$nor' || path ***REMOVED***= '$and') {
      if (!Array.isArray(val)) {
        throw new CastError('Array', val, path);
      }
      for (let k = val.length - 1; k >= 0; k--) {
        if (val[k] ***REMOVED*** null || typeof val[k] !***REMOVED*** 'object') {
          throw new CastError('Object', val[k], path + '.' + k);
        }
        const beforeCastKeysLength = Object.keys(val[k]).length;
        const discriminatorValue = val[k][schema.options.discriminatorKey];
        if (discriminatorValue ***REMOVED*** null) {
        ...
        if (Object.keys(val[k]).length ***REMOVED***= 0 && beforeCastKeysLength !***REMOVED*** 0) {
          val.splice(k, 1);
        }
      }
    ...
    } else if (path ***REMOVED***= '$where') {
      type = typeof val;

      if (type !***REMOVED*** 'string' && type !***REMOVED*** 'function') {
        throw new Error('Must have a string or function for $where');
      }

      if (type ***REMOVED***= 'function') {
        obj[path] = val.toString();
      }

      continue;
    } else if (path ***REMOVED***= '$expr') {
      val = cast$expr(val, schema);
      continue;
    } else if (path ***REMOVED***= '$elemMatch') {
      val = cast(schema, val, options, context);
    } else if (path ***REMOVED***= '$text') {
      val = castTextSearch(val, path);
    } else if (path ***REMOVED***= '$comment' && !schema.paths.hasOwnProperty('$comment')) {
      ...
      if (!schematype) {
        ...
        while (j--) {
          ...
          if (_schematype != null &&
            ...
            pathLastHalf !***REMOVED*** discriminatorKey) {
            const discriminatorVal = get(obj, pathFirstHalf + '.' + discriminatorKey);
            const discriminators = _schematype.schema.discriminators;
            if (typeof discriminatorVal ***REMOVED***= 'string' && discriminators[discriminatorVal] != null) {

              schematype = discriminators[discriminatorVal].path(pathLastHalf);
            } else if (discriminatorVal != null &&
              Object.keys(discriminatorVal).length ***REMOVED***= 1 &&
              Array.isArray(discriminatorVal.$in) &&
              discriminatorVal.$in.length ***REMOVED***= 1 &&
              typeof discriminatorVal.$in[0] ***REMOVED***= 'string' &&
        ...
        if (isObject(val)) {
          ...
          if (geo) {
            ...
            if (geo ***REMOVED***= '$within') {
              ...
              value = withinType;
            } else if (geo ***REMOVED***= '$near' &&
                typeof value.type ***REMOVED***= 'string' && Array.isArray(value.coordinates)) {
              // geojson; cast the coordinates
              value = value.coordinates;
            } else if ((geo ***REMOVED***= '$near' || geo ***REMOVED***= '$nearSphere' || geo ***REMOVED***= '$geoIntersects') &&
                value.$geometry && typeof value.$geometry.type ***REMOVED***= 'string' &&
            ...
            } else if (geo ***REMOVED***= '$geoWithin') {
              if (value.$geometry) {
                if (isMongooseObject(value.$geometry)) {
                  value.$geometry = value.$geometry.toObject({ virtuals: false });
                }
                const geoWithinType = value.$geometry.type;
                if (ALLOWED_GEOWITHIN_GEOJSON_TYPES.indexOf(geoWithinType) ***REMOVED***= -1) {
      ...
      } else if (Array.isArray(val) && ['Buffer', 'Array'].indexOf(schematype.instance) ***REMOVED***= -1 && !options.sanitizeFilter) {
        const casted = [];
