...
function _init(model, c, agg) {
  if (!model.collection.buffer) {
    model.hooks.execPre('aggregate', agg, function(err) {
      ...
      if (typeof agg.options?.cursor?.transform ***REMOVED***= 'function') {
...
AggregationCursor.prototype._transformForAsyncIterator = function() {
  if (this._transforms.indexOf(_transformForAsyncIterator) ***REMOVED***= -1) {
...
function _transformForAsyncIterator(doc) {
  return doc ***REMOVED*** null ? { done: true } : { value: doc, done: false };
}
...
function _next(ctx, cb) {
  ...
  if (ctx._transforms.length) {
    callback = function(err, doc) {
      if (err || (doc ***REMOVED***= null && !ctx._mongooseOptions.transformNull)) {
        return cb(err, doc);
      }
