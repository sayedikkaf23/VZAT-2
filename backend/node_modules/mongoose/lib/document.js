...
function Document(obj, fields, skipId, options) {
  if (typeof skipId ***REMOVED***= 'object' && skipId != null) {
    options = skipId;
  ...
  if (options.isNew != null && options.isNew !***REMOVED*** true) {
  ...
  if (obj != null && typeof obj !***REMOVED*** 'object') {
    throw new ObjectParameterError(obj, 'obj', 'Document');
  }

  let defaults = true;
  if (options.defaults !***REMOVED*** undefined) {
  ...
  if (typeof fields ***REMOVED***= 'boolean' || fields ***REMOVED***= 'throw') {
    if (fields !***REMOVED*** true) {
      this.$__.strictMode = fields;
    }
    fields = undefined;
  } else if (schema.options.strict !***REMOVED*** true) {
  ...
  const hasIncludedChildren = exclude ***REMOVED***= false && fields ?
    $__hasIncludedChildren(fields) :
    null;

  if (this._doc ***REMOVED*** null) {
...
Object.defineProperty(Document.prototype, '$locals', {
  ...
  enumerable: false,
  get: function() {
    if (this.$__.locals ***REMOVED*** null) {
      this.$__.locals = {};
    }
...
function $applyDefaultsToNested(val, path, doc) {
  ...
  const pathPieces = path.indexOf('.') ***REMOVED***= -1 ? [path] : path.split('.');
  ...
  for (let i = 0; i < plen; ++i) {
    ...
    if (type.defaultValue ***REMOVED***= void 0) {
      continue;
    }
    ...
    for (let j = 0; j < len; ++j) {
      ...
      if (j ***REMOVED***= len - 1) {
        if (cur[piece] !***REMOVED*** void 0) {
          break;
        }

        try {
          const def = type.getDefault(doc, false);
          if (def !***REMOVED*** void 0) {
            cur[piece] = def;
          }
        } catch (err) {
...
Document.prototype.$__buildDoc = function(obj, fields, skipId, exclude, hasIncludedChildren) {
  ...
  for (; ii < plen; ++ii) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (exclude ***REMOVED***= true) {
        if (curPath in fields) {
          break;
        }
      } else if (exclude ***REMOVED***= false && fields && !included) {
        if (curPath in fields) {
          included = true;
...
Document.prototype.init = function(doc, opts, fn) {
  if (typeof opts ***REMOVED***= 'function') {
    fn = opts;
    opts = null;
  }
...
Document.prototype.$__init = function(doc, opts) {
  ...
  if (doc._id != null && opts.populated && opts.populated.length) {
    ...
    for (const item of opts.populated) {
      ...
      if (item._childDocs ***REMOVED*** null) {
        continue;
      }
      for (const child of item._childDocs) {
        if (child ***REMOVED*** null || child.$__ ***REMOVED*** null) {
          continue;
        }
  ...
  const hasIncludedChildren = this.$__.exclude ***REMOVED***= false && this.$__.selected ?
...
function init(self, obj, doc, opts, prefix) {
  ...
  for (let index = 0; index < len; ++index) {
    ...
    if (!schemaType && utils.isPOJO(value)) {
      ...
      if (doc.hasOwnProperty(i) && value !***REMOVED*** void 0 && !opts.hydratedPopulatedDocs) {
        delete doc[i];
      }
      if (value ***REMOVED***= null) {
        doc[i] = schemaType._castNullish(null);
      } else if (value !***REMOVED*** undefined) {
        const wasPopulated = value.$__ ***REMOVED*** null ? null : value.$__.wasPopulated;
 ...
 *     doc.$session() ***REMOVED***= session; // true
 *     doc.$session(null);
 *     doc.$session() ***REMOVED***= null; // true
 *
...
Document.prototype.$session = function $session(session) {
  ...
  if (session ***REMOVED*** null && this.$__.session ***REMOVED*** null) {
    return;
  }
...
Document.prototype.overwrite = function overwrite(obj) {
  ...
  for (const key of keys) {
    ...
    if (this.$__schema.options.versionKey && key ***REMOVED***= this.$__schema.options.versionKey) {
      continue;
    }
    if (this.$__schema.options.discriminatorKey && key ***REMOVED***= this.$__schema.options.discriminatorKey) {
      continue;
    }
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  const adhoc = type && type !***REMOVED*** true;
  const constructing = type ***REMOVED***= true;
  let adhocs;
  let keys;
  let i = 0;
  let pathtype;
  ...
  if (path ***REMOVED*** null) {
    [path, val] = [val, path];
  } else if (typeof path !***REMOVED*** 'string') {
    ...
    if (path instanceof Document) {
      if (path.$__isNested) {
        ...
        path = path.$__schema ***REMOVED***= this.$__schema
    ...
    if (len ***REMOVED***= 0 && _skipMinimizeTopLevel) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (type ***REMOVED***= true &&
          !prefix &&
          valForKey != null &&
          pathtype ***REMOVED***= 'nested' &&
          this._doc[key] != null) {
        delete this._doc[key];
      }

      if (utils.isNonBuiltinObject(valForKey) && pathtype ***REMOVED***= 'nested') {
        ...
        if (constructing && valForKey ***REMOVED***= void 0 &&
            this.$get(pathName) !***REMOVED*** void 0) {
          continue;
        }

        if (pathtype ***REMOVED***= 'adhocOrUndefined') {
          pathtype = getEmbeddedDiscriminatorPath(this, pathName, { typeOnly: true });
        }

        if (pathtype ***REMOVED***= 'real' || pathtype ***REMOVED***= 'virtual') {
          this.$set(pathName, valForKey, constructing, options);
        } else if (pathtype ***REMOVED***= 'nested' && valForKey instanceof Document) {
          this.$set(pathName,
            valForKey.toObject({ transform: false }), constructing, options);
        } else if (strict ***REMOVED***= 'throw') {
          if (pathtype ***REMOVED***= 'nested') {
            throw new ObjectExpectedError(key, valForKey);
          } else {
            throw new StrictModeError(key);
          }
        } else if (pathtype ***REMOVED***= 'nested' && valForKey ***REMOVED*** null) {
          this.$set(pathName, valForKey, constructing, options);
        }
      } else {
        this.$set(pathName, valForKey, constructing, options);
  ...
  if (pathType ***REMOVED***= 'nested' && val) {
    if (typeof val ***REMOVED***= 'object' && val != null) {
      if (val.$__ != null) {
  ...
  if (typeof this.$__schema.aliases[parts[0]] ***REMOVED***= 'string') {
    parts[0] = this.$__schema.aliases[parts[0]];
  }

  if (pathType ***REMOVED***= 'adhocOrUndefined' && strict) {
    ...
    for (i = 0; i < parts.length; ++i) {
      ...
      if (i + 1 < parts.length && this.$__schema.pathType(subpath) ***REMOVED***= 'virtual') {
        mpath.set(path, val, this);
        return this;
      }

      schema = this.$__schema.path(subpath);
      if (schema ***REMOVED*** null) {
        continue;
      }
    ...
    if (!mixed && !schema) {
      if (strict ***REMOVED***= 'throw') {
        throw new StrictModeError(path);
      }
      return this;
    }
  } else if (pathType ***REMOVED***= 'virtual') {
  ...
  for (i = 0; i < parts.length - 1; ++i) {
    ...
    curPath += (curPath.length !***REMOVED*** 0 ? '.' : '') + parts[i];
    if (!cur) {
      this.$set(curPath, {});
  ...
  if (parts.length <= 1) {
    ...
    for (i = 0; i < len; ++i) {
      const subpath = parts.slice(0, i + 1).join('.');
      if (this.$get(subpath, null, { getters: false }) ***REMOVED***= null) {
        pathToMark = subpath;
        break;
      }
    }
  ...
  if (!schema) {
    ...
    if (pathType ***REMOVED***= 'nested' && val ***REMOVED*** null) {
      cleanModifiedSubpaths(this, path);
    }
    return this;
  }
  ...
  try {
    ...
    const refMatches = (() => {
      if (schema.options ***REMOVED*** null) {
        return false;
      }
      if (!(val instanceof Document)) {
        return false;
      ...
      // Check ref
      const refOpt = typeof schema.options.ref ***REMOVED***= 'function' && !schema.options.ref[modelSymbol] ? schema.options.ref.call(this, this) : schema.options.ref;

      const ref = refOpt?.modelName || refOpt;
      if (ref != null && (ref ***REMOVED***= model.modelName || ref ***REMOVED***= model.baseModelName)) {
        return true;
      }

      // Check refPath
      const refPath = schema.options.refPath;
      if (refPath ***REMOVED*** null) {
        return false;
      }
      const modelName = val.get(refPath);
      return modelName ***REMOVED***= model.modelName || modelName ***REMOVED***= model.baseModelName;
    })();
  ...
  if (schema.$isSingleNested && (this.isDirectModified(path) || val ***REMOVED*** null)) {
    cleanModifiedSubpaths(this, path);
  }
...
Document.prototype.$__shouldModify = function(pathToMark, path, options, constructing, parts, schema, val, priorVal) {
  ...
  if (val ***REMOVED***= void 0 && !this.$__isSelected(path)) {
    // when a path is not selected in a query, its initial
    // value will be undefined.
    return true;
  }

  if (val ***REMOVED***= void 0 && path in this.$__.activePaths.getStatePaths('default')) {
  ...
  if (!deepEqual(val, priorVal !***REMOVED*** undefined ? priorVal : utils.getValue(path, this))) {
    return true;
  }

  if (!constructing &&
      val !***REMOVED*** null &&
      val !***REMOVED*** undefined &&
...
Document.prototype.$__set = function(pathToMark, path, options, constructing, parts, schema, val, priorVal) {
  ...
  if (shouldModify) {
    if (this.$__.primitiveAtomics && this.$__.primitiveAtomics[path]) {
      ...
      if (Object.keys(this.$__.primitiveAtomics).length ***REMOVED***= 0) {
...
Document.prototype.$__getValue = function(path) {
  if (typeof path !***REMOVED*** 'string' && !Array.isArray(path)) {
    throw new TypeError(
...
Document.prototype.$inc = function $inc(path, val) {
  ...
  if (schemaType ***REMOVED*** null) {
    if (this.$__.strictMode ***REMOVED***= 'throw') {
      throw new StrictModeError(path);
    } else if (this.$__.strictMode ***REMOVED***= true) {
      return this;
    }
  } else if (schemaType.instance !***REMOVED*** 'Number') {
  ...
  if (shouldSet) {
    ...
    if (this.$__.primitiveAtomics[path] ***REMOVED*** null) {
...
Document.prototype.get = function(path, type, options) {
  ...
  if (noDottedPath) {
    ...
    if (schema != null && options.getters !***REMOVED*** false) {
      return schema.applyGetters(obj, this);
    }
    return obj;
  }

  if (schema != null && schema.instance ***REMOVED***= 'Mixed') {
  ...
  const hasDot = path.indexOf('.') !***REMOVED*** -1;
  let obj = this._doc;

  const pieces = hasDot ? path.split('.') : [path];
  // Might need to change path for top-level alias
  if (typeof this.$__schema.aliases[pieces[0]] ***REMOVED***= 'string') {
...
Document.prototype.$__saveInitialState = function $__saveInitialState(path) {
  ...
  if (savedState != null) {
    ...
    const topLevelPath = firstDot ***REMOVED***= -1 ? savedStatePath : savedStatePath.slice(0, firstDot);
...
Document.prototype.$isEmpty = function(path) {
  ...
  if (arguments.length !***REMOVED*** 0) {
    ...
    if (typeof v !***REMOVED*** 'object') {
      return false;
    }
    if (utils.isPOJO(v)) {
      return _isEmpty(v);
    }
    return Object.keys(v.toObject(isEmptyOptions)).length ***REMOVED***= 0;
  }

  return Object.keys(this.toObject(isEmptyOptions)).length ***REMOVED***= 0;
};
...
function _isEmpty(v) {
  ...
  if (typeof v !***REMOVED*** 'object' || Array.isArray(v)) {
    return false;
  }
...
Document.prototype.modifiedPaths = function(options) {
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (typeof cur ***REMOVED***= 'object' && cur !***REMOVED*** null) {
      if (cur._doc) {
        cur = cur._doc;
      }
...
Document.prototype.isModified = function(paths, options, modifiedPaths) {
  if (paths) {
    ...
    if (directModifiedPathsObj ***REMOVED*** null) {
      return false;
    }

    if (typeof paths ***REMOVED***= 'string') {
      paths = paths.indexOf(' ') ***REMOVED***= -1 ? [paths] : paths.split(' ');
    }
    ...
    if (ignoreAtomics) {
      directModifiedPaths = directModifiedPaths.filter(path => {
        ...
        if (value != null && value[arrayAtomicsSymbol] != null && value[arrayAtomicsSymbol].$set ***REMOVED***= undefined) {
          return false;
        }
        return true;
      });
    ...
    return isModifiedChild || paths.some(function(path) {
      return directModifiedPaths.some(function(mod) {
        return mod ***REMOVED***= path || path.startsWith(mod + '.');
      });
    });
  }
...
Document.prototype.$isDefault = function(path) {
  ...
  if (typeof path ***REMOVED***= 'string' && path.indexOf(' ') ***REMOVED***= -1) {
...
Document.prototype.isSelected = function isSelected(path) {
  if (this.$__.selected ***REMOVED*** null) {
    return true;
  }
  if (!path) {
    return false;
  ...
  if (path ***REMOVED***= '_id') {
    return this.$__.selected._id !***REMOVED*** 0;
  }

  if (path.indexOf(' ') !***REMOVED*** -1) {
    path = path.split(' ');
  }
  if (Array.isArray(path)) {
  ...
  if (paths.length ***REMOVED***= 1 && paths[0] ***REMOVED***= '_id') {
  ...
  for (const cur of paths) {
    ...
    if (cur.startsWith(pathDot)) {
      return inclusive || cur !***REMOVED*** pathDot;
    }
...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  if (typeof pathsToValidate ***REMOVED***= 'function' || typeof options ***REMOVED***= 'function' || typeof arguments[2] ***REMOVED***= 'function') {
    throw new MongooseError('Document.prototype.validate() no longer accepts a callback');
  }
  this.$op = 'validate';

  if (arguments.length ***REMOVED***= 1) {
    if (typeof arguments[0] ***REMOVED***= 'object' && !Array.isArray(arguments[0])) {
      options = arguments[0];
      pathsToValidate = null;
    }
  }
  if (options && typeof options.pathsToSkip ***REMOVED***= 'string') {
    const isOnePathOnly = options.pathsToSkip.indexOf(' ') ***REMOVED***= -1;
...
function _evaluateRequiredFunctions(doc) {
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (p != null && typeof p.originalRequiredValue ***REMOVED***= 'function') {
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
    ...
    for (const subdoc of topLevelSubdocs) {
      if (subdoc.$basePath) {
        ...
        if (doc.$isModified(fullPathToSubdoc, null, modifiedPaths) &&
          ...
          if (doc.$__.pathsToScopes ***REMOVED*** null) {
  ...
  for (const path of paths) {
    ...
    if (_pathType.$isMongooseDocumentArray) {
      for (const p of paths) {
        if (p ***REMOVED*** null || p.startsWith(_pathType.path + '.')) {
          paths.delete(p);
        }
      }
    }
    ...
    if (!_pathType.caster && _pathType.validators.length ***REMOVED***= 0 && !_pathType.$parentSchemaDocArray) {
      paths.delete(path);
    } else if (_pathType.$isMongooseArray &&
      !_pathType.$isMongooseDocumentArray && // Skip document arrays...
      !_pathType.$embeddedSchemaType.$isMongooseArray && // and arrays of arrays
      _pathType.validators.length ***REMOVED***= 0 && // and arrays with top-level validators
      _pathType.$embeddedSchemaType.validators.length ***REMOVED***= 0) {
      paths.delete(path);
    }
  }
  ...
  for (const path of paths) {
    ...
    if (_pathType.$parentSchemaDocArray && typeof _pathType.$parentSchemaDocArray.path ***REMOVED***= 'string') {
...
Document.prototype.$__validate = function(pathsToValidate, options, callback) {
  ...
  } else if (typeof pathsToValidate ***REMOVED***= 'function') {
    callback = pathsToValidate;
    options = null;
    pathsToValidate = null;
  } else if (typeof options ***REMOVED***= 'function') {
    callback = options;
    options = null;
  }
  ...
  const _complete = () => {
    ...
    if (shouldValidateModifiedOnly && validationError != null) {
      ...
      if (Object.keys(validationError.errors).length ***REMOVED***= 0) {
  ...
  if (typeof pathsToValidate ***REMOVED***= 'string') {
    pathsToValidate = pathsToValidate.split(' ');
  }

  if (paths.length ***REMOVED***= 0) {
  ...
  function validatePath(path) {
    if (path ***REMOVED*** null || validated[path]) {
      return;
    }
    ...
    immediate(function() {
      ...
      if (schemaType[schemaMixedSymbol] != null && path !***REMOVED*** schemaType.path) {
        return --total || complete();
      }
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  if (arguments.length ***REMOVED***= 1 && typeof arguments[0] ***REMOVED***= 'object' && !Array.isArray(arguments[0])) {
    options = arguments[0];
    pathsToValidate = null;
  }
  ...
  if (typeof pathsToValidate ***REMOVED***= 'string') {
    const isOnePathOnly = pathsToValidate.indexOf(' ') ***REMOVED***= -1;
    pathsToValidate = isOnePathOnly ? [pathsToValidate] : pathsToValidate.split(' ');
  } else if (typeof pathsToSkip ***REMOVED***= 'string' && pathsToSkip.indexOf(' ') !***REMOVED*** -1) {
...
Document.prototype.invalidate = function(path, err, val, kind) {
  ...
  if (!err || typeof err ***REMOVED***= 'string') {
  ...
  if (this.$__.validationError ***REMOVED***= err) {
...
Document.prototype.$markValid = function(path) {
  ...
  if (Object.keys(this.$__.validationError.errors).length ***REMOVED***= 0) {
    this.$__.validationError = null;
  }
};
...
function _markValidSubpaths(doc, path) {
  ...
  if (Object.keys(doc.$__.validationError.errors).length ***REMOVED***= 0) {
...
function _checkImmutableSubpaths(subdoc, schematype, priorVal) {
  ...
  for (const key of Object.keys(schema.paths)) {
    ...
    if (path.$immutableSetter ***REMOVED*** null) {
      continue;
    }
    const oldVal = priorVal ***REMOVED*** null ? void 0 : priorVal.$__getValue(key);
 ...
 *     newProduct ***REMOVED***= product; // true
 *
...
Document.prototype.$isValid = function(path) {
  if (this.$__.validationError ***REMOVED*** null || Object.keys(this.$__.validationError.errors).length ***REMOVED***= 0) {
    return true;
  }
  if (path ***REMOVED*** null) {
    return false;
  ...
  if (Array.isArray(path)) {
    return path.some(p => this.$__.validationError.errors[p] ***REMOVED*** null);
  }

  return this.$__.validationError.errors[path] ***REMOVED*** null;
};
...
Document.prototype.$__undoReset = function $__undoReset() {
  if (this.$__.backup ***REMOVED*** null || this.$__.backup.activePaths ***REMOVED*** null) {
    return;
  }
...
Document.prototype.$__dirty = function() {
  ...
  all = all.concat(this.$__.activePaths.map('default', function(path) {
    if (path ***REMOVED***= '_id' || _this.$__getValue(path) ***REMOVED*** null) {
      return;
    }
    return {
      path: path,
...
Document.prototype.$__setSchema = function(schema) {
  ...
  if (schema.path('schema') ***REMOVED*** null) {
    this.schema = schema;
  }
...
function applyQueue(doc) {
  ...
  for (const pair of q) {
    if (pair[0] !***REMOVED*** 'pre' && pair[0] !***REMOVED*** 'post' && pair[0] !***REMOVED*** 'on') {
...
Document.prototype.$toObject = function(options, json) {
  ...
  if (virtuals || (getters && virtuals !***REMOVED*** false)) {
    applyVirtuals(this, ret, options, options);
  }

  if (options.versionKey ***REMOVED***= false && this.$__schema.options.versionKey) {
    delete ret[this.$__schema.options.versionKey];
  }

  const transform = options._calledWithOptions.transform ?? true;
  let transformFunction = undefined;
  if (transform ***REMOVED***= true) {
    transformFunction = defaultOptions.transform;
  } else if (typeof transform ***REMOVED***= 'function') {
  ...
  if (typeof transformFunction ***REMOVED***= 'function') {
    const xformed = transformFunction(this, ret, options);
    if (typeof xformed !***REMOVED*** 'undefined') {
      ret = xformed;
    }
  }
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  let cur = self._doc;
  let v;
  const aliases = typeof (toObjectOptions && toObjectOptions.aliases) ***REMOVED***= 'boolean'
  ...
  for (i = 0; i < numPaths; ++i) {
    ...
    if (assignPath.indexOf('.') ***REMOVED***= -1 && assignPath ***REMOVED***= path) {
...
function applyGetters(self, json) {
  ...
  while (i--) {
    ...
    for (let ii = 0; ii < plen; ++ii) {
      ...
      if (branch != null && typeof branch !***REMOVED*** 'object') {
        break;
      } else if (ii ***REMOVED***= last) {
...
function applySchemaTypeTransforms(self, json) {
  ...
  for (const path of paths) {
    ...
    if (typeof topLevelTransformFunction ***REMOVED***= 'function') {
    ...
    } else if (typeof embeddedSchemaTypeTransformFunction ***REMOVED***= 'function') {
      const val = self.$get(path);
      if (val ***REMOVED***= undefined) {
        continue;
      }
...
function omitDeselectedFields(self, json) {
  ...
  if (selected ***REMOVED***= void 0) {
    selected = {};
    queryhelpers.applyPaths(selected, schema);
  }
  if (selected ***REMOVED*** null || Object.keys(selected).length ***REMOVED***= 0) {
    return json;
  }
...
Document.prototype.populate = async function populate() {
  const pop = {};
  const args = [...arguments];
  if (typeof args[args.length - 1] ***REMOVED***= 'function') {
...
Document.prototype.populated = function(path, val, options) {
  ...
  if (val ***REMOVED*** null || val ***REMOVED***= true) {
    ...
    if (v) {
      return val ***REMOVED***= true ? v : v.value;
    }
    return undefined;
  }
...
Document.prototype.depopulate = function(path) {
  if (typeof path ***REMOVED***= 'string') {
    path = path.indexOf(' ') ***REMOVED***= -1 ? [path] : path.split(' ');
  }
  ...
  for (const singlePath of path) {
    ...
    if (virtualKeys.indexOf(singlePath) !***REMOVED*** -1) {
...
Document.prototype.$__delta = function $__delta() {
  ...
  if (!dirty.length && VERSION_ALL !***REMOVED*** this.$__.version) {
    return;
  }
  const where = {};
  const delta = {};
  ...
  for (; d < len; ++d) {
    ...
    if (!pop && this.$__.selected) {
      ...
      if (this.$__.selected[top] && this.$__.selected[top].$elemMatch) {
        ...
        if (pathSplit.length > 1 && pathSplit[1] ***REMOVED*** 0 && typeof where[top] ***REMOVED***= 'undefined') {
    ...
    if (this.$isDefault(data.path) && this.$__.selected) {
      if (data.path.indexOf('.') ***REMOVED***= -1 && isPathExcluded(this.$__.selected, data.path)) {
        continue;
      }
  ...
  if (Object.keys(delta).length ***REMOVED***= 0) {
    return [where, null];
  }
...
function checkDivergentArray(doc, path, array) {
  ...
  const check = pop.options.match ||
      ...
      (pop.options.select._id ***REMOVED***= 0 ||
      /\s?-_id\s?/.test(pop.options.select));

  if (check) {
    const atomics = array[arrayAtomicsSymbol];
    if (Object.keys(atomics).length ***REMOVED***= 0 || atomics.$set || atomics.$pop) {
      return path;
    }
  }
}
...
function operand(self, where, delta, data, val, op) {
  ...
  // disabled versioning?
  if (self.$__schema.options.versionKey ***REMOVED***= false) return;

  // path excluded from versioning?
  if (shouldSkipVersioning(self, data.path)) return;

  // already marked for versioning?
  if (VERSION_ALL ***REMOVED***= (VERSION_ALL & self.$__.version)) return;
  ...
  if (op ***REMOVED***= '$push' || op ***REMOVED***= '$addToSet' || op ***REMOVED***= '$pullAll' || op ***REMOVED***= '$pull') {
...
function handleAtomics(self, where, delta, data, value) {
  ...
  if (typeof value.$__getAtomics ***REMOVED***= 'function') {
...
Document.prototype.$__hasOnlyPrimitiveValues = function $__hasOnlyPrimitiveValues() {
  return !this.$__.populated && !this.$__.wasPopulated && (this._doc ***REMOVED*** null || Object.values(this._doc).every(v => {
    return v ***REMOVED*** null
      || typeof v !***REMOVED*** 'object'
