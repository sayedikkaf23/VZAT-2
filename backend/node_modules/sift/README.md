...
Checks if `query ***REMOVED***= value`. Note that **\$eq can be omitted**. For **\$eq**, and **\$ne**
...
[{ name: "frank" }, { name: "joe" }].filter(
  sift({ $where: "this.name ***REMOVED***= 'frank'" }),
); // ["frank"]
[{ name: "frank" }, { name: "joe" }].filter(
  sift({
    $where: function () {
      return this.name ***REMOVED***= "frank";
    },
  }),
); // ["frank"]
```
