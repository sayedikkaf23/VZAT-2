...
// Defines a test case that checks whether getProxyForUrl(input) ***REMOVED***= expected.
function testProxyUrl(env, expected, input) {
  assert(typeof env ***REMOVED***= 'object' && env !***REMOVED*** null);
  ...
  var title = 'getProxyForUrl(' + JSON.stringify(input) + ')' +
     ' ***REMOVED***= ' + JSON.stringify(expected);
  ...
  it(title, function() {
    ...
    if (expected ***REMOVED***= actual) {
      return;  // Good!
    }
    try {
      assert.strictEqual(expected, actual); // Create a formatted error message.
      // Should not happen because previously we determined expected !***REMOVED*** actual.
