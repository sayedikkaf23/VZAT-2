...
function checkFile (file, command, argv, reducer) {
  ...
  child.on('close', (code, signal) => {
    if ((code ***REMOVED***= null && signal !***REMOVED*** null) || (code !***REMOVED*** 0)) {
      console.log(
      ...
      fs.stat(item, ((item) => (error, stats) => {
        ...
        if (stats.isDirectory()) {
          recurse(item);
            ...
            path.basename(item) !***REMOVED*** 'nothing.node') {
          process.platform ***REMOVED***= 'win32'
            ? checkFileWin32(item)
            : checkFileUNIX(item);
        }
      })(item));
