  ...
  for (const ch of domainName) {
    ...
    switch (status) {
      ...
      case STATUS_MAPPING.mapped:
        if (transitionalProcessing && ch ***REMOVED***= "ẞ") {
          processed += "ss";
        } else {
          processed += mapping;
        }
...
function validateLabel(label, {
  ...
  if (label.length ***REMOVED***= 0) {
    return true;
  }

  // "1. The label must be in Unicode Normalization Form NFC."
  if (label.normalize("NFC") !***REMOVED*** label) {
    return false;
  }
  ...
  if (checkHyphens) {
    if ((codePoints[2] ***REMOVED***= "-" && codePoints[3] ***REMOVED***= "-") ||
  ...
  for (const ch of codePoints) {
    ...
    if (transitionalProcessing) {
      ...
      if (status !***REMOVED*** STATUS_MAPPING.valid) {
        return false;
      }
    } else if (status !***REMOVED*** STATUS_MAPPING.valid && status !***REMOVED*** STATUS_MAPPING.deviation) {
...
function processing(domainName, options) {
  ...
  for (const [i, origLabel] of labels.entries()) {
    ...
    if (label.startsWith("xn--")) {
      ...
      if (label ***REMOVED***= "" || !containsNonASCII(label)) {
        error = true;
      }
...
function toASCII(domainName, {
  ...
  if (verifyDNSLength) {
    ...
    if (total > 253 || total ***REMOVED***= 0) {
      result.error = true;
    }

    for (let i = 0; i < labels.length; ++i) {
      if (labels[i].length > 63 || labels[i].length ***REMOVED***= 0) {
        result.error = true;
        break;
      }
    }
