...
function build () {
  var win32 = os.platform() ***REMOVED***= 'win32'
  var shell = win32
  ...
  try {
    ...
    args = [
      process.execPath,
      path.join(require.resolve('node-gyp/package.json'), '..', typeof pkg.bin ***REMOVED***= 'string' ? pkg.bin : pkg.bin['node-gyp']),
      'rebuild'
    ]
    shell = false
  } catch (_) {}
...
function exec (cmd) {
  if (process.platform !***REMOVED*** 'win32') {
    var shell = os.platform() ***REMOVED***= 'android' ? 'sh' : true
...
function buildFromSource () {
  return hasFlag('--build-from-source') || process.env.npm_config_build_from_source ***REMOVED***= 'true'
}

function verbose () {
  return hasFlag('--verbose') || process.env.npm_config_loglevel ***REMOVED***= 'verbose'
}
...
function hasFlag (flag) {
  ...
  try {
    return JSON.parse(process.env.npm_config_argv).original.indexOf(flag) !***REMOVED*** -1
  } catch (_) {
    return false
  }
}
