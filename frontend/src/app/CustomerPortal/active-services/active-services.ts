...
export class ActiveServices implements OnInit {
  ...
  get paginationInfo(): string {
    ...
    if (totalItems ***REMOVED***= 0) return 'Showing 0 entries';
  ...
  goToPage(page: number): void {
    if (page >= 1 && page <= this.totalPages && page !***REMOVED*** this.currentPage) {
  ...
  getCurrentPayment(service: PaymentScheduleService): PaymentScheduleService | null {
    ...
    return payments.find(p => p.status ***REMOVED***= 'due' || p.status ***REMOVED***= 'overdue') || null;
  }
